...
Static Function ReportDef()
    ...
    __oTBxCanc  := F130Sel(__lCachedQry, FwExecStatement():New( '' ), FWPreparedStatement():New( '' ))
    __oProcSE5	:= F130Sel(__lCachedQry, FwExecStatement():New( '' ), FWPreparedStatement():New( '' ))
    __oF130Dbx	:= F130Sel(__lCachedQry, FwExecStatement():New( '' ), FWPreparedStatement():New( '' ))
    __oTipoBa	:= F130Sel(__lCachedQry, FwExecStatement():New( '' ), FWPreparedStatement():New( '' ))

    __cDBType		:= Alltrim(Upper(TCGetDB()))
    __lBQ10925		:= SuperGetMV("MV_BQ10925",,"2") == "1"
    __lProcSaldoTit	:= .F. //ExistProc( F130Sel( FindFunction( "GetSPName" ), GetSPName( "FIN002", "10" ), "FIN002" ), STATICCALL(FINXFIN,VERIDPROC) ) -- STATICCALL restrito a fonte padrao TOTVS (C9909 em customizacao), neutralizado
...
Static Function ReportPrint(oReport)
    ...
    AEval(aSM0Bkp, {|filial| F130Sel(filial[SM0_GRPEMP] == cEmpAnt, AAdd(aSM0, AClone(filial)), Nil)})
    FwFreeArray(aSM0Bkp)
    ...
    If nOrdem == 5 //natureza
        oBreak := TRBreak():New(oSection1, {|| F130Sel(!MV_MULNATR,SE1->E1_FILIAL + SE1->E1_NATUREZ,aDados[NATUREZA])} , {|| STR0037 + " " + cNomNat })
        oBrkNat := oBreak
        If MV_MULNATR
    ...
    Elseif nOrdem == 4 .Or. nOrdem == 6 //Data do vencimento e emissao
        oBreak  := TRBreak():New(oSection1, {|| SE1->E1_FILIAL + F130Sel(nOrdem == 4, F130Sel(mv_par40 = 2, Dtos(SE1->E1_VENCTO), Dtos(SE1->E1_VENCREA)), Dtos(SE1->E1_EMISSAO))} , {|| STR0037 + DtoC(dDtVenc) }) //"S U B - T O T A L ---->"
        oBreak2 := TRBreak():New(oSection1, {|| SE1->E1_FILIAL + F130Sel(nOrdem == 4, SubStr(F130Sel(mv_par40 = 2, Dtos(SE1->E1_VENCTO), Dtos(SE1->E1_VENCREA)),1,6), Substr(Dtos(SE1->E1_EMISSAO),1,6))} , {|| STR0041 + "("+AllTrim(Str(nTotTitMes))+" "+F130Sel(nTotTitMes > 1, OemToAnsi(STR0039), OemToAnsi(STR0040))+")"} ) //"T O T A L  D O  M E S --->"
        If mv_par19 == 1 //1 = Analitico   2 = Sintetico
            ...
            TRFunction():New(oSection1:Cell("VAL_SOMA"),"","ONPRINT",oBreak2,,cPictToTit,{|lSection, lReport| F130Sel(lReport, nTotGeral, nTotMes)},.F.,.F.)
        Endif
    ...
    ElseIf nOrdem == 7 //vencto e banco
        oBreak := TRBreak():New(oSection1, {||SE1->E1_FILIAL + F130Sel(MV_PAR40=2,DtoC(SE1->E1_VENCTO)+SE1->E1_PORTADO+SE1->E1_AGEDEP+SE1->E1_CONTA,DtoC(SE1->E1_VENCREA)+SE1->E1_PORTADO+SE1->E1_AGEDEP+SE1->E1_CONTA)},;
        {||STR0037 + DtoC(dDtVenc) + F130Sel(!Empty(cNumBco), " - " + STR0066 + " " + cNumBco + " " + ;
    ...
    If lTotFil .Or. !(mv_par22 == mv_par23)
        ...
        If mv_par19 == 1	//1- Analitico  2-Sintetico
            ...
            TRFunction():New(oSection1:Cell("VAL_SOMA"),"","ONPRINT",oBrkFil,,cPictToTit,{|lSection,lReport| F130Sel(lReport, nTotGeral, nTotFil)},.F.,.F.)
        EndIf
    EndIf
    ...
    If lTotEmp
        ...
        If mv_par19 == 1	//1- Analitico  2-Sintetico
            ...
            TRFunction():New(oSection1:Cell("VAL_SOMA"),"","ONPRINT",oBrkEmp,,cPictToTit,{|lSection,lReport| F130Sel(lReport, nTotGeral, nTotEmp)},.F.,.F.)
        EndIf
    Endif
    /* GESTAO - fim
    */
    If mv_par19 == 1 //1 = Analitico - 2 = Sintetico
        ...
        oReport:SetTotalText({|| STR0038 + "(" + AllTrim(Str(nTotTit))+" "+F130Sel(nTotTit > 1, STR0039, STR0040)+")" + F130Sel(nTxZero > 0,STR0087, " ")}) //"  ****** SALDO INCORRETO, POIS EXISTEM TÕTULOS EM MOEDAS ESTRANGEIRAS CONVERTIDAS COM TAXA DE MOEDA ZERO. VERIFIQUE QUAIS OS TÕTULOS COM A DESCRI«√O 'SEM TAXA' NA COLUNA HIST”RICO"
        ...
        TRFunction():New(oSection1:Cell("VAL_SOMA"),"","ONPRINT",oBreak,,cPictToTit,{|lSection, lReport| F130Sel(lReport, nTotGeral, nTotVenc)},.F.,.T.)
    Endif
    ...
    If !lConFilAbx
        cFilDe  := F130Sel( __lFWCodFil, FWGETCODFILIAL, SM0->M0_CODFIL )
        cFilAte := F130Sel( __lFWCodFil, FWGETCODFILIAL, SM0->M0_CODFIL )
    ELSE
    ...
    DbSelectArea("SE1")
    DbCloseArea()
    cTMPSE1 := F130Sel( MV_MULNATR .And. nOrdem == 5, "SE1NAT","SE1")
    ...
    Elseif nOrdem = 4
        ...
        EndIf
        cCond1	:= F130Sel(mv_par40 == 2, "SE1->E1_VENCTO", "SE1->E1_VENCREA")+" <= mv_par10"
        cCond2	:= F130Sel(mv_par40 == 2, "SE1->E1_VENCTO", "SE1->E1_VENCREA")
    ...
    Elseif nOrdem == 7
        ...
        EndIf
        cCond1	:= F130Sel(mv_par40 = 2, "SE1->E1_VENCTO", "SE1->E1_VENCREA")+" <= mv_par10"
        cCond2	:= "DtoS("+F130Sel(mv_par40 = 2, "SE1->E1_VENCTO", "SE1->E1_VENCREA")+")+SE1->E1_PORTADO+SE1->E1_AGEDEP+SE1->E1_CONTA"
    ...
    aEval(aStru,{|Estrutura| F130Sel( Estrutura[2] <> "M", cCampos += "," + AllTrim(Estrutura[1]) ,Nil)})
    ...
    If MV_PAR33 != 3
        ...
        cWhrFase2 += F130Sel(Empty(MV_PAR32), "", " AND SE1.E1_TIPO NOT IN " + FormatIn(MV_PAR32,";"))
    ...
    If MV_MULNATR .And. nOrdem == 5 
        If nLenSelFil == 0
            // Cria vetor com os codigos das filiais da empresa corrente
            aFiliais := FinRetFil()
            lContinua := SM0->(!Eof()) .And. SM0->M0_CODIGO == cEmpAnt .and. F130Sel( __lFWCodFil, FWGETCODFILIAL, SM0->M0_CODFIL ) <= cFilAte
        Else
            aFiliais    := Aclone(aSelFil)
            cFilDe      := aSelFil[1]
            cFilAte     := aSelFil[nLenSelFil]
        ...
        While lContinua
            ...
            Endif
            cFilAnt := F130Sel( __lFWCodFil, FWGETCODFILIAL, SM0->M0_CODFIL )
        ...
        While !(SE1->(Eof()))
            ...
            If SE1->E1_FILIAL == cFilSE1 .And. &cCond1          
                ...
                While !Eof() .And. SE1->E1_FILIAL == cFilSE1 .And. &cCond1
                    ...
                    dDataAnt := F130Sel(nOrdem == 6 , SE1->E1_EMISSAO,  F130Sel(mv_par40 = 2, SE1->E1_VENCTO, SE1->E1_VENCREA))
                    cCarAnt  := &cCond2
                    ...
                    If SE1->E1_FILIAL == cFilSE1 .And. &cCond2 == cCarAnt                        
                        While !Eof() .And. SE1->E1_FILIAL == cFilSE1 .And. &cCond2 == cCarAnt                            
                            ...
                            dDtCtb	:= F130Sel(Empty(SE1->E1_EMIS1), SE1->E1_EMISSAO, SE1->E1_EMIS1)
                            ...
                            */
                            ...
                            If (SE1->E1_TIPO $ MVABATIM	+"/"+MVFUABT .And. mv_par33 != 1) .Or. (SE1->E1_EMISSAO > mv_par36 .and. MV_PAR38 == 2)
                                cMTitPai := F130Sel(!Empty(SE1->E1_TITPAI), SE1->E1_TITPAI, U_FTITPAI())
                            ...
                            If !Empty(SE1->E1_BAIXA) .And. lClcMultLj
                                If !Empty(dDtBxMulta := F130ULBX(cFilSE5, SE1->E1_PREFIXO, SE1->E1_NUM, SE1->E1_PARCELA, SE1->E1_TIPO, SE1->E1_CLIENTE ))
                                    ...
                                    If lAchouDt  := (lSldRetro .And. dDataBase < dDtBxMulta)
                                        dDtBxMulta := FR130DBX()
                                    EndIf
                                    dUltBaixa := F130Sel(lAchouDt, dDtBxMulta,FR130DBX())
                                EndIf
                            EndIf
                            ...
                            If lSldRetro	// Saldo Retroativo = Sim
                                ...
                                nTxMoedSld := F130Sel(mv_par39==2,F130Sel(!Empty(SE1->E1_TXMOEDA),SE1->E1_TXMOEDA,nTxSM2),0)
                                ...
                                If lSPFin005 .And. SE1->E1_MOEDA > 1 .And. SE1->E1_DECRESC > 0 .And. SE1->E1_SDDECRE == 0
                                    nTxMoeda := F130Sel(MV_PAR39 == 2,F130Sel(!Empty(SE1->E1_TXMOEDA),SE1->E1_TXMOEDA,nTxSM2),nTxSM2)
                                ...
                                If SE1->E1_DECRESC > 0
                                    ...
                                    ElseIf Round(NoRound(xMoeda(SE1->E1_VALOR,SE1->E1_MOEDA,mv_par15,dDataReaj,nDecs+1,F130Sel(mv_par39==2,F130Sel(!Empty(SE1->E1_TXMOEDA),SE1->E1_TXMOEDA,RecMoeda(SE1->E1_EMISSAO,SE1->E1_MOEDA)),0)),nDecs+1),nDecs) == nSaldo
                                        nSaldo := Round(NoRound(xMoeda(SE1->E1_VALOR-SE1->E1_DECRESC,SE1->E1_MOEDA,mv_par15,dDataReaj,nDecs+1,F130Sel(mv_par39==2,F130Sel(!Empty(SE1->E1_TXMOEDA),SE1->E1_TXMOEDA,RecMoeda(SE1->E1_EMISSAO,SE1->E1_MOEDA)),0)),nDecs+1),nDecs)
                                    Endif
                                EndIf                                
                                // Soma Acrescimo para recompor o saldo na data escolhida.
                                If SE1->E1_ACRESC > 0 .And. F130Sel(Empty(dUltBaixa), .T., dDataBase < dUltBaixa)
                                    If  mv_par15 == SE1->E1_MOEDA .and. Str(SE1->E1_VALOR,17,2) == Str(nSaldo,17,2)
                                        nSaldo += SE1->E1_ACRESC
                                    ElseIf Round(NoRound(xMoeda(SE1->E1_VALOR,SE1->E1_MOEDA,mv_par15,dDataReaj,nDecs+1,F130Sel(mv_par39==2,F130Sel(!Empty(SE1->E1_TXMOEDA),SE1->E1_TXMOEDA,RecMoeda(SE1->E1_EMISSAO,SE1->E1_MOEDA)),0)),nDecs+1),nDecs) == nSaldo
                                        nSaldo := Round(NoRound(xMoeda(SE1->E1_VALOR+SE1->E1_ACRESC,SE1->E1_MOEDA,mv_par15,dDataReaj,nDecs+1,F130Sel(mv_par39==2,F130Sel(!Empty(SE1->E1_TXMOEDA),SE1->E1_TXMOEDA,RecMoeda(SE1->E1_EMISSAO,SE1->E1_MOEDA)),0)),nDecs+1),nDecs)
                                    EndIf
                                EndIf
                                ...
                                If SE1->E1_TIPO $ MVABATIM +"/"+MVFUABT .And. ((SE1->E1_BAIXA <= dDataBase .And. !Empty(SE1->E1_BAIXA)) .Or. (SE1->E1_MOVIMEN <= dDataBase .And. !Empty(SE1->E1_MOVIMEN))) .And. SE1->E1_SALDO == 0
                                    ...
                                    If mv_par33 == 1 .And. MV_PAR37 > 1 .And. !Empty(cMTitPai)
                                        ...
                                        If !oJsonTitul:HasProperty(cChaveJson)
                                            ...
                                            oJsonTitul[cChaveJson] := FR130DBX(cChaveTit, F130Sel(MV_PAR37 == 2, "FK1_DTDISP", "FK1_DTDIGI")) 
                                        EndIf
                                    EndIf
                                ...
                                If cPaisLoc == "BRA"
                                    If (( cMVBR10925 == "1" .and. SE1->E1_EMISSAO <= MV_PAR36 .and. !(SE1->E1_TIPO $ "PIS/COF/CSL").and. !(SE1->E1_TIPO $ MVABATIM) ) ;
                                        ...
                                        !(SE1->E1_TIPO $ MVIRF).and. !(SE1->E1_TIPO $ MVABATIM) .AND. ( (SA1->(A1_RECIRRF) $ "1 " ) .And. SA1->A1_TPESSOA == 'EP' ) )
                                                ...
                                                If (F130Sel(!Empty(SE1->E1_NUMBOR),!Alltrim(SEA->EA_ORIGEM) $ "FINA061|FINA590I",.T.) .and. SE1->E1_DATABOR <= MV_PAR36)                     
                                        ...
                                        If nRecSA1 > 0
                                            ...
                                            while  SE5->(!EOF()) .And. SE5->(MsSeek(cFilSE5+SE1->(E1_PREFIXO+E1_NUM+E1_PARCELA+E1_TIPO+E1_CLIENTE+E1_LOJA)+cSeq));
                                                ...
                                                nSaldo	:= F130Sel(SE5->E5_DATA <= mv_par36,  (nSaldo - SE5->E5_VRETIRF), nSaldo)
                                ...
                                EndIf
                            Else
                                nSaldo := xMoeda((SE1->E1_SALDO+SE1->E1_SDACRES-SE1->E1_SDDECRE),SE1->E1_MOEDA,mv_par15,dDataReaj,ndecs+1,F130Sel(MV_PAR39==2,F130Sel(!Empty(SE1->E1_TXMOEDA),SE1->E1_TXMOEDA,RecMoeda(SE1->E1_EMISSAO,SE1->E1_MOEDA)),0))
                            Endif
                            ...
                            If !(!Empty( SE1->E1_BAIXA ) .AND. SE1->E1_BAIXA < dDatabase) .Or. cMvDesFin == "P"
                                ...
                                If Mv_par15 > 1
                                    If SE1->E1_MOEDA == Mv_par15
                                        nDescont := xMoeda((nDescont),1,Mv_par15,dDataReaj,ndecs+1,F130Sel(MV_PAR39==2,F130Sel(!Empty(SE1->E1_TXMOEDA),SE1->E1_TXMOEDA,RecMoeda(SE1->E1_EMISSAO,SE1->E1_MOEDA)),0))
                                    Else
                                        nDescont := xMoeda((nDescont),SE1->E1_MOEDA,mv_par15,dDataReaj,ndecs+1,F130Sel(MV_PAR39==2,F130Sel(!Empty(SE1->E1_TXMOEDA),SE1->E1_TXMOEDA,RecMoeda(SE1->E1_EMISSAO,SE1->E1_MOEDA)),0))
                                    EndIf
                                EndIf
                                If nDescont > 0
                            ...
                            aDados[CLIENT] 	:= RTrim(SE1->E1_CLIENTE) + "-" + SE1->E1_LOJA + "-" + F130Sel(mv_par29 == 1, SubStr(GetLGPDValue('SA1', 'A1_NREDUZ'),1,20), SubStr(GetLGPDValue('SA1','A1_NOME'),1,20))
                            ...
                            If !ltxZero
                                aDados[VL_ORIG] := Round(NoRound(xMoeda(SE1->E1_VALOR,SE1->E1_MOEDA,mv_par15,SE1->E1_EMISSAO,ndecs+1,F130Sel(!Empty(SE1->E1_TXMOEDA),SE1->E1_TXMOEDA,Nil)) * F130Sel((SE1->E1_TIPO$MVABATIM +"/"+MVFUABT+"/"+MV_CRNEG+"/"+MVRECANT+"/"+MVABATIM) .OR. lAbatIMPBx, -1,1),nDecs+1),nDecs)
                            Else
                                aDados[VL_ORIG] := 0
                            Endif
                            aDados[VL_NOMINAL] := 0
                            ...
                            If dDataBase > dVencRea	//vencidos
                                If mv_par19 == 1 //1 = Analitico - 2 = Sintetico
                                    If !ltxZero
                                        aDados[VL_NOMINAL] := nSaldo * F130Sel(SE1->E1_TIPO$MVRECANT+"/"+MV_CRNEG+"/"+MVABATIM +"/"+MVFUABT, -1,1)
                                    Else
                                        aDados[VL_NOMINAL] := 0
                                    Endif
                                EndIf
                                ...
                                If !( SE1->E1_TIPO $ MVRECANT + "|" + MV_CRNEG ) .And. ( lFJurCst .Or. !Empty(SE1->E1_VALJUR) .Or. !Empty(SE1->E1_PORCJUR) .Or. lClcMultLj )
                                    ...
                                    nJuros := FA070Juros(MV_PAR15, nBaseJuros + F130Sel(MV_PAR33 == 2, nAbatim, 0), "SE1", dUltBaixa,,, lSldRetro, dMVDtBase)

                                    If lClcMultLj .And. !lBxMulta 
                                        //Calcula o valor da Multa  :funcao LojxRMul :fonte Lojxrec 
                                        nMulta := LojxRMul(,,,nSaldo + F130Sel(MV_PAR33 == 2,nAbatim,0),SE1->E1_ACRESC,SE1->E1_VENCREA,dDataBase,lAchouDt,SE1->E1_MULTA,,SE1->E1_PREFIXO,SE1->E1_NUM,SE1->E1_PARCELA,SE1->E1_TIPO,SE1->E1_CLIENTE,SE1->E1_LOJA,"SE1",.T.)
                                ...
                                If lFValAcess .And. lCheckVA
                                    nVa := FValAcess(SE1->E1_PREFIXO, SE1->E1_NUM, SE1->E1_PARCELA, SE1->E1_TIPO, SE1->E1_CLIENTE, SE1->E1_LOJA, SE1->E1_NATUREZ,;
                                        F130Sel(Empty(SE1->E1_BAIXA), .F., .T.), "", "R", Nil, Nil, SE1->E1_MOEDA, MV_PAR15, SE1->E1_TXMOEDA, Nil, lSldRetro)
                                EndIf
                                ...
                                If mv_par19 == 1 //1 = Analitico - 2 = Sintetico
                                    If !ltxZero
                                        aDados[VL_CORRIG] := (nSaldo+nJuros+nMulta)* F130Sel(SE1->E1_TIPO$MVRECANT+"/"+MV_CRNEG+"/"+MVABATIM +"/"+MVFUABT, -1,1)
                                        lVl_Corr := .F.
                                    Else
                                        aDados[VL_CORRIG] := 0
                                    Endif
                                ...
                                If SE1->E1_TIPO $ MVRECANT+"/"+MV_CRNEG .or. (mv_par33 == 1 .and. SE1->E1_TIPO $ MVABATIM +"/"+MVFUABT)
                                    nTit0 -= Round(NoRound(xMoeda(SE1->E1_VALOR,SE1->E1_MOEDA,mv_par15,SE1->E1_EMISSAO,ndecs+1,F130Sel(MV_PAR39==2,F130Sel(!Empty(SE1->E1_TXMOEDA),SE1->E1_TXMOEDA,RecMoeda(SE1->E1_EMISSAO,SE1->E1_MOEDA)),0)),ndecs+1),ndecs)
                                    nTit1 -= (nSaldo)
                                    nTit2 -= (nSaldo+nJuros+nMulta)
                                    nMesTit0 -= Round(NoRound( xMoeda(SE1->E1_VALOR,SE1->E1_MOEDA,mv_par15,SE1->E1_EMISSAO,ndecs+1,F130Sel(MV_PAR39==2,F130Sel(!Empty(SE1->E1_TXMOEDA),SE1->E1_TXMOEDA,RecMoeda(SE1->E1_EMISSAO,SE1->E1_MOEDA)),0)),ndecs+1),ndecs)
                                    nMesTit1 -= (nSaldo)
                            ...
                            Else //a vencer
                                ...
                                If mv_par19 == 1 //1 = Analitico - 2 = Sintetico
                                    If !ltxZero
                                        aDados[VL_VENCIDO] := nSaldo * F130Sel((SE1->E1_TIPO$MV_CRNEG+"/"+MVRECANT+"/"+MVABATIM +"/"+MVFUABT) .OR. lAbatIMPBx, -1,1)
                            ...
                            If dDataBase > SE1->E1_VENCREA .And. !(SE1->E1_TIPO $ MVRECANT+"/"+MV_CRNEG)
                                ...
                                nAtraso := F130Sel(nAtraso < 0, 0, nAtraso)
                            ...
                            If mv_par19 == 1 //1 = Analitico - 2 = Sintetico 18
                                If !ltxZero
                                    aDados[HISTORICO] := F130Sel(lDvc,SE1->E1_HIST,SubStr(SE1->E1_HIST,1,25)) + F130Sel(E1_TIPO $ MVPROVIS,"*"," ") + ;
                                    F130Sel(Str(xMoeda(SE1->E1_SALDO,SE1->E1_MOEDA,mv_par15,dDataReaj,ndecs+1,F130Sel(MV_PAR39==2,F130Sel(!Empty(SE1->E1_TXMOEDA),SE1->E1_TXMOEDA,RecMoeda(SE1->E1_EMISSAO,SE1->E1_MOEDA)),0)),17,2) ==Str(xMoeda(SE1->E1_VALOR,SE1->E1_MOEDA,mv_par15,dDataReaj,ndecs+1,F130Sel(MV_PAR39==2,F130Sel(!Empty(SE1->E1_TXMOEDA),SE1->E1_TXMOEDA,RecMoeda(SE1->E1_EMISSAO,SE1->E1_MOEDA)),0)),17,2)," ","P")
                                Else
                            ...
                            If nOrdem == 1 //Cliente
                                cNomFor := F130Sel(mv_par29 == 1, AllTrim(SA1->A1_NREDUZ),AllTrim(SA1->A1_NOME)) +" "+Substr(F130Sel(!Empty(cCampoCli),SA1->(&cCampocli),SA1->A1_DDD+"-"+SA1->A1_TEL),1,18)
                            Elseif nOrdem == 8 //codigo do cliente
                                cNomFor := SA1->A1_COD+" "+SA1->A1_LOJA+" "+AllTrim(SA1->A1_NOME)+" "+Substr(F130Sel(!Empty(cCampoCli),SA1->(&cCampocli),SA1->A1_DDD+"-"+SA1->A1_TEL),1,18)
                            Endif
                            ...
                            dDtVenc := F130Sel(nOrdem == 4 .OR. nOrdem == 7, F130Sel(mv_par40 = 2, SE1->E1_VENCTO, SE1->E1_VENCREA), SE1->E1_EMISSAO)
                    ...
                    //¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
                    lQuebra := .F.
                    If nOrdem == 4  .and. (Month(F130Sel(mv_par40 = 2, SE1->E1_VENCTO, SE1->E1_VENCREA)) # Month(dDataAnt) .or. F130Sel(mv_par40 = 2, SE1->E1_VENCTO, SE1->E1_VENCREA) > mv_par10)
                        If(mv_par19 == 2) //1 = Analitico - 2 = Sintetico
                            lQuebra := .T.
                        Else
                    ...
                    If lQuebra .and. nMesTTit # 0
                        ...
                        If nOrdem == 4  .and. (Month(F130Sel(mv_par40 = 2, SE1->E1_VENCTO, SE1->E1_VENCREA)) # Month(dDataAnt) .or.;
                        F130Sel(mv_par40 = 2, SE1->E1_VENCTO, SE1->E1_VENCREA) > mv_par10) .And. (mv_par19 == 2)
                            nMesTit0 := nMesTit1 := nMesTit2 := nMesTit3 := nMesTit4 := nMesTTit := nMesTitJ := 0
                        EndIf
                    Endif
            ...
            If nTotFil0 <> 0
                nTotFil := F130Sel(nTotFil2>0,nTotFil2+nTotFil3,nTotFil3)  //F130Sel(mv_par33 == 23, nTotFil0, nTotFil2+nTotFil3)
                nTotEmp += nTotFil
                cNomFil := cFilAnt + " - " + AllTrim(FwFilialName(cEmpAnt, cFilAnt))
                cNomEmp := Substr(cFilAnt, 1, nTamEmp) + " - " + AllTrim(FwFilRazSocial(cEmpAnt, cFilAnt))
            EndIf
...
If nOrdem = 1
    //mv_par29 - Imprime Nome?
    cQuebra := F130Sel(mv_par29 == 1,Substr(GetLGPDValue('SA1', 'A1_NREDUZ'),1,30),Substr(GetLGPDValue('SA1', 'A1_NOME'),1,30)) + " " + STR0054 + Right(cCarAnt,2)+F130Sel(lConFilAbx,STR0055+cFilAnt + " - " + cFilName,"")//"Loja - "###" Filial - "
Elseif nOrdem == 4 .or. nOrdem == 6
    cQuebra := PadR(STR0037,28) + DtoC(cCarAnt) + "  " + F130Sel(lConFilAbx,cFilAnt+ " - " + cFilName,"  ")
Elseif nOrdem = 3
    cQuebra := PadR(STR0037,28) + F130Sel(Empty(SA6->A6_NREDUZ),STR0029,SA6->A6_NREDUZ) + " " + F130Sel(lConFilAbx,cFilAnt+ " - " + cFilName,"")
ElseIf nOrdem == 5 //por Natureza
    SED->( dbSetOrder( 1 ) )
    SED->( MSSeek(cFilAnt+cCarAnt) )
    cQuebra := PadR(STR0037,28) + cCarAnt + " "+Substr(SED->ED_DESCRIC,1,40) + " " + F130Sel(lConFilAbx,cFilAnt+ " - " + cFilName,"")
Elseif nOrdem == 7
    cQuebra := PadR(STR0037,28) + SubStr(cCarAnt,7,2)+"/"+SubStr(cCarAnt,5,2)+"/"+SubStr(cCarAnt,3,2)+" - "+SubStr(cCarAnt,9,3) + " " +F130Sel(lConFilAbx,cFilAnt+ " - " + cFilName,"")
ElseIf nOrdem = 8
       cQuebra := SA1->A1_COD+" "+Substr(GetLGPDValue('SA1', 'A1_NOME'),1,30) + " " + F130Sel(lConFilAbx,cFilAnt+ " - " + cFilName,"")
ElseIf nOrdem = 9
    cCodCart := SubStr(cCarAnt,__nTamPort+1,1)
    cCarteira := Situcob(cCodCart)
    cQuebra := SA6->A6_COD + " " + SA6->A6_NREDUZ + " " + SubStr(cCarteira,1,20) + " " + F130Sel(lConFilAbx,cFilAnt + " - " + cFilName,"")
Endif
...
GESTAO - inicio */
If Empty(cTexto)
    oSection:Cell("QUEBRA"   ):SetBlock({|| STR0038 +"("+ AllTrim(Str(nTotTit)) +" "+ F130Sel(nTotTit > 1, STR0039, STR0040) +")"}) //"TOTAL"
Else
...
oSection:Cell("QUEBRA"   ):SetBlock({|| PadR(STR0041,28) + "("+ALLTRIM(STR(nMesTTit))+" "+F130Sel(nMesTTit > 1,OemToAnsi(STR0039),OemToAnsi(STR0040))+")"})
...
oSection:Cell("QUEBRA"   ):SetBlock({|| STR0043 + " " + F130Sel(MV_PAR21 == 1,cFilAnt+" - " + AllTrim(FwFilialName(cEmpAnt, cFilAnt)),"")})  //"T O T A L   F I L I A L ----> "
...
cString += 'DTOS('+F130Sel(mv_par40 = 2, 'SE1->E1_VENCTO', 'SE1->E1_VENCREA')+')>="'+DTOS(mv_par09)+'".And.'
cString += 'DTOS('+F130Sel(mv_par40 = 2, 'SE1->E1_VENCTO', 'SE1->E1_VENCREA')+')<="'+DTOS(mv_par10)+'".And.'
    ...
    While (cQryAlias)->( !Eof() )
        If FTITRAPAI((cQryAlias)->E1_TITPAI) //Verifica se o Titulo PAI e um RA e nao abate o PCC
            If AllTrim(cPrefixo + cNumero + cParcela + cTipo + cCodCli + cLoja) == AllTrim((cQryAlias)->E1_TITPAI)
                nTotPcc += xMoeda((cQryAlias)->E1_VALOR,(cQryAlias)->E1_MOEDA,nMoeda,dDataRef,,F130Sel(cPaisLoc=="BRA",(cQryAlias)->E1_TXMOEDA,0))
            EndIf
        EndIf
        (cQryAlias)->(dbSkip())
    EndDo
...
Return( MascNat(SED->ED_CODIGO) + " - " + SED->ED_DESCRIC + F130Sel(MV_PAR21 == 1, cFilAnt + " - " + AllTrim(FwFilialName(cEmpAnt, cFilAnt)), "" ) )
...
nTotal += nTotVenc	:= F130Sel(oTotCorr:GetValue() == NIL .OR. oTotVenc:GetValue() == NIL, nTotVenc, oTotCorr:GetValue() + oTotVenc:GetValue())
...
If !lTemCli
    ...
    If __oTCli == Nil
        ...
        cQuery := ChangeQuery(cQuery)
        __oTCli := F130Sel(__lCachedQry, FwExecStatement():New(cQuery), FWPreparedStatement():New(cQuery))
...
Static Function F130ULBX(cFilSE5 as Char, cPrefSE5 as Char, cNumSE5 as Char, cParcSE5 as Char, cTpSE5 as Char, cCliSE5 as Char) As Date
    ...
    If __oUltBxMl == Nil
        ...
        __oUltBxMl := F130Sel(__lCachedQry, FwExecStatement():New(cQuery), FWPreparedStatement():New(cQuery))
    EndIf

    __oUltBxMl:setUnsafe(1, F130Sel(MV_PAR37==2,"E5_DTDISPO","E5_DATA"))
...
Static Function ExisteFKD(cSGBD As Character, cFilOrig as Character) As Logical 
    ...
    If __oExisFKD == Nil
        ...
        cQuery     := ChangeQuery(cQuery)
        __oExisFKD := F130Sel(__lCachedQry, FwExecStatement():New(cQuery), FWPreparedStatement():New(cQuery))
    EndIf
    
    __oExisFKD:SetString(1,FWXFilial("FKD",cFilOrig))
    __oExisFKD:SetString(2,Space(1))

    lExistFKD := F130Sel(__lCachedQry,__oExisFKD:ExecScalar("FKD_FILIAL", "600", "15"), MpSysExecScalar(__oExisFKD:GetFixQuery(), "FKD_FILIAL")) > 0
...
Static Function BuscaCmp(cFilE5 as Character, cPrefixo as Character, cNumTit as Character, cParcela as Character, cTipo as Character,;
    ...
    If __oBusCmp == Nil 
        ...
        cQuery     := ChangeQuery(cQuery)
        __oBusCmp := F130Sel(lCachQry, FwExecStatement():New(cQuery), FWPreparedStatement():New(cQuery))
    EndIF
    ...
    cAliasSE5 := F130Sel(lCachQry,__oBusCmp:OpenAlias(), MpSysOpenQuery(__oBusCmp:GetFixQuery()))
...
/*/
//-----------------------------------------------------------
Static Function F130Sel(lCond, xValA, xValB)
	Local uRet
