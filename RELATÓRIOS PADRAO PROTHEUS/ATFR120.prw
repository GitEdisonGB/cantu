...
// Obter os campos do Usuario
	...
	bResidual	:= { ||	F120Sel( cMoeda == "1", F120Sel( (TRB->N3_TIPO = "05" .Or.;
	...
	bLucroPreju	:= { ||	F120Sel( cMoeda == "1", (TRB->N4_VENDA - Eval(bResidual)),;
		F120Sel( cMoeda == Getmv("MV_ATFMOED"), ((TRB->N4_VENDA / TRB->N4_TXMEDIA) - Eval(bResidual)),;
		F120Sel( SM2->(MsSeek(TRB->N4_DATA)), (TRB->N4_VENDA / SM2->&("M2_MOEDA"+cMoeda) - Eval(bResidual)), 0))) }
	...
	While !TRB->(Eof())		
		...
		While !TRB->(Eof()) .And. cFilPos == TRB->N3_FILIAL
			...
			While !TRB->(Eof()) .AND. cQuebra == TRB->(&cChave)
				...
				SN1->( DbGoto(TRB->RECSN1) )
				SN3->( DbGoto(TRB->RECSN3) )
				SN4->( MsSeek(cChvSN4))
...
/*/
//-----------------------------------------------------------
Static Function F120Sel(lCond, xValA, xValB)
	Local uRet
