...
cMoeda := F320Sel(cMoeda=="0", "1", cMoeda)
...
If ExistProc( cProcNam, VerIDProc())
	...
	While !oReport:Cancel() .And. !(cAliasTop)->(Eof())	
		...
		aResult := TCSPExec( xProcedures(cProcNam) ,cFilAnt;					// 01. Codigo da Filial Corrente
													...
													,F320Sel(lD3Servico,'1','0');	// 15. Considera o parametro MV_D3SERVI																										
                                                    ,F320Sel(IntDL(),'1','0');     	// 16. Indica se existe integracao com WMS
...
//	cSelect  := "B1_COD,B1_TIPO,B1_UM,B1_GRUPO,B1_DESC," //  comentado esta linha pois ao definir o filtro pelo usuario não 
	...
	If SuperGetMV('MV_D3SERVI', .F., 'N') == 'N' .And. IntDL()
		...
		If cWMSNew
			cWhereD3 += " OR  (D3_SERVIC <> '   ' AND D3_TM > '500' ) )"
		Else
	...
	While !oReport:Cancel() .And. !(cAliasTop)->(Eof())	
		...
		While !oReport:Cancel() .And. !(cAliasTop)->(Eof()) .And. (cAliasTop)->B1_TIPO == cTipAnt
			...
			lPassou := F320Sel(nSaldoAtu > 0, .t., lPassou)
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		...
		If lPassou
			lTotal:=.T.
				...
				nSalant := nSaldoAtu-nCompras-nReqProd-nReqCons-nProducao+nVendas-nReqTrans-F320Sel(!lLocProc, nReqProc, Abs(nReqProc))-nDevVendas+nDevComprs-nEntrTerc+nSaiTerc
...
/*/
User Function EngPos22Compile(cProcesso as character, cEmpresa as character, cProcName as character, cLocalDB as character, cBuffer as character, cError as character)
...
/*/
User Function EngSPS22Signature(cProcess as character)
...
/*/
//-----------------------------------------------------------
Static Function F320Sel(lCond, xValA, xValB)
	Local uRet
