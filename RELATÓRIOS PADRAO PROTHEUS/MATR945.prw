...
If mv_par03 == 1 .Or. mv_par03 == 3 // Notas Fiscais de Entrada
	...
	For nPosFil := 1 to Len(aFilAtiv)
		cFilSF1	+= F945Sel(nPosFil==1,"('","'")+aFilAtiv[nPosFil,2]+F945Sel(nPosFil==Len(aFilAtiv),"')","',")
		cFilSA2	+= F945Sel(nPosFil==1,"('","'")+aFilAtiv[nPosFil,4]+F945Sel(nPosFil==Len(aFilAtiv),"')","',")
	Next
...
If mv_par03 == 2 .Or. mv_par03 == 3 // Notas Fiscais de Saida
	...
	For nPosFil := 1 to Len(aFilAtiv)
		cFilSF2	+= F945Sel(nPosFil==1,"('","'")+aFilAtiv[nPosFil,3]+F945Sel(nPosFil==Len(aFilAtiv),"')","',")
		cFilSD2 := F945Sel(nPosFil==1,"('"," ")+aFilAtiv[nPosFil,3]+F945Sel(nPosFil==Len(aFilAtiv),"')","',")
	Next
	...
	While !oReport:Cancel() .And. !(cAliasSFT)->(Eof());
		...
		Else
			...
			If !(cAliasSFT)->FT_TIPO $ "D/B"
				...
				EndIf
				cTpRur	:= F945Sel(at(SA2->A2_TIPORUR,"FLJ") > 0 ,aPFunRur[at(SA2->A2_TIPORUR,"FLJ")],"")
			...
			Else                                                                                                                
				...
				If (lMTR94501)
					...
					aPFunRur[2] := aAliqFunR[1]
				EndIf
				cTpRur	:= F945Sel(at(SA1->A1_TIPO,"FLJ") > 0 ,aPFunRur[at(SA1->A1_TIPO,"FLJ")],"")
	...
	While !oReport:Cancel() .And. !(cAliasSF2)->(Eof());
		...
		Else
			...
			If (cAliasSF2)->FT_TIPO $ "D/B"
				...
				cCnpj 	:= SA2->A2_CGC
				cNome	:= SA2->A2_NOME
				cTpRur	:= F945Sel(SA2->A2_TIPO != "F",STR0013,STR0014)
				cTpCliFor := SA2->A2_TIPORUR
			Else                                                                                                       
				SA1->(dbSetOrder(1))
				...
				cCnpj 	:= SA1->A1_CGC
				cNome	:= SA1->A1_NOME
				cTpRur	:= F945Sel(SA1->A1_TIPO != "F",STR0013,STR0014)
				cTpCliFor := SA1->A1_TIPO
			Endif	
...
nTipo:=F945Sel(aReturn[4]==1,15,18)
...
For nForFilial := 1 to Len( aFilsCalc )
	...
	If aFilsCalc[ nForFilial, 1 ]
		...
		If mv_par03 == 1 // Notas Fiscais de Entrada
			...
			cFilSF	:= ''
			For nPosFil := 1 to Len(aFilAtiv)
				cFilSF	+= F945Sel(nPosFil==1,"('","'")+aFilAtiv[nPosFil,2]+F945Sel(nPosFil==Len(aFilAtiv),"')","',")
			Next
		...
		ElseIf mv_par03 == 2 // Notas Fiscais de Saida
			...
			cFilSF	:= ''
			For nPosFil := 1 to Len(aFilAtiv)
				cFilSF	+= F945Sel(nPosFil==1,"('","'")+aFilAtiv[nPosFil,3]+F945Sel(nPosFil==Len(aFilAtiv),"')","',")
			Next
		...
		nTamReg:=F945Sel(nTamReg>0,nTamReg,2)
		SetRegua(nTamReg)
		nDiaAnt:=mv_par01
		...
		While !((cAliasSFT)->(Eof())) .And. (cAliasSFT)->FT_FILIAL $ cFilSF .And. (cAliasSFT)->FT_EMISSAO >= mv_par01 .And. (cAliasSFT)->FT_EMISSAO <= mv_par02 .And. mv_par03 != 2
			...
			@ li,060 PSAY F945Sel((cAliasSFT)->FT_TIPO$"D/B",Substr(SA1->A1_NOME,1,20),Substr(SA2->A2_NOME,1,20))
			...
			@ li,076 PSAY F945Sel(nPosPer > 0 ,aPFunRur[nPosPer],"")
			
			@ li,099 PSAY F945Sel((cAliasSFT)->FT_TIPO$"D/B",Transform(SA1->A1_CGC,PicPesFJ(F945Sel(Len(alltrim(SA1->A1_CGC))<14,"F","J"))),Transform(SA2->A2_CGC,PicPesFJ(F945Sel(Len(alltrim(SA2->A2_CGC))<14,"F","J"))))	
			...
			@ li,189 PSAY F945Sel((cAliasSFT)->FT_TIPO == "D","D","")
		...
		While !((cAliasSF2)->(Eof())) .And. (cAliasSF2)->FT_FILIAL $ cFilSF .And. (cAliasSF2)->FT_EMISSAO >= mv_par01 .And. (cAliasSF2)->FT_EMISSAO <= mv_par02 .And. mv_par03 != 1 .and. !lEnd
			...
			@ li,060 PSAY F945Sel((cAliasSF2)->FT_TIPO$"D/B", Substr(SA2->A2_NOME,1,20),Substr(SA1->A1_NOME,1,20))
			
			@ li,084 PSAY F945Sel(SA1->A1_TIPO != "F",STR0013,STR0014) //"Nao Cons."###"Consum."
			@ li,098 PSAY F945Sel((cAliasSF2)->FT_TIPO$"D/B",Transform(SA2->A2_CGC,PicPesFJ(F945Sel(Len(alltrim(SA2->A2_CGC))<14,"F","J"))),Transform(SA1->A1_CGC,PicPesFJ(F945Sel(Len(alltrim(SA1->A1_CGC))<14,"F","J"))))	
			...
			@ li,199 PSAY F945Sel((cAliasSF2)->FT_TIPO == "D","D","")
...
/*/
//-----------------------------------------------------------
Static Function F945Sel(lCond, xValA, xValB)
	Local uRet
