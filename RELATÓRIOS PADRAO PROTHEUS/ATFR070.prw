...
/*
* Validação feita quando selecionado multifiliais é preciso que todos os MV_ULTDEPR estejam com a mesma data.
*/
If MV_PAR15 == 1 .and. len(aSelfil) > 1 .and. !VldUltDepr(aSelFil)
	Help(,,"ULTDEPRC",,STR0064,1,0) // "Umas das filiais selecionadas está com a data de última depreciação divergente."
	Return
EndIf
...
Local cDescri :=	OemToAnsi(STR0001) + " " +;	// "Este programa ir  emitir a posi‡„o valorizada dos"
...
Local cPerg   := "AFR070"   
Local nTamConta := TamSx3('CT1_CONTA')[1] + 6
Local nTamCT1Desc := TamSx3('CT1_DESC01')[1]
oReport  := TReport():New( cReport, cTitulo, cPerg, bReport, cDescri )

oSection1 := TRSection():New( oReport, STR0035,,aOrd ) // "Entidade Contabil"
TRCell():New( oSection1, "ENTIDADES"	, "",STR0063 /*X3Titulo*/, /*Picture*/, nTamConta /*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/) //"Código Entidade"
TRCell():New( oSection1, "DESCENTID"	, "",STR0060 /*X3Titulo*/, /*Picture*/, nTamCT1Desc/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/) //Desc Moeda1
oSection1:SetHeaderSection(.F.)

oSection11 := TRSection():New( oSection1, STR0036 , {"SN3"}) // "Dados da Entidade"
...
oSection12 := TRSection():New( oSection1, STR0037,{"SN3"}) // "Valores"
...
TRCell():New( oSection12, "N3_VRDMES"		, "", STR0027 /*X3Titulo*/, /*Picture*/, 18    /*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)  //"Deprec. no mês"
...
TRCell():New( oSubTotal, "N3_VRDMES"		, "", STR0027 /*X3Titulo*/, /*Picture*/, 18    /*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)  //"Deprec. no mês"
...
TRCell():New( oBaixa, "N3_VRDMES"		, "", STR0027 /*X3Titulo*/, /*Picture*/, 18    /*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)  //"Deprec. no mês"
...
TRCell():New( oTotBaixa, "N3_VRDMES"		, "", STR0027 /*X3Titulo*/, /*Picture*/, 18    /*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)  //"Deprec. no mês"
...
TRCell():New( oTotalGeral , "N3_VRDMES"		, "", STR0027 /*X3Titulo*/, /*Picture*/, 18    /*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)  //"Deprec. no mês"
...
TRCell():New( oTotGerBx , "N3_VRDMES"		, "", STR0027 /*X3Titulo*/, /*Picture*/, 18    /*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)  //"Deprec. no mês"
...
TRCell():New( oTotal, "N3_VRDMES"		, "", STR0027 /*X3Titulo*/, /*Picture*/, 18    /*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)  //"Deprec. no mês"
...
Local oSection12		as object
Local oTxtSubtotal		as object
Local oSubtotal			as object
Local oTxtBaixa			as object
Local oBaixa			as object
Local oTxtTotBx			as object
Local oTotBaixa			as object
Local oTxtTotGer		as object
Local oTotalGeral		as object
Local oTxtTotGBx		as object
Local oTotGerBx			as object
Local oTxtTotal			as object
Local oTotal			as object
Local aTotGeral			as array
Local aTQuebra1  		as array
Local aTotBaixa	 		as array
Local aTotGBx	 		as array
Local nSuf       		as numeric
Local nMoedas    		as numeric
Local lDescricao 		as logical
Local cMoedas    		as character
Local nQuantidade 		as numeric
Local nQuantBx			as numeric
Local nTotal 	 		as numeric
Local nTotBx 	 		as numeric
Local nOrdem 			as numeric
Local dUltDepr   		as date
Local cGrupoPass 		as character
Local lCtb 		 		as logical
Local cAliasQry			as character
Local nCount        	as numeric
Local cAtfCont	 		as character
Local cQuery			as character
Local cOrdem			as character
Local nCpo				as numeric
Local cSuf				as character
Local nVlrAtual			as numeric
Local cCabCond1 		as character
Local cCabCond2 		as character
Local cFilQbr1			as character
Local cEntQbr1  		as character
Local n_pag 			as numeric
Local cTmpSN3Fi1		as character
Local cCampSn3 			as character
...
oSection12	 	:= oReport:Section(1):Section(2)
oTxtSubtotal 	:= oReport:Section(1):Section(3)
oSubtotal 	 	:= oReport:Section(1):Section(4)
oTxtBaixa	 	:= oReport:Section(2)
oBaixa	 	 	:= oReport:Section(3)
oTxtTotBx	 	:= oReport:Section(4)
oTotBaixa	 	:= oReport:Section(5)
oTxtTotGer	 	:= oReport:Section(6)
oTotalGeral	 	:= oReport:Section(7)
oTxtTotGBx	 	:= oReport:Section(8)
oTotGerBx	 	:= oReport:Section(9)
oTxtTotal	 	:= oReport:Section(10)
oTotal	 	 	:= oReport:Section(11)
aTotGeral  		:= {}
aTQuebra1  		:= {}
aTotBaixa	 	:= {}
aTotGBx	  		:= {}
...
cGrupoPass 		:= SuperGetMv("MV_GRPASS")
lCtb 		 	:= CtbInUse()
cAliasQry		:= ""
nCount        	:= 0
cAtfCont	 	:= SuperGetMv("MV_ATFCONT")
cQuery			:= ""
cOrdem			:= ""
nCpo			:= 0
cSuf			:= ""
...
oReport:OnPageBreak( {|| F070Sel((n_pag+1) > mv_par12, (n_pag := mv_par13,oReport:SetPageNumber(n_pag-1)),n_pag += 1) } ) 
...
cMoedas += F070Sel(MV_PAR06 == 1, "S", "N");    nMoedas += F070Sel(MV_PAR06 == 1, 1, 0)
cMoedas += F070Sel(MV_PAR07 == 1, "S", "N");    nMoedas += F070Sel(MV_PAR07 == 1, 1, 0)
cMoedas += F070Sel(MV_PAR08 == 1, "S", "N");    nMoedas += F070Sel(MV_PAR08 == 1, 1, 0)
cMoedas += F070Sel(MV_PAR09 == 1, "S", "N");    nMoedas += F070Sel(MV_PAR09 == 1, 1, 0)
...
cQuery := " SELECT "
	For  nCount := 1 to len(aCpos)
			cCampSn3 += aCpos[nCount][1] +","	  	
	Next nCount
	cCampSn3 := substr(cCampSn3,1,len(cCampSn3)-1)
cQuery += cCampSn3
cQuery += " , R_E_C_N_O_ "
cQuery += "  FROM "+RetSqlName("SN3")+" "
If MV_PAR15 == 1 .and. len(aSelfil) > 1
...
While (cAliasQry)->(!Eof()) .And. !oReport:Cancel()
	...
	SN3->(dbGoto((cAliasQry)->R_E_C_N_O_ )) //POSICIONA NA SN3 para personalizado o SN3 precisa estar posicionado
	...
	While (cAliasQry)->(!Eof()) .AND. &cCond1 .And. !oReport:Cancel()
		...
		SN3->(dbGoto((cAliasQry)->R_E_C_N_O_ )) //POSICIONA NA SN3 para personalizado o SN3 precisa estar posicionado
		//Filtra as classificações patrimoniais. Não fiz na query pra não alterar a estrutura
		If Len(aSelClass) > 0
						...
						If SubStr(cMoedas,nSuf,1) == "S"
							...
							If lAtClDepr .OR. SN1->N1_PATRIM $ "P "
								...
								If Month((cAliasQry)->N3_DTBAIXA) == Month(dUltDepr) .AND. Year((cAliasQry)->N3_DTBAIXA) == Year(dUltDepr)
									aTotBaixa[nSuf][03] += F070Sel(Empty(SN1->N1_DTBLOQ) .Or. Dtos(SN1->N1_DTBLOQ) < Dtos(dUltDepr),&(cAliasQry + '->N3_VRDMES'+cSuf),0)
									IF cSuf == "1"
		...
		For nSuf := 1 To 5
			...
			IF SubStr(cMoedas,nSuf,1) == "S"
				...
				oSection12:Cell("SIMBMOEDA"):SetBlock( { || SuperGetMv("MV_SIMB"+cSuf) } )
				...
				oSection12:Cell("VLATUALIZADO"):SetBlock( { || F070Sel( cSuf == "1", nVlrAtual, 0 ) } )
				oSection12:Cell("VLATUALIZADO"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VORIG1" ,19,nSuf) )
				
				oSection12:Cell("N3_VRDMES"):SetBlock( { || F070Sel( Empty(SN1->N1_DTBLOQ) .Or. Dtos(SN1->N1_DTBLOQ) < Dtos(dUltDepr), &(cAliasQry + '->N3_VRDMES'+cSuf), 0 ) } )
				oSection12:Cell("N3_VRDMES"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRDMES"+cSuf,18,nSuf) )
				...
				oSection12:Cell("N3_VRCDA1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDA1",19,1) )
				...
				If lAtClDepr .OR. SN1->N1_PATRIM $ "P "
					...
					aTquebra1[nSuf][03] += F070Sel(Empty(SN1->N1_DTBLOQ) .Or. Dtos(SN1->N1_DTBLOQ) < Dtos(dUltDepr),&(cAliasQry + '->N3_VRDMES'+cSuf),0)
	...
	If aTQuebra1[1][12] != 0
		...
		For nSuf := 1 To 5
			...
			IF SubStr(cMoedas,nSuf,1) == "S"
				...
				oSubTotal:Cell("VLATUALIZADO"):SetBlock( { || F070Sel( cSuf == "1", aTQuebra1[nSuf][02], 0 ) } )
				oSubTotal:Cell("VLATUALIZADO"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VORIG1" ,19,nSuf) )
				
				oSubTotal:Cell("N3_VRDMES"):SetBlock( { || aTQuebra1[nSuf][03] } )
				oSubTotal:Cell("N3_VRDMES"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRDMES"+cSuf,18,nSuf) )
				...
				oSubTotal:Cell("N3_VRCMES1"):SetBlock( { || F070Sel( cSuf == "1", aTQuebra1[nSuf][06], 0 ) } )
				oSubTotal:Cell("N3_VRCMES1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCMES1",15,1) )
				
				oSubTotal:Cell("N3_VRCBAL1"):SetBlock( { || F070Sel( cSuf == "1", aTQuebra1[nSuf][07], 0 ) } )
				oSubTotal:Cell("N3_VRCBAL1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCBAL1",17,1) )
				
				oSubTotal:Cell("N3_VRCACM1"):SetBlock( { || F070Sel( cSuf == "1", aTQuebra1[nSuf][08], 0 ) } )
				oSubTotal:Cell("N3_VRCACM1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCACM1",19,1) )
				
				oSubTotal:Cell("N3_VRCDM1"):SetBlock( { || F070Sel( cSuf == "1", aTQuebra1[nSuf][09], 0 ) } )
				oSubTotal:Cell("N3_VRCDM1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDM1",15,1) )
				
				oSubTotal:Cell("N3_VRCDB1"):SetBlock( { || F070Sel( cSuf == "1", aTQuebra1[nSuf][10], 0 ) } )
				oSubTotal:Cell("N3_VRCDB1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDB1",17,1) )
				
				oSubTotal:Cell("N3_VRCDA1"):SetBlock( { || F070Sel( cSuf == "1", aTQuebra1[nSuf][11], 0 ) } )
				oSubTotal:Cell("N3_VRCDA1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDA1",19,1) )
				If oSection1:GetOrder() == 1
					If lCtb
	...
	If mv_par14 == 1	//Imprime total de baixas
		If aTotBaixa[1][12] != 0
			...
			For nSuf := 1 To 5
				cSuf      := Str(nSuf,1)
				If SubStr(cMoedas,nSuf,1) == "S"
					oBaixa:Cell("SIMBMOEDA"):SetBlock( { || SuperGetMv("MV_SIMB"+cSuf) } )
					...
					oBaixa:Cell("VLATUALIZADO"):SetBlock( { || F070Sel( cSuf == "1", aTotBaixa[nSuf][02], 0 ) } )
					oBaixa:Cell("VLATUALIZADO"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VORIG1" ,19,nSuf) )
					
					oBaixa:Cell("N3_VRDMES"):SetBlock( { || aTotBaixa[nSuf][03] } )
					oBaixa:Cell("N3_VRDMES"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRDMES"+cSuf,18,nSuf) )
					...
					oBaixa:Cell("N3_VRCMES1"):SetBlock( { || F070Sel( cSuf == "1", aTotBaixa[nSuf][06], 0 ) } )
					oBaixa:Cell("N3_VRCMES1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCMES1",15,1) )
					
					oBaixa:Cell("N3_VRCBAL1"):SetBlock( { || F070Sel( cSuf == "1", aTotBaixa[nSuf][07], 0 ) } )
					oBaixa:Cell("N3_VRCBAL1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCBAL1",17,1) )
					
					oBaixa:Cell("N3_VRCACM1"):SetBlock( { || F070Sel( cSuf == "1", aTotBaixa[nSuf][08], 0 ) } )
					oBaixa:Cell("N3_VRCACM1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCACM1",19,1) )
					
					oBaixa:Cell("N3_VRCDM1"):SetBlock( { || F070Sel( cSuf == "1", aTotBaixa[nSuf][09], 0 ) } )
					oBaixa:Cell("N3_VRCDM1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDM1",15,1) )
					
					oBaixa:Cell("N3_VRCDB1"):SetBlock( { || F070Sel( cSuf == "1", aTotBaixa[nSuf][10], 0 ) } )
					oBaixa:Cell("N3_VRCDB1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDB1",17,1) )
					
					oBaixa:Cell("N3_VRCDA1"):SetBlock( { || F070Sel( cSuf == "1", aTotBaixa[nSuf][11], 0 ) } )
					oBaixa:Cell("N3_VRCDA1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDA1",19,1) )
					If nOrdem == 1
						If lCtb
							If SubStr(CT1->CT1_CONTA,1,1) $ cGrupoPass .And. CT1->CT1_NORMAL == "1"
		...
		If aTotBaixa[1][12] != 0
			...
			For nSuf := 1 To 5
				cSuf      := Str(nSuf,1)
				If SubStr(cMoedas,nSuf,1) == "S"
					oTotBaixa:Cell("SIMBMOEDA"):SetBlock( { || SuperGetMv("MV_SIMB"+cSuf) } )
					...
					oTotBaixa:Cell("VLATUALIZADO"):SetBlock( { || F070Sel( cSuf == "1", aTQuebra1[nSuf][02]+aTotBaixa[nSuf][02], 0 ) } )
					oTotBaixa:Cell("VLATUALIZADO"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VORIG1" ,19,nSuf) )
					
					oTotBaixa:Cell("N3_VRDMES"):SetBlock( { || aTQuebra1[nSuf][03]+aTotBaixa[nSuf][03] } )
					oTotBaixa:Cell("N3_VRDMES"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRDMES"+cSuf,18,nSuf) )
					...
					oTotBaixa:Cell("N3_VRCMES1"):SetBlock( { || F070Sel( cSuf == "1", aTQuebra1[nSuf][06]+aTotBaixa[nSuf][06], 0 ) } )
					oTotBaixa:Cell("N3_VRCMES1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCMES1",15,1) )
					
					oTotBaixa:Cell("N3_VRCBAL1"):SetBlock( { || F070Sel( cSuf == "1", aTQuebra1[nSuf][07]+aTotBaixa[nSuf][07], 0 ) } )
					oTotBaixa:Cell("N3_VRCBAL1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCBAL1",17,1) )
					
					oTotBaixa:Cell("N3_VRCACM1"):SetBlock( { || F070Sel( cSuf == "1", aTQuebra1[nSuf][08]+aTotBaixa[nSuf][08], 0 ) } )
					oTotBaixa:Cell("N3_VRCACM1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCACM1",19,1) )
					
					oTotBaixa:Cell("N3_VRCDM1"):SetBlock( { || F070Sel( cSuf == "1", aTQuebra1[nSuf][09]+aTotBaixa[nSuf][09], 0 ) } )
					oTotBaixa:Cell("N3_VRCDM1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDM1",15,1) )
					
					oTotBaixa:Cell("N3_VRCDB1"):SetBlock( { || F070Sel( cSuf == "1", aTQuebra1[nSuf][10]+aTotBaixa[nSuf][10], 0 ) } )
					oTotBaixa:Cell("N3_VRCDB1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDB1",17,1) )
					
					oTotBaixa:Cell("N3_VRCDA1"):SetBlock( { || F070Sel( cSuf == "1", aTQuebra1[nSuf][11]+aTotBaixa[nSuf][11], 0 ) } )
					oTotBaixa:Cell("N3_VRCDA1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDA1",19,1) )
					oTotBaixa:PrintLine()
				EndIf
			Next
			oTotBaixa:Finish()
...
For nSuf := 1 To 5
	...
	IF SubStr(cMoedas,nSuf,1) == "S"
		...
		oTotalGeral:Cell("VLATUALIZADO"):SetBlock( { || F070Sel( cSuf == "1", aTotGeral[nSuf][02], 0 ) } )
		oTotalGeral:Cell("VLATUALIZADO"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VORIG1" ,19,nSuf) )
		
		oTotalGeral:Cell("N3_VRDMES"):SetBlock( { || aTotGeral[nSuf][03] } )
		oTotalGeral:Cell("N3_VRDMES"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRDMES"+cSuf,18,nSuf) )
		...
		oTotalGeral:Cell("N3_VRCMES1"):SetBlock( { || F070Sel( cSuf == "1", aTotGeral[nSuf][06], 0 ) } )
		oTotalGeral:Cell("N3_VRCMES1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCMES1",15,1) )
		
		oTotalGeral:Cell("N3_VRCBAL1"):SetBlock( { || F070Sel( cSuf == "1", aTotGeral[nSuf][07], 0 ) } )
		oTotalGeral:Cell("N3_VRCBAL1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCBAL1",17,1) )
		
		oTotalGeral:Cell("N3_VRCACM1"):SetBlock( { || F070Sel( cSuf == "1", aTotGeral[nSuf][08], 0 ) } )
		oTotalGeral:Cell("N3_VRCACM1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCACM1",19,1) )
		
		oTotalGeral:Cell("N3_VRCDM1"):SetBlock( { || F070Sel( cSuf == "1", aTotGeral[nSuf][09], 0 ) } )
		oTotalGeral:Cell("N3_VRCDM1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDM1",15,1) )
		
		oTotalGeral:Cell("N3_VRCDB1"):SetBlock( { || F070Sel( cSuf == "1", aTotGeral[nSuf][10], 0 ) } )
		oTotalGeral:Cell("N3_VRCDB1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDB1",17,1) )
		
		oTotalGeral:Cell("N3_VRCDA1"):SetBlock( { || F070Sel( cSuf == "1", aTotGeral[nSuf][11], 0 ) } )
		oTotalGeral:Cell("N3_VRCDA1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDA1",19,1) )
...
If mv_par14 == 1	//Imprime total de baixas
	...
	For nSuf := 1 To 5
		If SubStr(cMoedas,nSuf,1) == "S"
			cSuf      := Str(nSuf,1)
			oTotGerBx:Cell("SIMBMOEDA"):SetBlock( { || SuperGetMv("MV_SIMB"+cSuf) } )
			...
			oTotGerBx:Cell("VLATUALIZADO"):SetBlock( { || F070Sel( cSuf == "1", aTotGBx[nSuf][02], 0 ) } )
			oTotGerBx:Cell("VLATUALIZADO"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VORIG1" ,19,nSuf) )
			
			oTotGerBx:Cell("N3_VRDMES"):SetBlock( { || aTotGBx[nSuf][03] } )
			oTotGerBx:Cell("N3_VRDMES"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRDMES"+cSuf,18,nSuf) )
			...
			oTotGerBx:Cell("N3_VRCMES1"):SetBlock( { || F070Sel( cSuf == "1", aTotGBx[nSuf][06], 0 ) } )
			oTotGerBx:Cell("N3_VRCMES1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCMES1",15,1) )
			
			oTotGerBx:Cell("N3_VRCBAL1"):SetBlock( { || F070Sel( cSuf == "1", aTotGBx[nSuf][07], 0 ) } )
			oTotGerBx:Cell("N3_VRCBAL1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCBAL1",17,1) )
			
			oTotGerBx:Cell("N3_VRCACM1"):SetBlock( { || F070Sel( cSuf == "1", aTotGBx[nSuf][08], 0 ) } )
			oTotGerBx:Cell("N3_VRCACM1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCACM1",19,1) )
			
			oTotGerBx:Cell("N3_VRCDM1"):SetBlock( { || F070Sel( cSuf == "1", aTotGBx[nSuf][09], 0 ) } )
			oTotGerBx:Cell("N3_VRCDM1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDM1",15,1) )
			
			oTotGerBx:Cell("N3_VRCDB1"):SetBlock( { || F070Sel( cSuf == "1", aTotGBx[nSuf][10], 0 ) } )
			oTotGerBx:Cell("N3_VRCDB1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDB1",17,1) )
			
			oTotGerBx:Cell("N3_VRCDA1"):SetBlock( { || F070Sel( cSuf == "1", aTotGBx[nSuf][11], 0 ) } )
			oTotGerBx:Cell("N3_VRCDA1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDA1",19,1) )
			oTotGerBx:PrintLine()
		EndIf
	Next
	...
	For nSuf := 1 To 5
		If SubStr(cMoedas,nSuf,1) == "S"
			cSuf      := Str(nSuf,1)
			oTotal:Cell("SIMBMOEDA"):SetBlock( { || SuperGetMv("MV_SIMB"+cSuf) } )
			...
			oTotal:Cell("VLATUALIZADO"):SetBlock( { || F070Sel( cSuf == "1", aTotGeral[nSuf][02]+aTotGBx[nSuf][02], 0 ) } )
			oTotal:Cell("VLATUALIZADO"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VORIG1" ,19,nSuf) )
			
			oTotal:Cell("N3_VRDMES"):SetBlock( { || aTotGeral[nSuf][03]+aTotGBx[nSuf][03] } )
			oTotal:Cell("N3_VRDMES"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRDMES"+cSuf,18,nSuf) )
			...
			oTotal:Cell("N3_VRCMES1"):SetBlock( { || F070Sel( cSuf == "1", aTotGeral[nSuf][06]+aTotGBx[nSuf][06], 0 ) } )
			oTotal:Cell("N3_VRCMES1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCMES1",15,1) )
			
			oTotal:Cell("N3_VRCBAL1"):SetBlock( { || F070Sel( cSuf == "1", aTotGeral[nSuf][07]+aTotGBx[nSuf][07], 0 ) } )
			oTotal:Cell("N3_VRCBAL1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCBAL1",17,1) )
			
			oTotal:Cell("N3_VRCACM1"):SetBlock( { || F070Sel( cSuf == "1", aTotGeral[nSuf][08]+aTotGBx[nSuf][08], 0 ) } )
			oTotal:Cell("N3_VRCACM1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCACM1",19,1) )
			
			oTotal:Cell("N3_VRCDM1"):SetBlock( { || F070Sel( cSuf == "1", aTotGeral[nSuf][09]+aTotGBx[nSuf][09], 0 ) } )
			oTotal:Cell("N3_VRCDM1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDM1",15,1) )
			
			oTotal:Cell("N3_VRCDB1"):SetBlock( { || F070Sel( cSuf == "1", aTotGeral[nSuf][10]+aTotGBx[nSuf][10], 0 ) } )
			oTotal:Cell("N3_VRCDB1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDB1",17,1) )
			
			oTotal:Cell("N3_VRCDA1"):SetBlock( { || F070Sel( cSuf == "1", aTotGeral[nSuf][11]+aTotGBx[nSuf][11], 0 ) } )
			oTotal:Cell("N3_VRCDA1"):SetPicture( F070Sel( cSuf != "1", "@Z", "" ) + AFPesqPict("SN3","N3_VRCDA1",19,1) )
			oTotal:PrintLine()
		EndIf
	Next
	oTotal:Finish()
...
aSize(aCpos,0)//limpa o array SN3
...
If lCtb
	...
	EndIf
Else
	dbSelectArea( F070Sel( nOrdem == 1, "SI1", "SI3" ) )
	IF MsSeek(xFilial(F070Sel(nOrdem==1,"SI1","SI3"),cFilCab)+cChave)
		cDescr := F070Sel( nOrdem==1,MascaraCTB(cChave),cChave) + " " + F070Sel(nOrdem==1,SI1->I1_DESC,SI3->I3_DESC)
	Endif
EndIf
...
Return(cRetorno)
/*/{Protheus.doc} VldUltDepr
	Irá validar se todas as filiais selecionadas estão com a mesma data no MV_ULTDEPR
	...
	@return lRet
	/*/
Static Function VldUltDepr(aSelfil)
Local lRet     := .T.
...
For nX:= 1 to len(aSelfil)
		cFilAnt  := aSelfil[nX]
		dUltDepr := SuperGetMV('MV_ULTDEPR', .F., '19800101')
		If valtype(dUltDepr) == 'D'
			aAdd(aValid, dUltDepr) 
		else
			lRet := .F.
			Exit
		EndIf	
...
if lRet
	dUltDepr := aValid[1]
	For nX:= 2 to len(aValid)
		if dUltDepr != aValid[nX]
			lRet := .F.
			Exit
		EndIF
	Next nX
...
//----------------------------------------------------------
/*/{Protheus.doc}F070Sel
Seletor generico usado no lugar de IIf()/If() ternario, para
respeitar o padrao do projeto (uso obrigatorio de If/Else/EndIf).

@author Ajuste Grupo Luiz Hohl
@since  01/08/2026
@param lCond Condicao avaliada
@param xValA Valor retornado quando lCond e verdadeiro
@param xValB Valor retornado quando lCond e falso
@return uRet Variant
/*/
//-----------------------------------------------------------
Static Function F070Sel(lCond, xValA, xValB)
	Local uRet
