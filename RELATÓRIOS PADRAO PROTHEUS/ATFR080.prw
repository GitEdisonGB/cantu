...
/* GESTAO - fim*/
	...
	/* Dialog é chamada apenas se o objeto oReport não estiver vazio*/
	If oReport <> nil
		oReport:PrintDialog()  
	EndIf
...
nMoedas += F080Sel(mv_par05 = 1, 1, 0)
nMoedas += F080Sel(mv_par06 = 1, 1, 0)
nMoedas += F080Sel(mv_par07 = 1, 1, 0)
nMoedas += F080Sel(mv_par08 = 1, 1, 0)
nMoedas += F080Sel(mv_par09 = 1, 1, 0)
...
Else
	For nA := nLenSelFil To 1 Step(-1)
		If  Ascan(aSelFil,AllTrim(aSM0[nA,SM0_CODFIL])) == 0 
			Adel(aSM0,nA)
			nInc++
		Endif
	Next
...
For nInc := 1 To Len( aSM0 )
	If aSM0[nInc][SM0_GRPEMP] == cEmpAnt .AND. aSM0[nInc][SM0_CODFIL] >= cFilDe .AND. aSM0[nInc][SM0_CODFIL] <= cFilAte		/* GESTAO */
		...
		While	SN3->(!Eof()) .And. SN3->N3_FILIAL=xFilial("SN3")						
			...
			While	SN3->(!Eof()) .And. xFilial("SN3") + cConta == SN3->N3_FILIAL + SN3->N3_CCONTAB
				...
				While	SN3->(!Eof()) .And. xFilial("SN3") == SN3->N3_FILIAL .and. cConta == SN3->N3_CCONTAB .and. Eval(bCondBk)
					...
					If lAtClDepr .OR. SN1->N1_PATRIM $ " P" .And. F080Sel(Len(aClassif) > 0,SN1->N1_PATRIM $ cClassif,.T.)
						...
						aItemMoeda[1, 2] := aItemMoeda[1, 2] + F080Sel(Empty(SN1->N1_DTBLOQ) .Or. Dtos(SN1->N1_DTBLOQ) < Dtos(dUltDepr), SN3->N3_VRDMES1, 0)
						...
						aItemMoeda[1, 8] := aItemMoeda[1, 8] + F080Sel(lInverte,(SN3->N3_VORIG1+SN3->N3_VRCACM1+SN3->N3_AMPLIA1)*(-1),SN3->N3_VORIG1+SN3->N3_VRCACM1+SN3->N3_AMPLIA1)
						aItemMoeda[1, 9] := aItemMoeda[1, 9] + F080Sel(lInverte,((SN3->N3_VORIG1+SN3->N3_VRCACM1+SN3->N3_AMPLIA1)-(SN3->N3_VRDACM1+SN3->N3_VRCDA1))*(-1),((SN3->N3_VORIG1+SN3->N3_VRCACM1+SN3->N3_AMPLIA1)-(SN3->N3_VRDACM1+SN3->N3_VRCDA1)))
						aItemMoeda[1,10] := aItemMoeda[1,10] + SN3->N3_AMPLIA1
		
						//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
						//³ Valores da Moeda 2.                                        ³
						//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
						aItemMoeda[2, 2] := aItemMoeda[2, 2] + F080Sel(Empty(SN1->N1_DTBLOQ) .Or. Dtos(SN1->N1_DTBLOQ) < Dtos(dUltDepr), SN3->N3_VRDMES2, 0)
						aItemMoeda[2, 5] := aItemMoeda[2, 5] + SN3->N3_VRDACM2
						aItemMoeda[2, 8] := aItemMoeda[2, 8] + F080Sel(lInverte,(SN3->N3_VORIG2+SN3->N3_AMPLIA2)*(-1),SN3->N3_VORIG2+SN3->N3_AMPLIA2)
						aItemMoeda[2, 9] := aItemMoeda[2, 9] + F080Sel(lInverte,((SN3->N3_VORIG2+SN3->N3_AMPLIA2)-SN3->N3_VRDACM2)*(-1),(SN3->N3_VORIG2+SN3->N3_AMPLIA2)-SN3->N3_VRDACM2)
						aItemMoeda[2,10] := aItemMoeda[2,10] + SN3->N3_AMPLIA2
		
						//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
						//³ Valores da Moeda 3.                                        ³
						//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
						aItemMoeda[3, 2] := aItemMoeda[3, 2] + F080Sel(Empty(SN1->N1_DTBLOQ) .Or. Dtos(SN1->N1_DTBLOQ) < Dtos(dUltDepr), SN3->N3_VRDMES3, 0)
						aItemMoeda[3, 5] := aItemMoeda[3, 5] + SN3->N3_VRDACM3
						aItemMoeda[3, 8] := aItemMoeda[3, 8] + F080Sel(lInverte,(SN3->N3_VORIG3+SN3->N3_AMPLIA3)*(-1),SN3->N3_VORIG3+SN3->N3_AMPLIA3)
						aItemMoeda[3, 9] := aItemMoeda[3, 9] + F080Sel(lInverte,((SN3->N3_VORIG3+SN3->N3_AMPLIA3)-SN3->N3_VRDACM3)*(-1),(SN3->N3_VORIG3+SN3->N3_AMPLIA3)-SN3->N3_VRDACM3)
						aItemMoeda[3,10] := aItemMoeda[3,10] + SN3->N3_AMPLIA3
		
						//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
						//³ Valores da Moeda 4.                                        ³
						//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
						aItemMoeda[4, 2] := aItemMoeda[4, 2] + F080Sel(Empty(SN1->N1_DTBLOQ) .Or. Dtos(SN1->N1_DTBLOQ) < Dtos(dUltDepr), SN3->N3_VRDMES4, 0)
						aItemMoeda[4, 5] := aItemMoeda[4, 5] + SN3->N3_VRDACM4
						aItemMoeda[4, 8] := aItemMoeda[4, 8] + F080Sel(lInverte,(SN3->N3_VORIG4+SN3->N3_AMPLIA4)*(-1),SN3->N3_VORIG4+SN3->N3_AMPLIA4)
						aItemMoeda[4, 9] := aItemMoeda[4, 9] + F080Sel(lInverte,((SN3->N3_VORIG4+SN3->N3_AMPLIA4)-SN3->N3_VRDACM4)*(-1),(SN3->N3_VORIG4+SN3->N3_AMPLIA4)-SN3->N3_VRDACM4)
						aItemMoeda[4,10] := aItemMoeda[4,10] + SN3->N3_AMPLIA4
		
						//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
						//³ Valores da Moeda 5.                                        ³
						//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
						aItemMoeda[5, 2] := aItemMoeda[5, 2] + F080Sel(Empty(SN1->N1_DTBLOQ) .Or. Dtos(SN1->N1_DTBLOQ) < Dtos(dUltDepr), SN3->N3_VRDMES5, 0)
						aItemMoeda[5, 5] := aItemMoeda[5, 5] + SN3->N3_VRDACM5
						aItemMoeda[5, 8] := aItemMoeda[5, 8] + F080Sel(lInverte,(SN3->N3_VORIG5+SN3->N3_AMPLIA5)*(-1),SN3->N3_VORIG5+SN3->N3_AMPLIA5)
						aItemMoeda[5, 9] := aItemMoeda[5, 9] + F080Sel(lInverte,((SN3->N3_VORIG5+SN3->N3_AMPLIA5)-SN3->N3_VRDACM5)*(-1),(SN3->N3_VORIG5+SN3->N3_AMPLIA5)-SN3->N3_VRDACM5)
					...
					Else
						...
						aItemMoeda[1, 7] := aItemMoeda[1, 7] - (SN3->N3_VRDACM1 + SN3->N3_VRCDA1)
						aItemMoeda[1, 8] := aItemMoeda[1, 8] - F080Sel(lInverte,(SN3->N3_VORIG1+SN3->N3_VRCACM1+SN3->N3_AMPLIA1)*(-1),(SN3->N3_VORIG1+SN3->N3_VRCACM1+SN3->N3_AMPLIA1))
						aItemMoeda[1, 9] := aItemMoeda[1, 9] - F080Sel(lInverte,(SN3->N3_VORIG1+SN3->N3_VRCACM1+SN3->N3_AMPLIA1)*(-1),(SN3->N3_VORIG1+SN3->N3_VRCACM1+SN3->N3_AMPLIA1))
						aItemMoeda[1,10] := aItemMoeda[1,10] - SN3->N3_AMPLIA1
						...
						aItemMoeda[2, 5] := aItemMoeda[2, 5] - SN3->N3_VRDACM2
						aItemMoeda[2, 8] := aItemMoeda[2, 8] -F080Sel(lInverte,(SN3->N3_VORIG2+SN3->N3_AMPLIA2)*(-1),(SN3->N3_VORIG2+SN3->N3_AMPLIA2))
						aItemMoeda[2, 9] := aItemMoeda[2, 9] -F080Sel(lInverte,(SN3->N3_VORIG2+SN3->N3_AMPLIA2)*(-1),(SN3->N3_VORIG2+SN3->N3_AMPLIA2))
						aItemMoeda[2,10] := aItemMoeda[2,10] - SN3->N3_AMPLIA2
						...
						aItemMoeda[3, 5] := aItemMoeda[3, 5] - SN3->N3_VRDACM3
						aItemMoeda[3, 8] := aItemMoeda[3, 8] - F080Sel(lInverte,(SN3->N3_VORIG3+SN3->N3_AMPLIA3)*(-1),(SN3->N3_VORIG3+SN3->N3_AMPLIA3))
						aItemMoeda[3, 9] := aItemMoeda[3, 9] - F080Sel(lInverte,(SN3->N3_VORIG3+SN3->N3_AMPLIA3)*(-1),(SN3->N3_VORIG3+SN3->N3_AMPLIA3))
						aItemMoeda[3,10] := aItemMoeda[3,10] - SN3->N3_AMPLIA3
						...
						aItemMoeda[4, 2] := aItemMoeda[4, 2] - F080Sel(Empty(SN1->N1_DTBLOQ) .Or. Dtos(SN1->N1_DTBLOQ) < Dtos(dUltDepr), SN3->N3_VRDMES4, 0)
						aItemMoeda[4, 5] := aItemMoeda[4, 5] - SN3->N3_VRDACM4
						aItemMoeda[4, 8] := aItemMoeda[4, 8] - F080Sel(lInverte,(SN3->N3_VORIG3+SN3->N3_AMPLIA3)*(-1),(SN3->N3_VORIG3+SN3->N3_AMPLIA3))
						aItemMoeda[4, 9] := aItemMoeda[4, 9] - F080Sel(lInverte,(SN3->N3_VORIG4+SN3->N3_AMPLIA4)*(-1),(SN3->N3_VORIG4+SN3->N3_AMPLIA4))
						aItemMoeda[4,10] := aItemMoeda[4,10] - SN3->N3_AMPLIA4
						...
						aItemMoeda[5, 5] := aItemMoeda[5, 5] - SN3->N3_VRDACM5
						aItemMoeda[5, 8] := aItemMoeda[5, 8] - F080Sel(lInverte,(SN3->N3_VORIG5+SN3->N3_AMPLIA5)*(-1),(SN3->N3_VORIG5+SN3->N3_AMPLIA5))
						aItemMoeda[5, 9] := aItemMoeda[5, 9] - F080Sel(lInverte,(SN3->N3_VORIG5+SN3->N3_AMPLIA5)*(-1),(SN3->N3_VORIG5+SN3->N3_AMPLIA5))
						aItemMoeda[5,10] := aItemMoeda[5,10] - SN3->N3_AMPLIA5
					EndIf
					lInverte := .F.
					SN3->(dbSkip())
				...
				If lCtb
					...
					cCodReduz := F080Sel(!Empty(CT1->CT1_RES),CT1->CT1_RES,cConta)
				Else
					dbSelectArea("SID")
					dbSetOrder(1)
					dbSeek( xFilial() + cItem )
		
					cDescrItem := F080Sel(Found() .And. !Empty(cItem), SID->ID_DESC, Space(20))
					
					dbSelectArea("SI3")
					dbSetOrder(1)
					dbSeek(xFilial() + cCusto)
					cDescCC := F080Sel(Found() .And. !Empty(cCusto), SI3->I3_DESC, Space(20))			
					...
					cCodReduz := F080Sel(!Empty(SI1->I1_RES),SI1->I1_RES,cConta)
				EndIf
				//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
				...
				If lImprime
					...
					//³ Conta.                                                     ³
					//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
					oSection1:Cell("CT1_CONTA"):SetBlock( {|| F080Sel( mv_par11 = 1, Mascara(cConta,,"C"), cCodReduz) } )
					oSection1:Cell("CT1_DESC01"):SetBlock( {|| F080Sel(lCtb,CT1->CT1_DESC01,SI1->I1_DESC) } )
			...
			If lImprime
				If mv_par12 == 1  //Imprime total da conta
					...
					Else
						oSection1:Cell("CT1_CONTA"):SetBlock( {|| STR0014 } )
						oSection1:Cell("CT1_DESC01"):SetBlock( {|| '  ' } )
						oSection1:Cell("CTT_CUSTO"):SetBlock( {|| '  ' } )
						oSection1:Cell("CTT_DESC01"):SetBlock( {|| '  ' } )
						oSection1:Cell("CTD_ITEM"):SetBlock( {|| '  ' } )
						oSection1:Cell("CTD_DESC01"):SetBlock( {|| '  ' } )
						oSection1:Cell("CTH_CLVL"):SetBlock( {|| '  ' } )
						oSection1:Cell("CTH_DESC01"):SetBlock( {|| '  ' } )
						oSection1:PrintLine()
					EndIf
		...
		If lImprime .And. (nLenSelFil > 1 .Or. (mv_par17 == 1 .and. nInc > 1))		/* GESTAO */
			...
			cTotFilial := STR0025 + " " + F080Sel(nLenSelFil > 0 .Or. mv_par17==1,cFilAnt + " - " + AllTrim(aSM0[nInc][SM0_NOMRED]),"") //"T O T A L   F I L I A L ----> "

			If !(oReport:nDevice == 4 .and. oReport:NEXCELPRINTTYPE == 3)
				oReport:PrintText(cTotFilial)
			Else
				oSection1:Cell("CT1_CONTA"):SetBlock( {|| cTotFilial } )
				oSection1:Cell("CT1_DESC01"):SetBlock( {|| '  ' } )
				oSection1:Cell("CTT_CUSTO"):SetBlock( {|| '  ' } )
				oSection1:Cell("CTT_DESC01"):SetBlock( {|| '  ' } )
				oSection1:Cell("CTD_ITEM"):SetBlock( {|| '  ' } )
		...
		If lTotEmp
			If nInc == Len(aSM0) .Or. !(Substr(aSM0[nInc,SM0_CODFIL],1,nTamEmp) == Substr(aSM0[nInc + 1,SM0_CODFIL],1,nTamEmp))
				...
				If lImprime
					...
					cTotEmp := STR0026 + " " + aSM0[nInc,SM0_EMPRESA] + " " + AllTrim(aSM0[nInc,SM0_DESCEMP]) //"T O T A L   E M P R E S A -->"

					If !(oReport:nDevice == 4 .and. oReport:NEXCELPRINTTYPE == 3)
						oReport:PrintText(cTotEmp)
					Else
						oSection1:Cell("CT1_CONTA"):SetBlock( {|| cTotEmp } )
						oSection1:Cell("CT1_DESC01"):SetBlock( {|| '  ' } )
						oSection1:Cell("CTT_CUSTO"):SetBlock( {|| '  ' } )
						oSection1:Cell("CTT_DESC01"):SetBlock( {|| '  ' } )
						oSection1:Cell("CTD_ITEM"):SetBlock( {|| '  ' } )
...
If lImprime
	...
	Else
		oSection1:Cell("CT1_CONTA"):SetBlock( {|| STR0015 } )
		oSection1:Cell("CT1_DESC01"):SetBlock( {|| '  ' } )
		oSection1:Cell("CTT_CUSTO"):SetBlock( {|| '  ' } )
		oSection1:Cell("CTT_DESC01"):SetBlock( {|| '  ' } )
		oSection1:Cell("CTD_ITEM"):SetBlock( {|| '  ' } )
...
/*/
//-----------------------------------------------------------
Static Function F080Sel(lCond, xValA, xValB)
	Local uRet
