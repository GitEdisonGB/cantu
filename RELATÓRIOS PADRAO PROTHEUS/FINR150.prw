...
Static Function ReportPrint(oReport)
	...
	lLandsCape := (oReport:GetOrientation() == 1)
	...
	Else
		If Empty(aSelFil)
			...
			If lConFilAbx
				...
				AEval(aSM0Bkp, {|filial| F150AddSM0(filial, cEmpAnt, aSM0)})
				FwFreeArray(aSM0Bkp)
	...
	oSection1:Cell("E2_NATUREZ"	):SetBlock({ || F150Natureza(aDados[NATUREZA]) })
	...
	If nOrdem == 2	//NaturezaF
		oBreak	:= TRBreak():New(oSection1, {|| F150ChaveNat(cFilBrk, cNatBrk)}, {|| cNomNat })
		oBrkNat	:= oBreak
	ElseIf nOrdem == 3 .Or. nOrdem == 6	//Vencimento e por Emissao
		oBreak  := TRBreak():New(oSection1, {|| F150VencEmi(nOrdem)}, {|| STR0026 + DToC(dDtVenc)})	//"S U B - T O T A L ----> "
		oBreak3 := TRBreak():New(oSection1, {|| F150MesVencEmi(nOrdem)}, {|| STR0030 + "(" + AllTrim(Str(nTotTitMes)) + " " + OemToAnsi(F150TitPl(nTotTitMes)) + ")"})		//"T O T A L   D O  M E S ---> "
		If MV_PAR20 == 1	//1- Analitico  2-Sintetico
			...
			TRFunction():New(oSection1:Cell("E2_HIST"),"","ONPRINT",oBreak3,,F150PictVal(cPictTit),{|lSection,lReport| F150OnPrint(lReport, nTotGeral, nTotMes)},.F.,.F.)
		EndIf
	...
	If SM0->(Reccount()) > 1 .And. lQtdFil2 .Or. lImpXlsTbl
		...
		Else
			If MV_PAR20 == 1	//1- Analitico  2-Sintetico
				...
				TRFunction():New(oSection1:Cell("E2_HIST"),"","ONPRINT",oBreak2,,F150PictVal(cPictTit),{|lSection,lReport| F150TotFil2(lReport, @nFilTot, @nAuxTotFil, nTotFil)},.F.,.F.)
			EndIf
		EndIf
	EndIf
	...
	If lTotEmp .And. MV_MULNATP .And. nOrdem == 2
		...
		If MV_PAR20 == 1	//1- Analitico  2-Sintetico
			...
			TRFunction():New(oSection1:Cell("VAL_SOMA"),"","ONPRINT",oBrkEmp,,PesqPict("SE2","E2_VALOR"),{|lSection,lReport| F150OnPrint(lReport, nTotGeral, nTotEmp)},.F.,.F.)
		EndIf
	EndIf
	/* GESTAO - fim*/
	...
	If MV_PAR20 == 1	//1- Analitico  2-Sintetico
		...
		oReport:SetTotalText({|| STR0027 + "(" + AllTrim(Str(nTotTit)) + " " + F150TitPl(nTotTit) + ")"})
		...
		TRFunction():New(oSection1:Cell("E2_HIST"), "", "ONPRINT", oBreak,, F150PictVal(cPictTit), {|lSection,lReport| F150OnPrint(lReport, nGerTot, nTotVenc)}, .F., .T.)
	EndIf
	...
	cCampos := ""
	AEval(aStru, {|campo| cCampos := F150AddCampo(campo, cCampos)})
				...
				While &cCond2 == cCarAnt .And. SE2->(!EoF()) .And. lContinua .And. SE2->E2_FILIAL == xFilial("SE2")
					...
					If MV_PAR21 == 1
						...
						nSaldo := SaldoTit(SE2->E2_PREFIXO,SE2->E2_NUM,SE2->E2_PARCELA,SE2->E2_TIPO,SE2->E2_NATUREZ,"P",SE2->E2_FORNECE,MV_PAR15,dDataReaj,,SE2->E2_LOJA,,;
							F150Taxa(lSemTaxaM2, nTaxaDia),F150Fmt34(),,__oTBxCanc, Nil, @lCmpMulFil) // 1 = DT BAIXA    3 = DT DIGIT
						...
						If __lPFIN002 .or. (!__lPFIN002 .and. __cMVFinFix < '20' )
							If lCmpMulFil .And. !lSE2FilCom .And. !lSE5FilCom
								nSaldo -= FRVlCompFil("P",SE2->E2_PREFIXO,SE2->E2_NUM,SE2->E2_PARCELA,SE2->E2_TIPO,SE2->E2_FORNECE,SE2->E2_LOJA,F150Fmt34(),,,,;
								MV_PAR15,SE2->E2_MOEDA,F150Taxa(lSemTaxaM2, nTaxaDia),dDataReaj,.T.)
							EndIf
						EndIf
					Else
						nSaldo := xMoeda((SE2->E2_SALDO+SE2->E2_SDACRES-SE2->E2_SDDECRE),SE2->E2_MOEDA,MV_PAR15,dDataReaj,ndecs+1,F150Taxa(lSemTaxaM2, nTaxaDia))
					EndIf
					...
					aDados[FORNEC]		:= SE2->E2_FORNECE+"-"+SE2->E2_LOJA+"-"+F150NomeFor()
					...
					If SE2->E2_TIPO $ MVABATIM .And. nSaldo > 0
						aDados[VL_ORIG]	:= xMoeda(SE2->E2_VALOR * -1,SE2->E2_MOEDA,MV_PAR15,SE2->E2_EMISSAO,ndecs+1,F150Taxa(lSemTaxaM2, nTaxaDia)) * F150Sinal(SE2->E2_TIPO)
						nSaldo := nSaldo * -1
					Else
						If cPaisLoc <> "BRA"
							If SE2->E2_TXMOEDA <> 0 .AND. se2->e2_moeda>1
								nTxCalc := 0
							Else
								nTxCalc := RecMoeda(dDataReaj , MV_PAR15)
							EndIf
							
							nValtit := xMoeda(SE2->E2_VALOR , SE2->E2_MOEDA  , MV_PAR15, SE2->E2_EMISSAO , ndecs+1 , F150Taxa(lSemTaxaM2, nTaxaDia),  nTxCalc) * F150Sinal(SE2->E2_TIPO)
							aDados[VL_ORIG]	:= round(nValtit,MsDecimais(SE2->E2_MOEDA))
						Else	
							If SE2->E2_TXMOEDA <> 0
								nTxCalc := SE2->E2_TXMOEDA
							Else
								nTxCalc := RecMoeda(dDataReaj , MV_PAR15)
							EndIf
							
							aDados[VL_ORIG]	:= xMoeda(SE2->E2_VALOR , SE2->E2_MOEDA , MV_PAR15 , SE2->E2_EMISSAO , ndecs+1 , F150Taxa(lSemTaxaM2, nTaxaDia),  nTxCalc) * F150Sinal(SE2->E2_TIPO)
						Endif
					EndIf
					...
					If dDataBase > SE2->E2_VENCREA 		//vencidos
						aDados[VL_NOMINAL] := nSaldo * F150Sinal(SE2->E2_TIPO)
						nJuros := 0
						dBaixa := dDataBase
						...
						If lFValAcess .And. lTemVA
							nVa := FValAcess(SE2->E2_PREFIXO, SE2->E2_NUM, SE2->E2_PARCELA, SE2->E2_TIPO, SE2->E2_FORNECE, SE2->E2_LOJA, SE2->E2_NATUREZ, !Empty(SE2->E2_BAIXA), "", "P",,, SE2->E2_MOEDA, MV_PAR15, SE2->E2_TXMOEDA,, (MV_PAR21 == 1))
						Else
							nVa := 0
						EndIf
						If nVa > 0
						...
						aDados[VL_CORRIG] := (nSaldo+nJuros) * F150Sinal(SE2->E2_TIPO)
						If SE2->E2_TIPO $ MVPAGANT+"/"+MV_CPNEG
							nTit0 -= xMoeda(SE2->E2_VALOR,SE2->E2_MOEDA,MV_PAR15,SE2->E2_EMISSAO,ndecs+1,F150Taxa(lSemTaxaM2, nTaxaDia))
							nTit1 -= nSaldo
							nTit2 -= nSaldo+nJuros
							nMesTit0 -= xMoeda(SE2->E2_VALOR,SE2->E2_MOEDA,MV_PAR15,SE2->E2_EMISSAO,ndecs+1,F150Taxa(lSemTaxaM2, nTaxaDia))
							nMesTit1 -= nSaldo
					...
					Else				  //a vencer
						If lFValAcess .And. lTemVA
							...
							!Empty(SE2->E2_BAIXA), "", "P",,, SE2->E2_MOEDA, MV_PAR15, SE2->E2_TXMOEDA,, (MV_PAR21 == 1))
						Else
							nVa := 0
						EndIf

						aDados[VL_VENCIDO] := (nSaldo * F150Sinal(SE2->E2_TIPO)) + nVa
					...
					If MV_PAR20 == 1  //1- Analitico  2-Sintetico
						...
						Else
							cMarcaProv := " "
						EndIf
						aDados[HISTORICO]	:= SubStr(SE2->E2_HIST, 1, 25) + cMarcaProv
					...
					ElseIf nOrdem == 5 //Fornecedor
						cNomFor := AllTrim(F150NomeFor())+" "+SubStr(GetLGPDValue("SA2","A2_TEL"),1,15)
					ElseIf nOrdem == 7	//Codigo Fornecedor
...
Static Function ImpT150R(nTot0,nTot1,nTot2,nTot3,nTot4,nTotTit,nTotJ,nTotTit,oReport,oSection1)
	...
	oSection1:Cell("FORNECEDOR"):SetBlock({|| STR0027 + "(" + AllTrim(Str(nTotTit))+" "+ F150TitPl(nTotTit)+")" })
...
Static Function IMes150R(nMesTit0,nMesTit1,nMesTit2,nMesTit3,nMesTit4,nMesTTit,nMesTitJ,nTotTitMes,oReport,oSection1)
	...
	oSection1:Cell("FORNECEDOR"):SetBlock({|| STR0030 + "("+AllTrim(Str(nTotTitMes))+" "+OemToAnsi(F150TitPl(nTotTitMes))+")" })
...
Static Function VerifProc()
	...
	If __lPFIN002 == Nil
		//StaticCall tem uso restrito a fonte padrao TOTVS (gera C9909 em customizacao) - neutralizado, cai sempre no fallback sem SP
		__lPFIN002	:= .F.
	EndIf
...
//----------------------------------------------------------
/*/{Protheus.doc}F150Taxa
Resolve a taxa usada nas conversoes de moeda do titulo: taxa do
titulo (E2_TXMOEDA) quando o parametro pede taxa do titulo,
senao 1 (sem taxa) ou a taxa do dia, conforme o titulo tenha ou
nao taxa M2. Extraido de expressao If() aninhada (repetida 12x)
para respeitar o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param lSemTaxa Indica se o titulo nao tem taxa M2 na data (lSemTaxaM2)
@param nTaxaDia Taxa do dia calculada (nTaxaDia)
@return nRet Numeric
/*/
//-----------------------------------------------------------
Static Function F150Taxa(lSemTaxa, nTaxaDia)
...
//----------------------------------------------------------
/*/{Protheus.doc}F150Sinal
Retorna o sinal (-1/1) usado nos totalizadores quando o titulo
e de pagamento antecipado/negociacao. Extraido de expressao
If() (repetida 6x) para respeitar o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param cTipo Tipo do titulo (E2_TIPO)
@return nRet Numeric
/*/
//-----------------------------------------------------------
Static Function F150Sinal(cTipo)
	Local nRet as Numeric

	If cTipo $ MV_CPNEG+"/"+MVPAGANT
		nRet := -1
	Else
		nRet := 1
	EndIf
...
//----------------------------------------------------------
/*/{Protheus.doc}F150Fmt34
Resolve o formato de data usado no SaldoTit/FRVlCompFil (3 = dt
digitacao, 1 = dt baixa) conforme o parametro MV_PAR34. Extraido
de expressao IIf() (repetida 2x) para respeitar o padrao do
projeto.
...
//----------------------------------------------------------
/*/{Protheus.doc}F150TitPl
Retorna o texto singular/plural (STR0028/STR0029) usado nos
totalizadores de titulos. Extraido de expressao IIf()/If()
(repetida 4x) para respeitar o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param nQtde Quantidade de titulos
@return cRet Character
/*/
//-----------------------------------------------------------
Static Function F150TitPl(nQtde)
...
//----------------------------------------------------------
/*/{Protheus.doc}F150VencEmi
Retorna a data usada na quebra de secao por vencimento/emissao:
E2_VENCREA quando a ordem e por vencimento (3), senao E2_EMISSAO.
Extraido de expressao IIf() para respeitar o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param nOrdem Ordem de impressao selecionada
@return dRet Date
/*/
//-----------------------------------------------------------
Static Function F150VencEmi(nOrdem)
	Local dRet as Date
...
//----------------------------------------------------------
/*/{Protheus.doc}F150MesVencEmi
Retorna o mes usado na quebra de secao por vencimento/emissao:
mes de E2_VENCREA quando a ordem e por vencimento (3), senao mes
de E2_EMISSAO. Extraido de expressao IIf() para respeitar o
padrao do projeto.
...
/*/
//-----------------------------------------------------------
Static Function F150MesVencEmi(nOrdem)
	Local nRet as Numeric

	If nOrdem == 3
		nRet := Month(SE2->E2_VENCREA)
	Else
		nRet := Month(SE2->E2_EMISSAO)
	EndIf

Return nRet

//----------------------------------------------------------
/*/{Protheus.doc}F150ChaveNat
Retorna a chave usada na quebra de secao por natureza: filial+
natureza do titulo quando nao usa natureza multi-empresa, senao
a chave ja calculada (cFilBrk+cNatBrk). Extraido de expressao
IIf() para respeitar o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param cFilBrk Filial usada na quebra multi-empresa
@param cNatBrk Natureza usada na quebra multi-empresa
@return cRet Character
/*/
//-----------------------------------------------------------
Static Function F150ChaveNat(cFilBrk, cNatBrk)
	Local cRet as Character

	If !MV_MULNATP
		cRet := SE2->(XFILIAL("SE2", E2_FILORIG) + E2_NATUREZ)
	Else
		cRet := cFilBrk + cNatBrk
	EndIf

Return cRet

//----------------------------------------------------------
/*/{Protheus.doc}F150PictVal
Retorna a picture usada na coluna de totalizacao: picture do
titulo (cPictTit) quando o pais e Chile, senao a picture padrao
do campo E2_VALOR. Extraido de expressao IIf() (repetida 2x)
para respeitar o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param cPictTit Picture do titulo calculada em ReportPrint
@return uRet Character/Block, picture retornada por PesqPict
/*/
//-----------------------------------------------------------
Static Function F150PictVal(cPictTit)
	Local uRet

	If cPaisLoc == "CHI"
		uRet := cPictTit
	Else
		uRet := PesqPict("SE2","E2_VALOR")
	EndIf

Return uRet

//----------------------------------------------------------
/*/{Protheus.doc}F150OnPrint
Seletor generico usado nos callbacks ONPRINT do TRFunction: o
total geral quando esta imprimindo o total do relatorio, senao
o total parcial (mes/filial/etc). Extraido de expressao If()
(repetida 2x) para respeitar o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param lReport Indica se e a impressao do total geral do relatorio
@param xValA Valor quando lReport e verdadeiro
@param xValB Valor quando lReport e falso
@return uRet Numeric
/*/
//-----------------------------------------------------------
Static Function F150OnPrint(lReport, xValA, xValB)
	Local uRet
...
//----------------------------------------------------------
/*/{Protheus.doc}F150TotFil2
Seletor do callback ONPRINT do total por filial (oBreak2): ao
imprimir o total do relatorio, guarda o total da filial corrente
(nFilTot) em nAuxTotFil e zera nFilTot (pra nao contar 2x no
proximo grupo), senao retorna o total acumulado (nTotFil).
Extraido de expressao If() com efeito colateral condicional para
respeitar o padrao do projeto - os parametros por referencia (@)
preservam o efeito colateral original.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param lReport Indica se e a impressao do total geral do relatorio
@param nFilTot Total acumulado da filial corrente (por referencia)
@param nAuxTotFil Variavel auxiliar (por referencia)
@param nTotFil Total acumulado geral
@return nRet Numeric
/*/
//-----------------------------------------------------------
Static Function F150TotFil2(lReport, nFilTot, nAuxTotFil, nTotFil)
	Local nRet as Numeric
...
//----------------------------------------------------------
/*/{Protheus.doc}F150AddSM0
Adiciona a filial na lista de filiais da empresa atual (aSM0)
quando pertence ao grupo de empresa informado. Extraido de
expressao IIf() usada so pelo efeito colateral (AAdd) para
respeitar o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param aFilial Registro de filial do FwLoadSM0()
@param cEmpAnt Codigo da empresa atual
@param aSM0 Array de filiais (mutado por AAdd)
@return Nil
/*/
//-----------------------------------------------------------
Static Function F150AddSM0(aFilial, cEmpAnt, aSM0)
	If aFilial[SM0_GRPEMP] == cEmpAnt
		AAdd(aSM0, AClone(aFilial))
	EndIf

Return Nil

//----------------------------------------------------------
/*/{Protheus.doc}F150AddCampo
Concatena o nome do campo em cCampos quando o campo nao e do
tipo Memo. Extraido de expressao If() usada so pelo efeito
colateral (concatenacao) para respeitar o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param aCampo Estrutura do campo (DbStruct())
@param cCampos Lista de campos acumulada ate aqui
@return cCampos Character, lista de campos atualizada
/*/
//-----------------------------------------------------------
Static Function F150AddCampo(aCampo, cCampos)
	If aCampo[2] <> "M"
		cCampos += ", " + AllTrim(aCampo[1])
	EndIf

Return cCampos

//----------------------------------------------------------
/*/{Protheus.doc}F150NomeFor
Retorna o nome do fornecedor respeitando a LGPD: nome reduzido
quando o parametro pede nome reduzido, senao o nome completo.
Extraido de expressao IIf()/If() (repetida 3x) para respeitar o
padrao do projeto.
...
/*/
//-----------------------------------------------------------
Static Function F150NomeFor()
	Local cRet as Character

	If MV_PAR28 == 1
		cRet := GetLGPDValue("SA2","A2_NREDUZ")
	Else
		cRet := GetLGPDValue("SA2","A2_NOME")
	EndIf

Return cRet

//----------------------------------------------------------
/*/{Protheus.doc}F150Natureza
Retorna a natureza formatada (MascNat) quando preenchida, senao
vazio. Extraido de expressao If() para respeitar o padrao do
projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param xNatureza Natureza gravada em aDados[NATUREZA]
@return cRet Character
/*/
//-----------------------------------------------------------
Static Function F150Natureza(xNatureza)
	Local cRet as Character

	If !Empty(xNatureza)
		cRet := MascNat(xNatureza)
	Else
		cRet := ""
	EndIf
