...
//(1=Empresa, 2=Unidade de Neg๓cio e 3=Filial)
STATIC gstEMP 		:= 1
STATIC gstUNI 		:= 2
STATIC gstFIL 		:= 3
...
ฑฑณSintaxe e ณ U_FinR710(void)						  					  ณฑฑ
ฑฑรฤฤฤฤฤฤฤฤฤฤลฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤดฑฑ
ฑฑณParametrosณ 															  ณฑฑ
ฑฑรฤฤฤฤฤฤฤฤฤฤลฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤดฑฑ
...
//ณ mv_par11				// Banco Fornecedor por             ณ
...
If Empty(mv_par03)
	HELP (" ",1,'DTDEBITO',,STR0057,1,0,,,,,,{STR0058} ) 	//"Data de dbito n?o informada na parametriza?o do relatขrio."###"Por favor, informe a data de dbito nos parmetros do relatขrio (pergunte)."
	oReport:CancelPrint()
	Return
Endif
...
TRPosition():New(oSection2,"SE2",1,{|| F710ChaveSE2(cAliasSea) } )
...
oSection2:Cell("EF_BENEF"):SetBlock( { || cDvCta := Posicione( "SA6", 1, cFilSA6 + (cAliasSEA)->(EA_PORTADO+EA_AGEDEP+EA_NUMCON), "A6_DVCTA" ), ;
										cNumConta := RTrim( (cAliasSea)->EA_NUMCON ) + F710Sep(cDvCta) + cDvCta, ;
										Fr710Benef(cAliasSea, lCheque, lAbatimento) } )

oSection2:Cell("A2_CGC"    ):SetBlock( { || Transform(SA2->A2_CGC, F710PicCGC(SA2->A2_CGC)) } )
...
If SE2->(DbSeek(FwxFilial("SE2",(cAliasSea)->EA_FILORIG)+(cAliasSea)->(EA_PREFIXO+EA_NUM+EA_PARCELA+EA_TIPO+EA_FORNECE+EA_LOJA)))
	...
	If lFValAcess
		...
		nVA	:= FValAcess( SE2->E2_PREFIXO, SE2->E2_NUM, SE2->E2_PARCELA, SE2->E2_TIPO, SE2->E2_FORNECE, SE2->E2_LOJA, SE2->E2_NATUREZ, lTemBaixa, "" , "P", , , SE2->E2_MOEDA, SE2->E2_MOEDA , SE2->E2_TXMOEDA )
	Endif
	
	If ! lAbatimento
		If MV_PAR06==1
			dDataConv := (cAliasSea)->EA_DATABOR
		Else
			dDataConv := dDataBase
		EndIf
		
		If mv_par07 == 1
			nVlrPagar := Round(NoRound(xMoeda(SE2->E2_SALDO-SE2->E2_SDDECRE+SE2->E2_SDACRES-nAbat+nJuros+nVA,SE2->E2_MOEDA,MV_PAR04,dDataConv,MsDecimais(1)+1,SE2->E2_TXMOEDA ,SM2->M2_MOEDA2),MsDecimais(1)+1),MsDecimais(1))
	    Else
			nVlrPagar := Round(NoRound(xMoeda(SE2->E2_VALOR-SE2->E2_DECRESC+SE2->E2_ACRESC-nAbat+nJuros+nVA,SE2->E2_MOEDA,MV_PAR04,dDataConv,MsDecimais(1)+1,SE2->E2_TXMOEDA ,SM2->M2_MOEDA2),MsDecimais(1)+1),MsDecimais(1))
		EndIf
	Endif
...
If lFWCodFil
	cCodFilLogo := FWGETCODFILIAL
Else
	cCodFilLogo := SM0->M0_CODFIL
EndIf

cLogo := cStartPath + "LGRL" + SM0->M0_CODIGO + cCodFilLogo + ".BMP" 	// Empresa+Filial
...
oReport:PrintText(Pad(	OemToAnsi(STR0038) + RTrim(SA6->A6_AGENCIA) + F710Sep(SA6->A6_DVAGE) + RTrim(SA6->A6_DVAGE) + ;
						OemToAnsi(STR0040) + RTrim(SA6->A6_NUMCON)  + F710Sep(SA6->A6_DVCTA) + RTrim(SA6->A6_DVCTA),100))
...
Case	(cAliasSea)->EA_MODELO $ "CH/02"
	...
	oSection2:Cell("EF_NUM"    ):SetBlock({ || F710Cheque(lCheque) })
...
Static Function F710BcoFor(cCampo As Char, cAliasSea As Char) As Char
	...
	If cCampo$"A2_BANCO"
		cRet := F710ValBco(lBcoTit, SE2->E2_FORBCO, (cAliasSea)->A2_BANCO, (cAliasSea)->FIL_BANCO)
	Elseif cCampo$"A2_AGENCIA"
		cRet := F710ValBco(lBcoTit, SE2->E2_FORAGE, (cAliasSea)->A2_AGENCIA, (cAliasSea)->FIL_AGENCI)
	Elseif cCampo$"A2_DVAGE"
		cRet := F710ValBco(lBcoTit, SE2->E2_FAGEDV, (cAliasSea)->A2_DVAGE, (cAliasSea)->FIL_DVAGE)
	Elseif cCampo$"A2_NUMCON"
		cRet := F710ValBco(lBcoTit, SE2->E2_FORCTA, (cAliasSea)->A2_NUMCON, (cAliasSea)->FIL_CONTA)
	Elseif cCampo$"A2_DVCTA"
		cRet := F710ValBco(lBcoTit, SE2->E2_FCTADV, (cAliasSea)->A2_DVCTA, (cAliasSea)->FIL_DVCTA)
	Elseif cCampo$"A6_NREDUZ"
		cBanco := F710ValBco(lBcoTit, SE2->E2_FORBCO, (cAliasSea)->A2_BANCO, (cAliasSea)->FIL_BANCO)
		cAgenc := F710ValBco(lBcoTit, SE2->E2_FORAGE, (cAliasSea)->A2_AGENCIA, (cAliasSea)->FIL_AGENCI)
		cConta := F710ValBco(lBcoTit, SE2->E2_FORCTA, (cAliasSea)->A2_NUMCON, (cAliasSea)->FIL_CONTA)
...
Fun็ใo para uso temporแrio, validando se a pergunta 11 existe no dicionแrio.
...
//----------------------------------------------------------
/*/{Protheus.doc}F710ChaveSE2
Monta a chave de posicionamento do SE2 usada pelo TRPosition
da secao de titulos. Extraido de expressao If() para respeitar
o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param cAliasSea Alias da query posicionada (SEA)
@return cRet Character
/*/
//-----------------------------------------------------------
Static Function F710ChaveSE2(cAliasSea)
	Local cRet as Character

	If Empty((cAliasSea)->EA_FILORIG) .AND. !Empty(xFilial("SE2"))
		cRet := xFilial("SE2")+(cAliasSea)->EA_PREFIXO+(cAliasSea)->EA_NUM+(cAliasSea)->EA_PARCELA+(cAliasSea)->EA_TIPO+(cAliasSea)->EA_FORNECE+AllTrim((cAliasSea)->EA_LOJA)
	Else
		cRet := (cAliasSea)->EA_FILORIG+(cAliasSea)->EA_PREFIXO+(cAliasSea)->EA_NUM+(cAliasSea)->EA_PARCELA+(cAliasSea)->EA_TIPO+(cAliasSea)->EA_FORNECE+AllTrim((cAliasSea)->EA_LOJA)
	EndIf

Return cRet

//----------------------------------------------------------
/*/{Protheus.doc}F710Sep
Retorna o separador "-" usado entre numero e digito verificador
(conta/agencia). Extraido de expressao IIf() para respeitar o
padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param cVal Digito verificador a testar
@return cRet Character, "-" ou ""
/*/
//-----------------------------------------------------------
Static Function F710Sep(cVal)
	Local cRet as Character
...
//----------------------------------------------------------
/*/{Protheus.doc}F710PicCGC
Retorna a mascara de exibicao do CGC/CPF conforme o tamanho do
documento. Extraido de expressao IIf() para respeitar o padrao
do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param cCgc Documento (CGC/CPF)
@return cRet Character, mascara @R
/*/
//-----------------------------------------------------------
Static Function F710PicCGC(cCgc)
	Local cRet as Character

	If Len(Alltrim(cCgc)) > 11
		cRet := "@R 99999999/9999-99"
	Else
		cRet := "@R 999999999-99"
	EndIf
...
//----------------------------------------------------------
/*/{Protheus.doc}F710Cheque
Retorna o texto exibido na coluna EF_NUM quando o modelo do
bordero usa cheque. Extraido de expressao IIf() para respeitar
o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param lCheque Indica se o titulo tem cheque vinculado
@return cRet Character
/*/
//-----------------------------------------------------------
Static Function F710Cheque(lCheque)
	Local cRet as Character
...
//----------------------------------------------------------
/*/{Protheus.doc}F710ValBco
Resolve o dado bancario (banco/agencia/conta/dv) priorizando o
cadastro vinculado ao titulo a pagar (E2_FORBCO etc, quando
lBcoTit), senao o dado do fornecedor (SA2) e por ultimo o dado
da filial (FIL). Extraido de expressoes If() aninhadas para
respeitar o padrao do projeto - usada 8x em F710BcoFor.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param lBcoTit Indica se deve priorizar o dado vinculado ao titulo
@param cValTit Valor vindo do titulo a pagar (SE2)
@param cValSea Valor vindo do fornecedor (SEA/SA2)
@param cValFil Valor vindo da filial (FIL)
@return cRet Character
/*/
//-----------------------------------------------------------
Static Function F710ValBco(lBcoTit, cValTit, cValSea, cValFil)
	Local cRet as Character
