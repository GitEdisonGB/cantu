...
Static lFWCodFil := .t.
STATIC lIsRussia	:= F110Sel(cPaisLoc$"RUS",.T.,.F.) // CAZARINI - Flag to indicate if is Russia location
...
oSection1:SetLineCondition({|| F110Sel(	(cAliasSN3)->(N3_FILIAL+N3_CBASE+N3_ITEM+N3_TIPO) != cBemAnt,;
...
bVOrig := { ||lAtClDepr:= AtClssVer( (cAliasSN3)->N1_PATRIM) ,;
				...
				F110Sel(lAtClDepr .OR. (cAliasSN3)->N1_PATRIM $ " P",(cAliasSN3)->&("N3_VORIG"+cMoeda),(cAliasSN3)->&("N3_VORIG"+cMoeda)*(-1)) }
...
oBreak1:SetTotalText( { ||  F110Sel(nOrder == 1, STR0022+" : "+cContaBrk, F110Sel(nOrder == 2, STR0023+" : "+cCustoBrk, STR0024+" : "+DTOC(dAquisBrk)))} )
...
cTypes10 := F110Sel(lIsRussia,"*" + AtfNValMod({1}, "*"),"")
...
Static Function Atf110Tmp( cAliasTMP, cChave, cFiltro, cUserFilter, cTmpFil )
Local aArea		:= GetArea()
Local cCampos	:= ""
Local cQuery	:= ""
Local cFilDe	:= ""
...
Local nI		:= 0
Local lAtfCusPrv := AFXAtCsPrv()
Local cTypesNM	:= "" // CAZARINI - 10/04/2017 - If is Russia, add new valuations models - main and recoverable models
Local aTypesNM	:= {}
Local nTypesNM	:= 0
DEFAULT cChave		:= ""
DEFAULT cFiltro		:= ""
...
DEFAULT cTmpFil	:= ""
/* GESTAO - fim */
cTypesNM := F110Sel(lIsRussia,"|" + AtfNValMod({1,2}, "|"),"")
...
/*/
//-----------------------------------------------------------
Static Function F110Sel(lCond, xValA, xValB)
	Local uRet
