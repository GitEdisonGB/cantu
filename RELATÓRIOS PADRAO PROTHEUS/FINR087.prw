...
±±ºPrograma  ³FINR087   ºAutor  ³Jeniffer Larrosa    ºFecha ³  07/12/00   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Rotina de impressao do Recibo de Cobranca                   º±±
±±º          ³Antigo RCBR010.prx                                          º±±
...
±±ºSergio F.     ³07/01/02³012384³Ajuste no Lay-out do relatorio          º±±
±±ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/

User Function Finr087()
...
/*
 * Implementação de versionamento de recibos de cobrança (Argentina)
 */
If cPaisLoc $ "ARG"
		cPerg := "FIR087A"	
	Else
...
While !Eof() .And. ( xFilial("SEL")==SEL->EL_FILIAL ) .And. SEL->EL_RECIBO <= mv_par02 .And. SEL->EL_SERIE == cSerie;
			 .And. lContinua .AND. F087Sel(lVersao,(SEL->EL_VERSAO >= cVerDe .AND. SEL->EL_VERSAO <= cVerAte),.T.)
	IncRegua()
	cCliente  := EL_CLIORIG
	cLoja     := EL_LOJORIG
	IF lAbortPrint
	...
	Do while xFilial("SEL")==SEL->EL_FILIAL.And.cRECIBO==SEL->EL_RECIBO.And.cSerRec==SEL->EL_SERIE.and.!SEL->(eof());
		.AND. F087Sel(lVersao,(SEL->EL_VERSAO == cVersao),.T.)
		IncRegua()
		IF lAbortPrint
			@ 00,01 PSAY OemToAnsi(STR0005)  // "** CANCELADO PELO OPERADOR **"
			lContinua := .F.
	...
	If Len(aNfs) > 0
		...
		For nA:=1  to Len(aNfs)                                         //6        7         8
			...
			If cPaisLoc $ "PTG|ANG|MEX"
			  	@nLin+nP,028 PSAY F087Sel(AllTrim(aNfs[nA][8])$ mv_crneg+"|"+mvrecant,aNfs[nA][3]*-1,aNfs[nA][3]) PICTURE TM(aNfs[nA][3],16,MsDecimais(nTamValor)) //"@R 99,999,999,999"  //Valor do Doc.
				@nLin+nP,046 PSAY aNfs[nA][4]  //Simbolo da moeda
				@nLin+nP,052 PSAY aNfs[nA][5]  //Data vencto.
			  	@nLin+nP,062 PSAY F087Sel(AllTrim(aNfs[nA][8])$ mv_crneg+"|"+mvrecant,aNfs[nA][6]*-1,aNfs[nA][6]) PICTURE TM(aNfs[nA][6],21,MsDecimais(nTamValor)) //"@R 999,999,999.99"  //Valor na moeda 1
			Else
			  	@nLin+nP,021 PSAY F087Sel(AllTrim(aNfs[nA][8])$ mv_crneg+"|"+mvrecant,aNfs[nA][3]*-1,aNfs[nA][3]) PICTURE TM(aNfs[nA][3],21,MsDecimais(nTamValor)) //"@R 99,999,999,999"  //Valor do Doc.
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºFuncao    ³VERIMP    ºAutor  ³Marcos Simidu       ºFecha ³  12/07/98   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºFuncao    ³Cabec087  ºAutor  ³Bruno Sobieski      ºFecha ³  11/03/99   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Encabezado del Recibo.                                      º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ FINR087                                                    º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
@ 02,048 PSAY OemToAnsi(STR0030)+F087Sel(Empty(cSerRec),'    ',cSerRec+"-")+cRecibo + "/" + Str(nCntRcbo,2) + " Rev. " + cVersao //"Recibo Nro: "
@ 02,084 PSAY "|"
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºFuncao    ³Roda087   ºAutor  ³Bruno Sobieski      ºFecha ³  11/03/99   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºFuncao    ³xTasas    ºAutor  ³Bruno Sobieski      ºFecha ³  11/03/99   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Imprime la tasa de las Monedas en uso en el pie de pagina   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ FINR087                                                    º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
For nK:=1  To ContaMoeda()
	Aadd(aTasas,F087Sel(RecMoeda(dDtEmis,StrZero(nK,1))==0,1,RecMoeda(dDtEmis,StrZero(nK,1))))
Next
...
/*/
//-----------------------------------------------------------
Static Function F087Sel(lCond, xValA, xValB)
	Local uRet
