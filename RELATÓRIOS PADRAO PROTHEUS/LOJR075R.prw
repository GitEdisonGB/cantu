...
Static lTroco := F075Sel(	cPaisLoc <> "BRA",;
...
Static lLGPD 		:= F075Sel(ExistFunc("LjPDUse"),LjPDUse(),.F.)	//Verifica se a funcionalidade de Dados Protegidos está sendo utilizada no sistema.
...
aTotCx[06] += Round(xMoeda(( F075Sel(lVlrPisCof,TRB->ABTOPCC,0) + F075Sel( lCliRecIss, TRB->VALISS, 0 ) ),nMoeda,mv_par09,TRB->DTEMI,nDecs+1),nDecs)
		...
		While !Eof() .AND. cFilAtu == TRB->FILIAL .AND. uDatAtu == TRB->DTEMI
			...
			While !Eof() .AND. cFilAtu == TRB->FILIAL .AND. uDatAtu == TRB->DTEMI .AND. cCaiAtu == TRB->CAIXA
				...
				If Empty( TRB->LDEV )
					...
					//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
					oSection4:Cell("N_FISCAL"):SetValue( F075Sel( !Empty( TRB->N_FISCAL ),TRB->N_FISCAL,TRB->N_FISCPED ) )
					oSection4:Cell("SERIE"):SetValue( F075Sel( !Empty( TRB->SERIE ),TRB->SERIE,TRB->SERPED ) )
					...
					//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
					oSection4:Cell("RETENC"):SetValue( Round( xMoeda( ( F075Sel( lVlrPisCof, TRB->ABTOPCC, 0 ) + ;
					F075Sel( lCliRecIss, TRB->VALISS, 0 ) ), ;
...
/*/
//-----------------------------------------------------------
Static Function F075Sel(lCond, xValA, xValB)
	Local uRet
