...
±±³Programa  ³MATR860   ³ Autor ³Felipe Nunes Toledo    ³ Data ³ 23/06/06 ³±±
...
nTamB1DESC := F860Sel(nTamB1DESC > 50, nTamB1DESC / 2, nTamB1DESC)
...
//³                                                                        ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oReport:= TReport():New("MATR860",OemToAnsi(STR0001),"MTR860", {|oReport| ReportPrint(oReport)},OemToAnsi(STR0002)+OemToAnsi(STR0003)) //##"O objetivo deste relat¢rio ‚ exibir detalhadamente todas as movimenta-‡”es feitas para cada Ordem de Produ‡„o ,mostrando inclusive os custos."
...
oSection1 := TRSection():New(oReport,STR0043,{"SD3","SB1"},/*Ordem*/) //"Ordens de Produção"
...
oReport:SetTitle(STR0001+" - "+AllTrim(GetMv("MV_SIMB"+Ltrim(Str(mv_par03))))+ " / " + F860Sel(lCusRep .And. mv_par07==2,STR0051,STR0052) )
...
If mv_par06 == 1
	...
	oFunction := TRFunction():New(oSection1:Cell('D3_QUANT'),NIL,"SUM",oBreak,NIL,/*Picture*/,{|| F860Sel((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)<>"PR", oSection1:Cell('D3_QUANT'):GetValue(), 0) },.F.,.F.)
	oFunction := TRFunction():New(oSection1:Cell('Custo'   ),NIL,"SUM",oBreak,NIL,/*Picture*/,{|| F860Sel((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)<>"PR", oSection1:Cell('Custo'   ):GetValue(), 0) },.F.,.F.)
EndIf
...
EndIf	
oFunction := TRFunction():New(oSection1:Cell('D3_QUANT'),NIL,"SUM",oBreak2,STR0026,/*Picture*/,{|| F860Sel((cQryOP)->ISMOD == 'S', oSection1:Cell('D3_QUANT'):GetValue() , 0 ) },.F.,.F.) //"Qtd. Mao de Obra ---"
oFunction := TRFunction():New(oSection1:Cell('Custo'   ),NIL,"SUM",oBreak2,STR0027,/*Picture*/,{|| F860Sel((cQryOP)->ISMOD == 'S', oSection1:Cell('Custo'):GetValue()    , 0 ) },.F.,.F.) //"Custo Mao de Obra --"

//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³Totais Gerais                            ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oFunction := TRFunction():New(oSection1:Cell('D3_QUANT'),NIL,"SUM",NIL,STR0028,/*Picture*/,{|| F860Sel((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)=="RE" .And. SubStr(D3_CF,3,1) # "9", oSection1:Cell('D3_QUANT'):GetValue(), 0) },.F.,.T.) //"QTDE. TOTAL REQUISICOES ------->"
oFunction := TRFunction():New(oSection1:Cell('Custo'   ),NIL,"SUM",NIL,STR0029,PesqPict("SD3","D3_CUSTO1",17),{|| F860Sel((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)=="RE" .And. SubStr(D3_CF,3,1) # "9", oSection1:Cell('Custo'   ):GetValue(), 0) },.F.,.T.) //"CUSTO TOTAL REQUISICOES ------->"

oFunction := TRFunction():New(oSection1:Cell('D3_QUANT'),NIL,"SUM",NIL,STR0030,/*Picture*/,{|| F860Sel(SubStr(D3_CF,1,2)=="PR",oSection1:Cell('D3_QUANT'):GetValue(),0) },.F.,.T.) //"QTDE. TOTAL PRODUCAO ---------->"
oFunction := TRFunction():New(oSection1:Cell('Custo'   ),NIL,"SUM",NIL,STR0031,PesqPict("SD3","D3_CUSTO1",17),{|| F860Sel(SubStr(D3_CF,1,2)=="PR",oSection1:Cell('Custo'   ):GetValue(),0) },.F.,.T.) //"CUSTO TOTAL PRODUCAO ---------->"

oFunction := TRFunction():New(oSection1:Cell('D3_QUANT'),NIL,"SUM",NIL,STR0032,/*Picture*/,{|| F860Sel((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)=="DE" .And. SubStr(D3_CF,3,1) # "9", (- oSection1:Cell('D3_QUANT'):GetValue()), 0) },.F.,.T.) //"QTDE. TOTAL DEVOLUCOES  ------->"
oFunction := TRFunction():New(oSection1:Cell('Custo'   ),NIL,"SUM",NIL,STR0033,PesqPict("SD3","D3_CUSTO1",17),{|| F860Sel((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)=="DE" .And. SubStr(D3_CF,3,1) # "9", (- oSection1:Cell('Custo'   ):GetValue()), 0) },.F.,.T.) //"CUSTO TOTAL DEVOLUCOES  ------->"

oFunction := TRFunction():New(oSection1:Cell('D3_QUANT'),NIL,"SUM",NIL,STR0034,/*Picture*/,{|| F860Sel((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)=="RE" .And. SubStr(D3_CF,3,1)=="9", oSection1:Cell('D3_QUANT'):GetValue(), 0) },.F.,.T.) //"QTDE. TOTAL REQ PODER 3 ------->"
oFunction := TRFunction():New(oSection1:Cell('Custo'   ),NIL,"SUM",NIL,STR0035,PesqPict("SD3","D3_CUSTO1",17),{|| F860Sel((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)=="RE" .And. SubStr(D3_CF,3,1)=="9", oSection1:Cell('Custo'   ):GetValue(), 0) },.F.,.T.) //"CUSTO TOTAL REQ PODER 3 ------->"
                                                                        
oFunction := TRFunction():New(oSection1:Cell('D3_QUANT'),NIL,"SUM",NIL,STR0036,/*Picture*/,{|| F860Sel((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)=="DE" .And. SubStr(D3_CF,3,1)=="9", (- oSection1:Cell('D3_QUANT'):GetValue()), 0) },.F.,.T.) //"QTDE. TOTAL DEV PODER 3 ------->"
oFunction := TRFunction():New(oSection1:Cell('Custo'   ),NIL,"SUM",NIL,STR0037,PesqPict("SD3","D3_CUSTO1",17),{|| F860Sel((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)=="DE" .And. SubStr(D3_CF,3,1)=="9", (- oSection1:Cell('Custo'   ):GetValue()), 0) },.F.,.T.) //"CUSTO TOTAL DEV PODER 3 ------->"

oFunction := TRFunction():New(oSection1:Cell('D3_QUANT'),NIL,"SUM",NIL,STR0038,/*Picture*/,{|| F860Sel((cQryOP)->ISMOD == 'S' .And. SubStr((cQryOP)->D3_CF,1,2)=="RE" .And. SubStr(D3_CF,3,1) # "9", oSection1:Cell('D3_QUANT'):GetValue(), 0) },.F.,.T.) //"QTDE. TOTAL REQ. MAO DE OBRA -->"
oFunction := TRFunction():New(oSection1:Cell('Custo'   ),NIL,"SUM",NIL,STR0039,PesqPict("SD3","D3_CUSTO1",17),{|| F860Sel((cQryOP)->ISMOD == 'S' .And. SubStr((cQryOP)->D3_CF,1,2)=="RE" .And. SubStr(D3_CF,3,1) # "9", oSection1:Cell('Custo'   ):GetValue(), 0) },.F.,.T.) //"CUSTO TOTAL REQ. MAO DE OBRA -->"

oFunction := TRFunction():New(oSection1:Cell('D3_QUANT'),NIL,"SUM",NIL,STR0040,/*Picture*/,{|| F860Sel((cQryOP)->ISMOD == 'S' .And. SubStr((cQryOP)->D3_CF,1,2)=="DE" .And. SubStr(D3_CF,3,1) # "9", (- oSection1:Cell('D3_QUANT'):GetValue()), 0) },.F.,.T.) //"QTDE. TOTAL DEV. MAO DE OBRA -->"
oFunction := TRFunction():New(oSection1:Cell('Custo'   ),NIL,"SUM",NIL,STR0041,PesqPict("SD3","D3_CUSTO1",17),{|| F860Sel((cQryOP)->ISMOD == 'S' .And. SubStr((cQryOP)->D3_CF,1,2)=="DE" .And. SubStr(D3_CF,3,1) # "9", (- oSection1:Cell('Custo'   ):GetValue()), 0) },.F.,.T.) //"CUSTO TOTAL DEV. MAO DE OBRA -->"
...
While !oReport:Cancel() .And. !(cQryOP)->(Eof())
	...
	While (cQryOP)->D3_FILIAL+(cQryOP)->D3_OP == xFilial("SD3")+cOpAnt
		...
		If ((cQryOP)->ISMOD == 'S')
			oReport:Section(1):Cell('D3_QUANT'):SetValue( F860Sel(SubStr((cQryOP)->D3_CF,1,2)=="DE",( -R860ToDec((cQryOP)->D3_QUANT )),R860ToDec((cQryOP)->D3_QUANT)) )
		Else
			oReport:Section(1):Cell('D3_QUANT'):SetValue( F860Sel(SubStr((cQryOP)->D3_CF,1,2)=="DE",( -(cQryOP)->D3_QUANT ),(cQryOP)->D3_QUANT) )
		EndIf
		...
		oReport:Section(1):Cell('Custo'   ):SetValue( F860Sel(SubStr((cQryOP)->D3_CF,1,2)=="DE",( -&(cCampoCus)),&(cCampoCus)) )
		If !(lCusRep .And. mv_par07==2)
			oReport:Section(1):Cell('B1_CUSTD'):SetValue( F860Sel(SubStr((cQryOP)->D3_CF,1,2)=="PR", RetFldProd((cQryOP)->B1_COD,"B1_CUSTD",cQryOP) ,0) )
		EndIf	
		If !(lCusRep .And. mv_par07==2)
			oReport:Section(1):Cell('TotalSTD'):SetValue( F860Sel(SubStr((cQryOP)->D3_CF,1,2)=="PR",( oSection1:Cell('B1_CUSTD'):GetValue() * oSection1:Cell('D3_QUANT'):GetValue() ) , 0)  )
		EndIf	
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Local bBloco := { |nV,nX| Trim(nV)+F860Sel(Valtype(nX)='C',"",Str(nX,1)) }
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
nTipo  := F860Sel(aReturn[4]==1,15,18)
...
Else
	Titulo  += " - "+AllTrim(GetMv("MV_SIMB"+Ltrim(Str(mv_par03)))) + " / " + F860Sel(lCusRep .And. mv_par07==2,STR0051,STR0052)
EndIf

//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Monta os Cabecalhos conforme Tamanho Produto                 ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
cabec1 := F860Sel(lNewTamProd,STR0044,STR0006)	//"CENTRO               ORDEM DE    MOV CODIGO DO       DESCRICAO              QUANTIDADE    UM         CUSTO       C U S T O  NUMERO       DATA DE"
cabec2 := F860Sel(lNewTamProd,STR0045,STR0007)	//"CUSTO                PRODUCAO        PRODUTO                                                      UNITARIO       T O T A L  DOCUMENTO    EMISSAO"
...
While lContinua .And. !Eof()
	...
	While !Eof() .And. D3_FILIAL+D3_OP = xFilial()+cOpAnt
		...
		If !IsProdMod(SD3->D3_COD)
			nTotQuant += F860Sel( SubStr(D3_CF,1,2) == "RE", D3_QUANT, 0 )
			nTotQuant += F860Sel( SubStr(D3_CF,1,2) == "DE", ( -D3_QUANT ), 0 )
			If SubStr(D3_CF,3,1) # "9"
				nTotCusto += F860Sel( SubStr(D3_CF,1,2) == "RE", nCusto, 0 )
				nTotCusto += F860Sel( SubStr(D3_CF,1,2) == "DE", ( -nCusto ), 0 )
			EndIf
		Else
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
			//³ Totaliza‡„o separada para a m„o-de-obra                  ³
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			nTotQuantMod += F860Sel( SubStr(D3_CF,1,2) == "RE", D3_QUANT, 0 )
			nTotQuantMod += F860Sel( SubStr(D3_CF,1,2) == "DE", ( -D3_QUANT ), 0 )
			If SubStr(D3_CF,3,1) # "9"
				nTotCustoMod += F860Sel( SubStr(D3_CF,1,2) == "RE", nCusto, 0 )
				nTotCustoMod += F860Sel( SubStr(D3_CF,1,2) == "DE", ( -nCusto ), 0 )
			EndIf
		EndIf
		
		nQtdeProd += F860Sel( SubStr(D3_CF,1,2) == "PR", D3_QUANT , 0 )
		nQtdeProd += F860Sel( SubStr(D3_CF,1,2) == "ER", -D3_QUANT , 0 )
...
/*/
//-----------------------------------------------------------
Static Function F860Sel(lCond, xValA, xValB)
	Local uRet
