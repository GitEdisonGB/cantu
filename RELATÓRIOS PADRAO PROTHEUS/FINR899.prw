...
*/
User Function Finr899()        // incluido pelo assistente de conversao do AP5 IDE em 09/09/99
	Local oReport
...
Static Function ReportPrint( oReport )
	...
	cVerDe   := MV_PAR06
	cVerAte  := MV_PAR07
	dDtIni   := F899Sel(cPaisLoc == "ARG", MV_PAR08, MV_PAR06)
	dDtFim   := F899Sel(cPaisLoc == "ARG", MV_PAR09, MV_PAR07)
	nMoeInf  := F899Sel(cPaisLoc == "ARG", MV_PAR10, MV_PAR08)
	cSerDe   := F899Sel(cPaisLoc == "ARG", MV_PAR11, MV_PAR09)
	cSerAte  := F899Sel(cPaisLoc == "ARG", MV_PAR12, MV_PAR10)
	nOutMoe  := F899Sel(cPaisLoc == "ARG", MV_PAR13, MV_PAR11)
	...
	oReport:SetTitle( oReport:Title() +;
							F899Sel(MV_PAR01==1,STR0010,F899Sel(MV_PAR01==2,STR0011,F899Sel(MV_PAR01==3,STR0012,STR0036))) +;
	...
	While  (cAliasTmp)->(!Eof())
		...
		If MV_PAR01==3
			lPrim	:=	F899Sel(cCLIENTE == (cAliasTmp)->EL_CLIORIG +"-"+ (cAliasTmp)->EL_LOJORIG,.F.,.T.)
		EndIf
		...
		While cOrdAnt == &cOrdem .And. (cAliasTmp)->(!Eof())
			...
			If ( Substr((cAliasTmp)->EL_TIPODOC,1,2) $ "RG|RI|RB|RS|RM")
				nValRet  := nValRet  +  Round(F899Sel(nMoeInf==1,(cAliasTmp)->EL_VLMOED1,xMoeda((cAliasTmp)->EL_VALOR,Max(Val((cAliasTmp)->EL_MOEDA),1),nMoeInf,(cAliasTmp)->EL_DTDIGIT,nDecs+1,(cAliasTmp)->EL_VLMOED1/(cAliasTmp)->EL_VALOR)),nDecs)
			Elseif	(Substr((cAliasTmp)->EL_TIPODOC,1,2) == "TB")
				If !((cAliasTmp)->EL_TIPO $ MV_CRNEG+"|"+MVRECANT)  //Tratamento para NCCs e RAs
					nValBaj  := nValBaj  +  Round(F899Sel(nMoeInf==1,(cAliasTmp)->EL_VLMOED1,xMoeda((cAliasTmp)->EL_VALOR,Max(Val((cAliasTmp)->EL_MOEDA),1),nMoeInf,(cAliasTmp)->EL_DTDIGIT,nDecs+1,(cAliasTmp)->EL_VLMOED1/(cAliasTmp)->EL_VALOR)),nDecs)
				...
				Else
					nValPaNc := nValPaNc +  Round(F899Sel(nMoeInf==1,(cAliasTmp)->EL_VLMOED1,xMoeda((cAliasTmp)->EL_VALOR,Max(Val((cAliasTmp)->EL_MOEDA),1),nMoeInf,(cAliasTmp)->EL_DTDIGIT,nDecs+1,(cAliasTmp)->EL_VLMOED1/(cAliasTmp)->EL_VALOR)),nDecs)
					nValDesc := nValDesc -  Round(xMoeda((cAliasTmp)->EL_DESCONT,Max(Val((cAliasTmp)->EL_MOEDA),1),nMoeInf,(cAliasTmp)->EL_DTDIGIT,nDecs+1,(cAliasTmp)->EL_VLMOED1/(cAliasTmp)->EL_VALOR),nDecs)
				EndIf
			Elseif	(Substr((cAliasTmp)->EL_TIPODOC,1,2) == "TF")
				nValDep  := nValDep  +  Round(F899Sel(nMoeInf==1,(cAliasTmp)->EL_VLMOED1,xMoeda((cAliasTmp)->EL_VALOR,Max(Val((cAliasTmp)->EL_MOEDA),1),nMoeInf,(cAliasTmp)->EL_DTDIGIT,nDecs+1,(cAliasTmp)->EL_VLMOED1/(cAliasTmp)->EL_VALOR)),nDecs)
			Elseif	(Substr((cAliasTmp)->EL_TIPODOC,1,2) == "TJ") 
				nCC      := nCC      +  Round(F899Sel(nMoeInf==1,(cAliasTmp)->EL_VLMOED1,xMoeda((cAliasTmp)->EL_VALOR,Max(Val((cAliasTmp)->EL_MOEDA),1),nMoeInf,(cAliasTmp)->EL_DTDIGIT,nDecs+1,(cAliasTmp)->EL_VLMOED1/(cAliasTmp)->EL_VALOR)),nDecs)
			Elseif (Substr((cAliasTmp)->EL_TIPODOC,1,2) == "CC") 
				nCC      := nCC      +  Round(F899Sel(nMoeInf==1,(cAliasTmp)->EL_VLMOED1,xMoeda((cAliasTmp)->EL_VALOR,Max(Val((cAliasTmp)->EL_MOEDA),1),nMoeInf,(cAliasTmp)->EL_DTDIGIT,nDecs+1,(cAliasTmp)->EL_VLMOED1/(cAliasTmp)->EL_VALOR)),nDecs)	
			Elseif (Substr((cAliasTmp)->EL_TIPODOC,1,2) == "CD")	
				nCD      := nCD      +  Round(F899Sel(nMoeInf==1,(cAliasTmp)->EL_VLMOED1,xMoeda((cAliasTmp)->EL_VALOR,Max(Val((cAliasTmp)->EL_MOEDA),1),nMoeInf,(cAliasTmp)->EL_DTDIGIT,nDecs+1,(cAliasTmp)->EL_VLMOED1/(cAliasTmp)->EL_VALOR)),nDecs)
			Elseif	(Substr((cAliasTmp)->EL_TIPODOC,1,2) $ "CH|EF")
				nValPr   := nValPr   +  Round( F899Sel( nMoeInf==1, (cAliasTmp)->EL_VLMOED1, xMoeda( (cAliasTmp)->EL_VALOR, Max(Val((cAliasTmp)->EL_MOEDA),1),nMoeInf,(cAliasTmp)->EL_DTDIGIT,nDecs+1,(cAliasTmp)->EL_VLMOED1/(cAliasTmp)->EL_VALOR)),nDecs)
				If Substr((cAliasTmp)->EL_TIPODOC,1,2) == "CH"
					If (cAliasTmp)->EL_EMISSAO < (cAliasTmp)->EL_DTVCTO
						nChead:= nChead + Round(F899Sel(nMoeInf==1,(cAliasTmp)->EL_VLMOED1,xMoeda((cAliasTmp)->EL_VALOR,Max(Val((cAliasTmp)->EL_MOEDA),1),nMoeInf,(cAliasTmp)->EL_DTDIGIT,nDecs+1,(cAliasTmp)->EL_VLMOED1/(cAliasTmp)->EL_VALOR)),nDecs)
					Else
						nCheqd:= nCheqd + Round(F899Sel(nMoeInf==1,(cAliasTmp)->EL_VLMOED1,xMoeda((cAliasTmp)->EL_VALOR,Max(Val((cAliasTmp)->EL_MOEDA),1),nMoeInf,(cAliasTmp)->EL_DTDIGIT,nDecs+1,(cAliasTmp)->EL_VLMOED1/(cAliasTmp)->EL_VALOR)),nDecs)
					EndIf
				Else
					nDin:= nDin + Round(F899Sel(nMoeInf==1,(cAliasTmp)->EL_VLMOED1,xMoeda((cAliasTmp)->EL_VALOR,Max(Val((cAliasTmp)->EL_MOEDA),1),nMoeInf,(cAliasTmp)->EL_DTDIGIT,nDecs+1,(cAliasTmp)->EL_VLMOED1/(cAliasTmp)->EL_VALOR)),nDecs)
				EndIf
			Elseif	(Substr((cAliasTmp)->EL_TIPODOC,1,2) == "RA")
				nValRa   := nValRa   +  Round(F899Sel(nMoeInf==1,(cAliasTmp)->EL_VLMOED1,xMoeda((cAliasTmp)->EL_VALOR,Max(Val((cAliasTmp)->EL_MOEDA),1),nMoeInf,(cAliasTmp)->EL_DTDIGIT,nDecs+1,(cAliasTmp)->EL_VLMOED1/(cAliasTmp)->EL_VALOR)),nDecs)
		...
		If lFR899Rec //Solo aplica para Mercado Internacional
			...
			nRecSEL := F899Sel(nTmpSEL > 0, nTmpSEL, nRecSEL)
		EndIf
		...
		If MV_PAR01 == 1 .Or. MV_PAR01 == 4           
			...
			oSection2:Cell("EL_RECIBO"):SetBlock({||F899Sel(Empty(cSerie),"    ",cSerie+"-")+cRecibo})	///Tipo
			If cPaisLoc $ "ARG" .And. lVersao
	...
	If nOrdenes > 0
		...
		oReport:PrintText(F899Sel(cPaisLoc<>"CHI",OemToAnsi(STR0038),OemToAnsi(STR0041)) + TransForm(nTotChAde,tm(nTotChAde,15,nDecs)))  //"TOTAL CHEQ ADELANTADO ----> "
...
/*/
//-----------------------------------------------------------
Static Function F899Sel(lCond, xValA, xValB)
	Local uRet
