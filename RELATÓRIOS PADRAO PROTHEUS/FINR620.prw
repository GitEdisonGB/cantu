...
//TReport():New(ExpC1 : Nome do relatorio,ExpC2 : Titulo,ExpC3 : Pergunte,ExpB4 : Bloco de codigo que sera executado na confirmacao da impressao,ExpC5 : Descricao)
	...
	oReport := TReport():New(cReport, STR0021, cPerg, {|oReport| ReportPrint(oReport)}, cDescri)
...
/* GESTAO - inicio */
	TRCell():New(oSection2, "FILORIG"		, "SE5" , SX3->(RetTitle("E5_FILORIG"))	, PesqPict("SE5","E5_FILORIG")	, TamSX3("E5_FILORIG")[1]+2	,/*lPixel*/,{|| F620FilOrig()}/*CodeBlock*/)
/* GESTAO - fim */
...
Static Function ReportPrint(oReport)
	...
	If mv_par09 > 1
		nCasas := GetMv("MV_CENT"+AllTrim(STR(mv_par09,2)))
	Else
		nCasas := GetMv("MV_CENT")
	EndIf
	...
	*/
	...
	//nao há necessidade de ser processado por filiais            
		...
		If SM0->(Reccount()) == 1 .or. Empty(xFilial("SE5"))
			mv_par12 := 2
		EndIf
	...
	If nOrdem == 2
		...
		/* GESTAO - fim*/			 
			...
			If lF620Qry
				cQueryAdd := ExecBlock("F620QRY", .F., .F.,{"",{aSaldo[nA,2], aSaldo[nA,3], aSaldo[nA,4]}})
	...
	If mv_par09 == 0
		mv_par09 := 1
	Endif
	cMoeda := AllTrim(Str(mv_par09,2,0))
	...
	cTitulo  += " " + GetMv("MV_MOEDA"+AllTrim(Str(nMoeda,2))) + STR0028 + cAnaSint + STR0028 //" - "###"Analitico"###"Sintetico"###" - "
   ...
   /* GESTAO - fim */
	...
	If nOrdem == 2
		nQtdeBancos := Len(aSaldo)
	Else
		nQtdeBancos := 1
	EndIf
	
	For nBancos := 1 To nQtdeBancos
		...
		If (Empty(cFilterUser) .And. nOrdem == 2) .Or. nOrdem <> 2
			If mv_par15 == 1 .And. lImpSaldo
				...
				If nOrdem == 2 // Ordem de banco
					...
					If nAscan > 0
						nSaldoAnt := Round(xMoeda(aSaldo[nAscan][6],nMoedaBco,mv_par09,F620DtSE5(),nCasas+1,nTxMoedBc),nCasas)
					Else
						nSaldoAnt := 0
					Endif
					lFirst := .T.
				Else
					...
					If lFirst
						...
						For nA := 1 To Len(aSaldo)
							nSaldoAnt += Round(xMoeda(aSaldo[nA][6],MoedaBco(aSaldo[nA][2],aSaldo[nA][3],aSaldo[nA][4]),mv_par09,F620DtSE5(),nCasas+1,nTxMoedBc),nCasas)
						Next
						lFirst := .F.
					Else
		...
		While (cAliasQry)->(!Eof())
			...
			If EVAL(bWhile) .And. &cCondicao .and. lContinua
				...
				While (cAliasQry)->(!EOF())
					...
					If (SE5->E5_TIPODOC == "SG") .And. (!SE5->E5_MOEDA $ "R$"+F620Cifrao()) //Sangria com moeda <> R$
						(cAliasQry)->(dbSkip())
						Loop
					EndIf
					...
					If mv_par11 == 1	//Analitico
						...
						oSection2:Cell("DATA"):SetBlock( { || F620DataCol(nOrdem) } )

						cDoc 	:= SUBSTR(Alltrim(SE5->E5_DOCUMEN),1,15)
						cCheque	:= Alltrim(SE5->E5_NUMCHEQ)

						If Empty(cDoc)
							If !empty(SE5->E5_PREFIXO)
								cSepPrefixo := "-"
							Else
								cSepPrefixo := ""
							EndIf
							
							If !empty(SE5->E5_PARCELA)
								cSufParcela := "-"+SE5->E5_PARCELA
							Else
								cSufParcela := ""
							EndIf
							
							cDoc := Alltrim(SE5->E5_PREFIXO)+cSepPrefixo+Alltrim(SE5->E5_NUMERO )+cSufParcela
						Endif
					...
					If mv_par11 == 1
						...
						Else					// Busca historico do titulo
							If SE5->E5_RECPAG == "R"
								...
								DbSetOrder(1)
								dbSeek( cChaveSe5 )
								oSection2:Cell("HISTORICO"):SetBlock( { || F620Hist(SE1->E1_HIST, cHistor) } )
							Else
								cHistor		:= SE5->E5_HISTOR
								...
								EndIf
								oSection2:Cell("HISTORICO"):SetBlock( { || F620Hist(SE2->E2_HIST, cHistor) } )
							Endif
						Endif
	...
	While (cAliasTmp)->(!Eof())
		If (cAliasTmp)->A6_FLUXCAI $ "S " .Or.  nOrdena == 2
			...
			If cPaisLoc=="BRA"
				nMoedaLim := 1
			Else
				nMoedaLim := Max((cAliasTmp)->A6_MOEDA,1)
			EndIf
			
			Aadd(aRet,{.F.,cTrbBanco,cTrbAgencia,cTrbConta,cTrbNome,nTrbSaldo,nMoeda, xMoeda((cAliasTmp)->A6_LIMCRED,nMoedaLim,mv_par09),cTrbFilial})
...
//----------------------------------------------------------
/*/{Protheus.doc}F620FilOrig
Retorna a filial de origem do movimento (E5_FILORIG), ou a
filial do proprio registro quando E5_FILORIG estiver vazio.
Extraido de expressao If() para respeitar o padrao do projeto.
...
//----------------------------------------------------------
/*/{Protheus.doc}F620While
Condicao do bWhile usado na leitura sequencial do SE5 quando
nao processa por filiais (mv_par12<>1). Extraido de expressao
If() para respeitar o padrao do projeto.
...
/*/
//-----------------------------------------------------------
Static Function F620While()
	Local lRet as Logical

	If mv_par12 == 1
		lRet := .T.
	Else
		lRet := SE5->E5_FILIAL == xFilial("SE5")
	EndIf

Return lRet

//----------------------------------------------------------
/*/{Protheus.doc}F620Cifrao
Retorna o sufixo de moeda usado na tabela 14 e no filtro de
sangria quando o pais nao e Brasil. Extraido de expressao IIf()
...
//----------------------------------------------------------
/*/{Protheus.doc}F620DtSE5
Retorna a data usada na conversao de moeda do saldo anterior:
data base quando o SE5 de referencia esta no EOF, senao a data
de disponibilidade do registro corrente. Extraido de expressao
...
//----------------------------------------------------------
/*/{Protheus.doc}F620Hist
Retorna o historico da coluna HISTORICO: o historico do titulo
(SE1/SE2) quando preenchido, senao o historico do movimento
bancario (SE5). Extraido de expressao If() para respeitar o
padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param cCampo Historico do titulo (E1_HIST/E2_HIST)
@param cHistor Historico do movimento bancario (SE5)
@return cRet Character
/*/
//-----------------------------------------------------------
Static Function F620Hist(cCampo, cHistor)
	Local cRet as Character
...
//----------------------------------------------------------
/*/{Protheus.doc}F620DataCol
Retorna a data exibida na coluna DATA: E5_DTDISPO, exceto na
ordem 5 (Data de Digitacao) onde exibe E5_DATA. Extraido de
expressao If() para respeitar o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param nOrdem Ordem de impressao selecionada (oSection1:GetOrder())
@return dRet Date
/*/
//-----------------------------------------------------------
Static Function F620DataCol(nOrdem)
	Local dRet as Date
