...
LOCAL nTaxa       := 1
LOCAL nMoeda      := 1
LOCAL nTpImp	  := 0	// Tipo de Impressao
...
nTpImp := F080Sel(ValType(oReport:nDevice)!=Nil, oReport:nDevice, 0)
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
cALiasSF1 := cAliasSD1
cALiasSB1 := F080Sel(!lEasy, cAliasSD1, "SB1")
cALiasSA1 := cAliasSD1
cALiasSA2 := cAliasSD1
lQuery :=.T.
...
cSelect := "%"
 	If nOrdem == 1
    	cSelect += "SD1.D1_FILIAL,SD1.D1_DOC,SD1.D1_SERIE," + F080Sel(SerieNfId("SD1",3,"D1_SERIE")<>"D1_SERIE", "SD1.D1_SDOC,", "")
    	cSelect += "SD1.D1_FORNECE,SD1.D1_LOJA,SD1.D1_COD,SD1.D1_ITEM,"
    ElseIf nOrdem == 2 
		cSelect += "SD1.D1_FILIAL,SD1.D1_COD,SD1.D1_DOC,SD1.D1_SERIE," + F080Sel(SerieNfId("SD1",3,"D1_SERIE")<>"D1_SERIE", "SD1.D1_SDOC,", "")
		cSelect += "SD1.D1_FORNECE,SD1.D1_LOJA,"
	ElseIf nOrdem == 3 
		cSelect += "SD1.D1_FILIAL,SD1.D1_DTDIGIT,SD1.D1_DOC,SD1.D1_SERIE," + F080Sel(SerieNfId("SD1",3,"D1_SERIE")<>"D1_SERIE", "SD1.D1_SDOC,", "")
		cSelect += "SD1.D1_FORNECE,SD1.D1_LOJA,"
    ElseIf nOrdem == 4 
    	cSelect += "SD1.D1_FILIAL,SD1.D1_EMISSAO,SD1.D1_DOC,SD1.D1_SERIE," + F080Sel(SerieNfId("SD1",3,"D1_SERIE")<>"D1_SERIE", "SD1.D1_SDOC,", "")
    	cSelect += "SD1.D1_FORNECE,SD1.D1_LOJA,"
    ElseIf nOrdem == 5 
    	cSelect += "SD1.D1_FILIAL,SD1.D1_FORNECE,SD1.D1_LOJA,SD1.D1_DOC,SD1.D1_SERIE," + F080Sel(SerieNfId("SD1",3,"D1_SERIE")<>"D1_SERIE", "SD1.D1_SDOC,", "") + "SD1.D1_ITEM,"
    Endif    
cSelect += "%"
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
...
If !lEasy        
	cSelect2 := "%"
		 cSelect2 += F080Sel(SerieNfId("SF1",3,"F1_SERIE")<>"F1_SERIE", "F1_SDOC,", "")
    ...
    If !Empty(cFilUsrSD1)
	cSelect4 := "D1_FILIAL,D1_DOC,D1_SERIE," + F080Sel(SerieNfId("SD1",3,"D1_SERIE")<>"D1_SERIE", "D1_SDOC,", "")
...
While !oReport:Cancel() .And. (cAliasSD1)->(!Eof()) .And. (cAliasSD1)->D1_FILIAL == xFilial("SD1") 
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	If mv_par19 == 2
		if F080Sel((cAliasSF1)->F1_MOEDA == 0, 1, (cAliasSF1)->F1_MOEDA) != mv_par18
			lFiltro := .F.
		Endif
	Endif
	...
	If lFiltro
		...
		If lDescLine == .T.
			...
			Elseif nOrdem == 3 .Or. nOrdem == 4
				...
				dDataAnt := F080Sel(nOrdem == 3, (cAliasSD1)->D1_DTDIGIT, (cAliasSD1)->D1_EMISSAO)
			Elseif nOrdem == 5
				If mv_par05 == 1
		...
		If nOrdem == 1 .Or. nOrdem == 3 .Or. nOrdem == 4 .Or. nOrdem ==5
				...
				If cPaisLoc <> "BRA"
 				...
 				Else
					...
					If (cAliasSF4)->F4_AGREG != "N"
						nValMerc  := nValMerc + (cAliasSD1)->D1_TOTAL + F080Sel(!(cAliasSF4)->F4_INCSOL$"A|N", (cAliasSD1)->D1_ICMSRET, 0)
						nTotGeral := nTotGeral + (cAliasSD1)->D1_TOTAL - (cAliasSD1)->D1_VALDESC + F080Sel(!(cAliasSF4)->F4_INCSOL$"A|N", (cAliasSD1)->D1_ICMSRET, 0)
			            If (cAliasSF4)->F4_AGREG = "I"
				           nValMerc  += (cAliasSD1)->D1_VALICM
				           nTotGeral += (cAliasSD1)->D1_VALICM
                        Endif 
					Else 
						nTotGeral := nTotGeral - (cAliasSD1)->D1_VALDESC + F080Sel((cAliasSF4)->F4_INCSOL!="A", (cAliasSD1)->D1_ICMSRET, 0)
					Endif
			...
			ElseIf mv_par05 == 2
				...
				Else
					...
					If (cAliasSF4)->F4_AGREG != "N"
						nValMerc := nValMerc + (cAliasSD1)->D1_TOTAL - (cAliasSD1)->D1_VALDESC + F080Sel((cAliasSF4)->F4_INCSOL!="A", (cAliasSD1)->D1_ICMSRET, 0)
			            If (cAliasSF4)->F4_AGREG = "I"
				            nValMerc += (cAliasSD1)->D1_VALICM
                        Endif 
					Else	
						nValMerc := nValMerc - (cAliasSD1)->D1_VALDESC + F080Sel((cAliasSF4)->F4_INCSOL!="A", (cAliasSD1)->D1_ICMSRET, 0)
					Endif
					
					If (cAliasSF4)->(dbSeek(xFilial("SF4") + (cAliasSD1)->D1_TES))
						nValMerc += F080Sel((cAliasSF1)->F1_TIPO != "P" .And. (cAliasSF4)->F4_IPI != "R", (cAliasSD1)->D1_VALIPI, 0)
					Else
						nValMerc += F080Sel((cAliasSF1)->F1_TIPO != "P", (cAliasSD1)->D1_VALIPI, 0)
					EndIf
				EndIf
			Endif
		Else
			...
			EndIf
			   If !lPlanTab .AND. nOrdem == 2			
					oSection2:Cell("nVunit"):SetValue(xmoeda((cAliasSD1)->D1_VUNIT,nMoeda,mv_par18,dDtDig,nDecs+1,nTaxa))
					oSection2:Cell("nVtotal"):SetValue(xmoeda((cAliasSD1)->D1_TOTAL - (cAliasSD1)->D1_VALDESC + F080Sel(!(cAliasSF4)->F4_INCSOL$"A|N", (cAliasSD1)->D1_ICMSRET, 0),nMoeda,mv_par18,dDtDig,nDecs+1,nTaxa))			
					If (cAliasSF4)->F4_AGREG != "N"
						nTotProd  += xmoeda((cAliasSD1)->D1_TOTAL - (cAliasSD1)->D1_VALDESC + F080Sel(!(cAliasSF4)->F4_INCSOL$"A|N", (cAliasSD1)->D1_ICMSRET, 0),nMoeda,mv_par18,dDtDig,nDecs+1,nTaxa)
						nTotQuant += (cAliasSD1)->D1_QUANT
						nTotData  += xmoeda((cAliasSD1)->D1_TOTAL - (cAliasSD1)->D1_VALDESC + F080Sel(!(cAliasSF4)->F4_INCSOL$"A|N", (cAliasSD1)->D1_ICMSRET, 0),nMoeda,mv_par18,,nDecs+1,nTaxa)
						nTotForn  += xmoeda((cAliasSD1)->D1_TOTAL - (cAliasSD1)->D1_VALDESC + F080Sel(!(cAliasSF4)->F4_INCSOL$"A|N", (cAliasSD1)->D1_ICMSRET, 0),nMoeda,mv_par18,,nDecs+1,nTaxa)
					    If (cAliasSF4)->F4_AGREG = "I"
		                ...
		                Endif 
					Else
						nTotProd  -= xmoeda((cAliasSD1)->D1_VALDESC + F080Sel(!(cAliasSF4)->F4_INCSOL$"A|N", (cAliasSD1)->D1_ICMSRET, 0),nMoeda,mv_par18,dDtDig,nDecs+1,nTaxa)
						nTotData  -= xmoeda((cAliasSD1)->D1_VALDESC + F080Sel(!(cAliasSF4)->F4_INCSOL$"A|N", (cAliasSD1)->D1_ICMSRET, 0),nMoeda,mv_par18,,nDecs+1,nTaxa)				
						nTotForn  -= xmoeda((cAliasSD1)->D1_VALDESC + F080Sel(!(cAliasSF4)->F4_INCSOL$"A|N", (cAliasSD1)->D1_ICMSRET, 0),nMoeda,mv_par18,,nDecs+1,nTaxa)				
					Endif
				EndIf
		...
		If ((cAliasSD1)->D1_FILIAL + (cAliasSD1)->D1_DOC + (cAliasSD1)->D1_SERIE + (cAliasSD1)->D1_FORNECE + (cAliasSD1)->D1_LOJA <> cDocAnt) .Or. ( lPlanTab .And. nCount == 1 .And. MV_PAR05 == 1)
			...
			If (nValIcm + nValMerc + nValIpi + nValImpInc + nValImpNoInc) > 0
				...
				ElseIf mv_par05 == 2
						...
						/* Neste trecho (lista itens = Não)
						 Só exibirá os itens abaixo se a impressão da planilha tem ordem = 2 (Por produto)*/
						If lPlanTab .AND. nOrdem != 2 .AND. cPaisLoc == 'BRA'
	...
	Elseif nOrdem == 3 .Or. nOrdem == 4 
		...
		If dDataAnt != F080Sel(nOrdem == 3, (cAliasSD1)->D1_DTDIGIT, (cAliasSD1)->D1_EMISSAO) .And. lPrintLine // nTotData > 0 
			lDescLine := .T.
			oReport:SkipLine()
			nLin := oReport:Row()
			oReport:PrintText(STR0046,nLin)	//"TOTAL NA DATA : "
...
If lImp	
	...
	ElseIf (nOrdem == 3 .Or. nOrdem == 4 .Or. nOrdem == 5) .And. MV_PAR05 == 1
		...
		nLin := oReport:Row()
		oReport:PrintText(STR0042,nLin)	
		oReport:PrintText(TransForm(nTotGeral,PesqPict("SF1","F1_VALMERC",17)),nLin,oSection2:Cell('nVtotal'):ColPos()-F080Sel(nOrdem==5, 30, 80))				
		oReport:SkipLine()
	Endif	
