...
Static Function ReportPrint(oReport)
	...
	Local dDtConvUsa := dDataBase
	Local dDtRecMoeda := dDataBase
	Local nMoedaRecMoeda := 0
	Local cSepDoc     := ""
	...
	If mv_par06 > 1
		nMoeda := GetMv("MV_CENT"+Alltrim(STR(mv_par06)))
	Else
		nMoeda := GetMv("MV_CENT")
	EndIf
	
	If lGestao
		cFilSE5 := FwFilial("SE5")
		cFilSE8 := FWFilial("SE8")
	Else
		cFilSE5 := xFilial("SE5")
		cFilSE8 := xFilial("SE8")
	EndIf
	...
	Else	
		...
		Else 
			...
			If cPaisLoc <> "BRA" .And. mv_par09 == 1
				dDtConvUsa := dDataBase
			Else
				dDtConvUsa := dDtConvSLD
			EndIf
			
			If mv_par07 == 1
				nSaldoAtu := Round(xMoeda(E8_SALATUA, nMoedaBco, mv_par06, dDtConvUsa, nCasDec), nMoeda)
			ElseIf mv_par07 == 2
				nSaldoAtu := Round(xMoeda(E8_SALRECO, nMoedaBco, mv_par06, dDtConvUsa, nCasDec), nMoeda)
			ElseIf mv_par07 == 3
				nSaldoAtu := Round(xMoeda(E8_SALATUA-E8_SALRECO, nMoedaBco, mv_par06, dDtConvUsa, nCasDec), nMoeda)
			EndIf
		EndIf
	...
	Else
		...
		If lTemPar11 //.T. se for todas as filiais
			lAllFil := (mv_par11 == 1)
		EndIf
	EndIf
	...
	If nTxMoedBc <= 1
		If MV_PAR09 == 1
			dDtRecMoeda := dDataBase
		Else
			dDtRecMoeda := (cAliasSE5)->E5_DTDISPO
		EndIf
		
		If cPaisLoc <> "BRA" .And. nMoedaBco > 1
			nMoedaRecMoeda := nMoedaBco
		Else
			nMoedaRecMoeda := mv_par06
		EndIf
		
		nTxMoeda := RecMoeda(dDtRecMoeda, nMoedaRecMoeda)
	EndIf
	...
	nValorPag := 0
	nValorRec := 0
	oMovBanc:Cell("SALDO ATUAL"	   ):SetBlock({|| nSaldoAtu += (nValorMov * F470SinalMov(cAliasSE5))})
	...
	EndIf
	oMovBanc:Cell("x-CONCILIADOS"  ):SetBlock({|| F470Reconc(cAliasSE5)})
	...
	If cPaisLoc <> "BRA"
		If cPaisLoc <> "ANG"
			If mv_par06 <> nMoedaBco .And. mv_par06 > 1
				oMovBanc:Cell("TAXA"):SetBlock({|| F470TaxaMov(cAliasSE5, nTxMoedBc, mv_par06)})
			Else
		...
		Else
			If mv_par06 <> nMoedaBco
				If nMoedaBco>1
					oMovBanc:Cell("TAXA"):SetBlock({|| F470TaxaDtDisp(cAliasSE5, nTxMoedBc, nMoedaBco)})
				Else
					oMovBanc:Cell("TAXA"):SetBlock({|| F470TaxaMsm(cAliasSE5, nTxMoedBc, lMsmMoeda, mv_par06)})
				EndIf
			Else
				oMovBanc:Cell("TAXA"):Disable()
			EndIf
	...
	While !oReport:Cancel() .And. (cAliasSE5)->(!Eof())
		...
		While !oReport:Cancel() .And. !(cAliasSE5)->(Eof())
			...
			If nMoedaBx > 0 .And. nMoedaBx != mv_par06 .And. !Empty((cAliasSE5)->E5_RECPAG) .And. !Empty((cAliasSE5)->E5_MOEDA)
				...
				If MV_PAR09 == 2
					dDataMovi := (cAliasSE5)->E5_DATA
				Else
					dDataMovi := dDataBase
				EndIf
				nCasDec   := nCasDecE1
				cTbl      := "SE1"
				If (cAliasSE5)->E5_TXMOEDA != 1 //Trata gravacao incorreta do E5_TXMOEDA
					nTxMov := (cAliasSE5)->E5_TXMOEDA
				Else
					nTxMov := 0
				EndIf
				...
				If mv_par09 == 1  // Taxa do dia
					...
					Else
						If nMoedaBx>1
							nTaxaMovi := RecMoeda(dDataMovi, nMoedaBx)
						Else
							nTaxaMovi := RecMoeda(dDataMovi, mv_par06)
						EndIf
					EndIf
				Else // Taxa do Movimento
					...
					If mv_par06 > 1						
						...
						Else//Exibir valores em moeda estrangeira
							...
							If nMoedaBx > 1 //Para conversao entre duas moedas estrangeiras, considerar o E5_TXMOEDA como taxa de partida
								nTaxaMovi := nTxMov
			...
			If Len(AllTrim(E5_DOCUMEN)) + Len(AllTrim(E5_NUMCHEQ)) > 35
				If !Empty(AllTrim((cAliasSE5)->E5_DOCUMEN))
					cSepDoc := "-"
				Else
					cSepDoc := " "
				EndIf
				
				cDocument := AllTrim(SUBSTR((cAliasSE5)->E5_DOCUMEN, 1, 20)) + cSepDoc + AllTrim((cAliasSE5)->E5_NUMCHEQ)
			Else
				If Empty((cAliasSE5)->E5_NUMCHEQ)
					cDocument := (cAliasSE5)->E5_DOCUMEN
			Else
					cDocument := (cAliasSE5)->E5_NUMCHEQ
				EndIf
			EndIf
			...
			Else
				If Empty((cAliasSE5)->E5_PREFIXO)
					cSepDoc := " "
				Else
					cSepDoc := "-"
				EndIf
				
				cPref_Tit := (cAliasSE5)->E5_PREFIXO + cSepDoc + (cAliasSE5)->E5_NUMERO
				
				If Empty((cAliasSE5)->E5_PARCELA)
					cSepDoc := " "
				Else
					cSepDoc := "-"
				EndIf
				
				cPref_Tit += cSepDoc + (cAliasSE5)->E5_PARCELA
			EndIf
	...
	For nX := 1 to 5
		...
		oTotal:Cell("NAOCONC"):SetBlock( { || F470TotalCol(nX, aTotais, 2, nMoeda) } )
		oTotal:Cell("CONC"):SetBlock( { || F470TotalCol(nX, aTotais, 3, nMoeda) } )
...
//----------------------------------------------------------------
/*/{Protheus.doc}F470SinalMov
Retorna o sinal (+1/-1) usado no acumulo do saldo atual do
extrato, conforme o movimento seja recebimento ou pagamento.
Extraido de expressao IIf()/If() para respeitar o padrao do
projeto (uso obrigatorio de If/Else/EndIf).

@author Ajuste Grupo Luiz Hohl
@since  30/07/26
@param cAliasReg Alias da query posicionada (SE5)
@return nRet Numeric, +1 para recebimento e -1 para pagamento
/*/
//-----------------------------------------------------------------
Static Function F470SinalMov(cAliasReg)
	Local nRet as Numeric

	If (cAliasReg)->E5_RECPAG == "R"
		nRet := 1
	Else
		nRet := -1
	EndIf
...
//----------------------------------------------------------------
/*/{Protheus.doc}F470Reconc
Retorna o marcador de conciliado usado na coluna x-CONCILIADOS.
Extraido de expressao IIf() para respeitar o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  30/07/26
@param cAliasReg Alias da query posicionada (SE5)
@return cRet Character, "x" se conciliado ou " " se nao
/*/
//-----------------------------------------------------------------
Static Function F470Reconc(cAliasReg)
	Local cRet as Character

	If Empty((cAliasReg)->E5_RECONC)
		cRet := " "
	Else
		cRet := "x"
	EndIf
...
//----------------------------------------------------------------
/*/{Protheus.doc}F470TaxaMov
Calcula a taxa exibida na coluna TAXA quando o pais nao e Brasil
e nao e Angola. Extraido de expressao IIf()/If() para respeitar
o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  30/07/26
@param cAliasReg Alias da query posicionada (SE5)
@param nTaxaBc Taxa da moeda do banco (nTxMoedBc)
@param nMoeda Moeda parametrizada (mv_par06)
@return nRet Numeric
/*/
//-----------------------------------------------------------------
Static Function F470TaxaMov(cAliasReg, nTaxaBc, nMoeda)
	Local nRet as Numeric
	Local dData as Date
	...
	Else
		If MV_PAR09 == 1
			dData := dDataBase
		Else
			dData := (cAliasReg)->E5_DTDISPO
		EndIf

		nRet := RecMoeda(dData, nMoeda)
	EndIf

Return nRet

//----------------------------------------------------------------
/*/{Protheus.doc}F470TaxaDtDisp
Calcula a taxa exibida na coluna TAXA para o pais Angola quando
a moeda do banco e estrangeira. Extraido de expressao IIf()/If()
...
@param cAliasReg Alias da query posicionada (SE5)
@param nTaxaBc Taxa da moeda do banco (nTxMoedBc)
@param nMoeda Moeda do banco (nMoedaBco)
@return nRet Numeric
/*/
//-----------------------------------------------------------------
Static Function F470TaxaDtDisp(cAliasReg, nTaxaBc, nMoeda)
	Local nRet as Numeric

	If nTaxaBc > 1
		nRet := nTaxaBc
	Else
		nRet := RecMoeda((cAliasReg)->E5_DTDISPO, nMoeda)
	EndIf

Return nRet

//----------------------------------------------------------------
/*/{Protheus.doc}F470TaxaMsm
Calcula a taxa exibida na coluna TAXA para o pais Angola quando
a moeda do banco nao e estrangeira. Extraido de expressao
IIf()/If() para respeitar o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  30/07/26
@param cAliasReg Alias da query posicionada (SE5)
@param nTaxaBc Taxa da moeda do banco (nTxMoedBc)
@param lMsm Indica se a moeda do titulo e igual a parametrizada
@param nMoeda Moeda parametrizada (mv_par06)
@return nRet Numeric
/*/
//-----------------------------------------------------------------
Static Function F470TaxaMsm(cAliasReg, nTaxaBc, lMsm, nMoeda)
	Local nRet as Numeric
...
//----------------------------------------------------------------
/*/{Protheus.doc}F470TotalCol
Retorna o valor formatado das colunas NAOCONC/CONC da secao de
totais, exibidas apenas nas linhas de recebimento e pagamento
(indice 2 e 3 de aTotais). Extraido de expressao IIf()/If() para
respeitar o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  30/07/26
@param nLinha Indice da linha corrente em aTotais (nX)
@param aValores Array de totais (aTotais)
@param nCol Coluna do array a exibir (2=nao conciliado, 3=conciliado)
@param nMoeda Moeda usada na formatacao (nMoeda)
@return cRet Character
/*/
//-----------------------------------------------------------------
Static Function F470TotalCol(nLinha, aValores, nCol, nMoeda)
	Local cRet as Character

	If nLinha == 2 .Or. nLinha == 3
		cRet := Transform(aValores[nLinha][nCol], tm(aValores[nLinha][nCol], 16, nMoeda))
	Else
		cRet := ""
	EndIf
...
Static Function CalcSldIni(nSaldoAtu, nSaldoIni) 
	...
	If mv_par06 > 1
		nMoeda := GetMv("MV_CENT"+STR(mv_par06,1))
	Else
		nMoeda := GetMv("MV_CENT")
	EndIf
