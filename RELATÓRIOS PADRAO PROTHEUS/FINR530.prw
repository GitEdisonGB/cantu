...
Static Function ReportPrint(oReport As Object)
	...
	cFilUser	:= ""
	nMoedaBco	:= 1
	nCasas		:= GetMv("MV_CENT"+(F530Sel(mv_par02 > 1 , Alltrim(STR(mv_par02)),"")))
	nMoeda    	:= mv_par02
	nTxMoedBc	:= 0
	cTab14 		:= ""
	nK			:= 0
	...
	If mv_par05 == 1 .And. Len(aSelFil) == 0
		If !lSE8Compar .And. ((lSE5Exclus .Or. (cMAEmpSE5+cMAUniSE5) == "EE"))
	...
	For nK := 1 To Len(aSelFil)
		...
		While !SA6->(Eof()) .And. SA6->A6_FILIAL == xFilial()
			...
			While Eval(bCondWhile)
				If !lSE8Compar .And. lSE5Exclus .And. (ASCan(aSelFil, {|x| AllTrim(x) == AllTrim(SE5->E5_FILIAL)}) == 0)
					SE5->(DbSkip())
					Loop
				ElseIf !lSE5Exclus				
				...
				If SE5->E5_TIPODOC $ "TR/TE" .And. Empty(SE5->E5_NUMERO)
					If !(SE5->E5_MOEDA $ "R$/DO/TB/TC/CH" + cTab14 + F530Sel(cPaisLoc=="BRA","","/$ "))
						SE5->(dbSkip())
						Loop
					Endif
				Endif
				...
				If mv_par02 > 1 .And. !Empty(SE5->E5_VLMOED2)
					...
					nTxMoeda := F530Sel(nTxMoeda=0,1,nTxMoeda)
...
/*/
Static Function fr530Check() As Character
...
Static Function PrimeraFil() As Character
	...
	If lGestao
		...
		ELse
			...
			If FWModeAccess("SE8",2) == "C" //verifica se é compartilhado em nivel unidade de negocio
				cfilUnit := F530Sel(EMPTY(FWAllUnitBusiness(cFilEmp)),"",FWAllUnitBusiness(cFilEmp)[1])
			Else
		...
		cFilResp := cFilEmp + cfilUnit + F530Sel(Len(aRet) > 0, aRet[1], "")
	...
	Else
		...
		Else
			aRet := FWAllFilial()[1]
			cFilResp := F530Sel(Len(aRet) > 0, aRet[1], "")
			cFilResp := xFilial("SE5",cFilResp)
		Endif
	EndIf
...
/*/
//-----------------------------------------------------------
Static Function F530Sel(lCond, xValA, xValB)
	Local uRet
