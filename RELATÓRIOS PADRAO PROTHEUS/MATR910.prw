...
nTamData := F910Sel(__SetCentury(), 10, 8)
...
TRCell():New(oSection2,"NNR_DESCRI","NNR",STR0061,/*Picture*/,/*Tamanho*/,/*lPixel*/, {|| F910Sel(lCusUnif, MV_PAR08, Posicione("NNR",1,xFilial("NNR")+MV_PAR08,"NNR_DESCRI")) })
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
...
Local lImpLivro, lImpTermos
Local nAcho,i,aGrupos:={}
Local cNumSeqTr := ""
Local nTotRegs  := 0
...
cPicD1Cust:= PesqPict("SD1","D1_CUSTO"+F910Sel(mv_par11 == 1, "", Str(mv_par11,1)),18)
lD3Servi := F910Sel(lWmsNew, .F., GetMV('MV_D3SERVI',.F.,'N')=='N')
...
Endif	
oReport:SetTitle( oReport:Title()+AllTrim(F910Sel(mv_par16==1, OemToAnsi(STR0030), OemToAnsi(STR0031))))
...
lIxbConTes := F910Sel(lIxbConTes == NIL, ExistBlock("MTAAVLTES"), lIxbConTes)
...
cWhereD1 := "%" 
cWhereD1 += "AND" 
//Desconsidera Notas Fiscales con Remitos
If cPaisloc <> "BRA" 
	cWhereD1 += " SD1.D1_REMITO = '' AND"
EndIf
If !lCusUnif
...
EndIf
If mv_par16 == 1
//		cOrder += "17,12"+F910Sel(lVEIC,',29',',28')
	cOrder += "12"+F910Sel(lVEIC,',29',',28')
Else
	If lCusUnif
		cOrder += "8,12"+F910Sel(lVEIC,',29',',28')
	Else
//			cOrder += "17,8,12"+F910Sel(lVEIC,',29',',28')
		cOrder += "8"+F910Sel(lVEIC,',29',',28')
	EndIf
EndIf	
cOrder += "%"
...
While !oReport:Cancel() .And. !(cAliasTop)->(Eof()) .And. lImpLivro
	...
	If lCusUnif
		...
		While !Eof() .And. SB2->B2_FILIAL+B2_COD == cSeek
	...
	Else
		dbSelectArea("SB2")
		If !dbSeek(xFilial("SB2")+(cAliasTop)->PRODUTO+mv_par08)
			dbSelectArea(cAliasTop)
			dbSkip()
			Loop
		EndIf
	...
	While !oReport:Cancel() .And. !(cAliasTop)->(Eof()) .And. (cAliasTop)->PRODUTO = cProdAnt
		...
		If Alltrim((cAliasTop)->ARQ) $ "SD1/SD2"
			...
			If lIxbConTes .And. SF4->F4_ESTOQUE != "S"
				...
				lTesNEst := F910Sel(ValType(lTesNEst) # "L", .F., lTesNEst)
			EndIf
...
If lImpTermos // Impressao dos Termos
	...
	oObj := FWSX1Util():New()	
	oObj:AddGroup("MTR910")
    oObj:SearchGroup()
	aGrp := oObj:GetGroup("MTR910")[2]
    
    For nI := 1 To Len(aGrp)
        AAdd(aVariaveis, {Rtrim(Upper('MV_PAR'+StrZero(nI, 2))), &('MV_PAR'+StrZero(nI, 2))})
    Next nI
...
lListaDev := F910Sel(ValType(lListaDev)#"L", GetMV("MV_LISTDEV"), lListaDev)
...
dbSelectArea("SB2")
MsSeek(xFilial("SB2")+(cAliasTop)->PRODUTO+F910Sel(lCusUnif,"",mv_par08))
oSection1:PrintLine()
oSection2:PrintLine()
...
/*/
//-----------------------------------------------------------
Static Function F910Sel(lCond, xValA, xValB)
	Local uRet
