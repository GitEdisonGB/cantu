...
TRCell():New(oCabec,"C5_CLIENTE"	,/*Tabela*/," "                     ,PesqPict("SC5","C5_CLIENTE")	,16								,/*lPixel*/,{|| F700Sel(SC5->C5_TIPO$"BD", STR0048, STR0049) + SC5->C5_CLIENTE                                                    })
TRCell():New(oCabec,"C5_LOJA"		,/*Tabela*/,STR0050					,PesqPict("SC5","C5_LOJACLI")	,TamSx3("C5_LOJACLI"	)[1]	,/*lPixel*/,{|| SC5->C5_LOJACLI 													                                         })
TRCell():New(oCabec,"CNOME"		    ,/*Tabela*/,RetTitle("A1_NOME")	    ,PesqPict("SA1","A1_NOME")		,TamSx3("A1_NOME"		)[1]	,/*lPixel*/,{|| FATPDObfuscate(F700Sel(SC5->C5_TIPO$"BD", SA2->A2_NOME, SA1->A1_NOME), F700Sel(SC5->C5_TIPO$"BD", "A2_NOME", "A1_NOME")) })
...
While !oReport:Cancel() .AND. !( cAliasSC6 )->( Eof() ) .AND. (cAliasSC6)->C6_FILIAL == xFilial("SC6")
	...
	dbSelectArea( cAliasSC6 ) 
	lFiltro := ValidMasc((cAliasSC6)->C6_PRODUTO,MV_PAR05)
	...
	If lFiltro
		...
		If  cNumero + cItem + cProduto <> (cAliasSC6)->C6_NUM + (cAliasSC6)->C6_ITEM + (cAliasSC6)->C6_PRODUTO
			...
			If ( lContInt .AND. Len(aQuant)>0 .AND. mv_par06 <> 3 ) .OR. ( lContInt .AND. mv_par06 == 3 )
				...
				nValDesc := F700Sel(nVlDesc==0, nVlDesc, nValDesc)
				...
				nValDesc  := xMoeda(nValDesc,nC5Moeda,mv_par08,F700Sel(mv_par12 == 1, dC5Emissao, dDataBase))
				nPrcVen   := xMoeda(nPrcVen ,nC5Moeda,mv_par08,F700Sel(mv_par12 == 1, dC5Emissao, dDataBase))
				nImpLinha := xMoeda(nImpLinha,nC5Moeda,mv_par08,F700Sel(MV_PAR12 == 1, dC5Emissao, dDataBase)) 
				nTFat     := xMoeda(nTFat,nC5Moeda,mv_par08,F700Sel(MV_PAR12 == 1, dC5Emissao, dDataBase))
...
While !( cAliasSC6 )->( Eof() ) .AND. (cAliasSC6)->C6_FILIAL == xFilial("SC6")
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	dbSelectArea( cAliasSC6 ) 
	lFiltro := !((!Empty(aReturn[7]).AND.!&(aReturn[7])) .OR. !(ValidMasc((cAliasSC6)->C6_PRODUTO,MV_PAR05)))
	...
	If lFiltro
		...
		If (lCabPed .AND. lContInt .AND. Len(aQuant)>0 .AND. mv_par06 <> 3) .OR. (lCabPed .AND. lContInt .AND. mv_par06 == 3)
			...
			Else
				...
				If msSeek( xFilial()+(cAliasSC6)->C6_CLI+(cAliasSC6)->C6_LOJA )
					...
					EndIf
					@li, PCol()+2 Psay STR0029 + SA1->A1_COD+' - '+ F700Sel(Empty(cCLIObfus), Subs(SA1->A1_NOME,1,40), cCLIObfus)//"CLIENTE : "
				EndIf
			EndIf
		...
		If  cNumero + cItem + cProduto <> (cAliasSC6)->C6_NUM + (cAliasSC6)->C6_ITEM + (cAliasSC6)->C6_PRODUTO
			...
			If ( lContInt .AND. Len(aQuant)>0 .AND. mv_par06 <> 3 ) .OR. ( lContInt .AND. mv_par06 == 3 )
				...
				@li,122 Psay xMoeda(nValDesc,nC5Moeda,mv_par08,F700Sel(mv_par12 == 1, dC5Emissao, dDataBase)) PicTure tm(nValDesc,11)
				@li,134 Psay xMoeda(nPrcVen ,nC5Moeda,mv_par08,F700Sel(mv_par12 == 1, dC5Emissao, dDataBase)) PicTure PesqPict((cAliasSC6),"C6_PRCVEN",13)
				@li,148 Psay Substr (cOp+"-"+cDescTab,1,24)
				If nImpLinha > 0
					@li,174 Psay xMoeda(nImpLinha,nC5Moeda,mv_par08,F700Sel(MV_PAR12 == 1, dC5Emissao, dDataBase)) PicTure PesqPict((cAliasSC6),"C6_VALOR",13)
				EndIf
				If nTFat > 0
					@li,192 Psay xMoeda(nTFat,nC5Moeda,mv_par08,F700Sel(MV_PAR12 == 1, dC5Emissao, dDataBase))    PicTure tm(nTFat,15)
				EndIf
				@li,210 Psay dEntreg
				...
				nTQtde  	+= nQtdVen
				nTQEnt  	+= nQtdEnt
				nTPed   	+= xMoeda(nPrcVen,nC5Moeda,mv_par08,F700Sel(MV_PAR12 == 1, dC5Emissao, dDataBase))				
				nAcTotFat	+= nTFat
	...
	If (cAliasSC6)->C6_NUM  <> cPedido .AND. lImp
		...
		@li,118 Psay xMoeda(nAcDescont,nC5Moeda,MV_PAR08,F700Sel(MV_PAR12 == 1, dC5Emissao, dDataBase))	PicTure tm(nAcDescont,15)
		If nTotImpPar > 0
			@li,170 Psay xMoeda(nTotImpPar,nC5Moeda,MV_PAR08,F700Sel(MV_PAR12 == 1, dC5Emissao, dDataBase))	Picture PesqPict((cAliasSC6),"C6_VALOR",15)
		EndIf
		If nAcTotFat > 0
			@li,192 Psay xMoeda(nAcTotFat,nC5Moeda,MV_PAR08,F700Sel(MV_PAR12 == 1, dC5Emissao, dDataBase))	PicTure tm(nAcTotFat,15)
		EndIf
		
		nTotFat  += xMoeda(nAcTotFat, nC5Moeda,mv_par08,F700Sel(MV_PAR12 == 1, dC5Emissao, dDataBase))
		nTotDesc += xMoeda(nAcDescont,nC5Moeda,mv_par08,F700Sel(MV_PAR12 == 1, dC5Emissao, dDataBase))
		nTotImp  += xMoeda(nTotImpPar,nC5Moeda,mv_par08,F700Sel(MV_PAR12 == 1, dC5Emissao, dDataBase))
...
/*/
//-----------------------------------------------------------
Static Function F700Sel(lCond, xValA, xValB)
	Local uRet
