...
Static lFwCodFil    := .T.
Static lUnidNeg     := F190Sel(lFwCodFil, SubStr(FwSM0Layout(), 1, 1) $ "U", .F.)   // Indica se usa Gestao Corporativa
...
Static Function ReportPrint(oReport As Object)
    ...
    oBreak1     := Nil
    oBreak2     := Nil
    nDecs       := GetMv("MV_CENT" + (F190Sel(MV_PAR12 > 1 , STR(MV_PAR12, 1), "")))
    aRelat      := {}
    nI          := 1
    ...
    lRelMulNat := F190Sel(MV_PAR11 == 2, MV_MULNATP, MV_MULNATR) .And. MV_PAR38 == 2 .And. MV_PAR39 != 2
    ...
    cFilAnt := F190Sel(lFwCodFil, FWGETCODFILIAL, SM0->M0_CODFIL)
...
Static Function InitReport(oReport As Object, aRelat As Array, nI As Numeric, cTotText As Character)
    ...
    oBaixas:Cell("E5_PREFIXO"):SetBlock(    { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_PREFIXO],     "")})
    oBaixas:Cell("E5_NUMERO"):SetBlock(     { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_NUMERO],      "")})
    oBaixas:Cell("E5_PARCELA"):SetBlock(    { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_PARCELA],     "")})
    oBaixas:Cell("E5_TIPODOC"):SetBlock(    { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_TIPO],        "")})
    oBaixas:Cell("E5_CLIFOR"):SetBlock(     { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_CLIFOR],      "")})
    oBaixas:Cell("NOME CLI/FOR"):SetBlock(  { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_NOMECLIFOR],  "")})
    oBaixas:Cell("E5_NATUREZ"):SetBlock(    { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_NATUREZA],    "")})
    oBaixas:Cell("E5_VENCTO"):SetBlock(     { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_VENCIMENTO],  "")})
    oBaixas:Cell("E5_HISTOR"):SetBlock(     { || aRelat[nI][ID_HISTORICO]})
    oBaixas:Cell("E5_DATA"):SetBlock(       { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_DTBAIXA],     "")})
    oBaixas:Cell("E5_VALOR"):SetBlock(      { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_VALORORIG],   Nil)})
    oBaixas:Cell("JUROS/MULTA"):SetBlock(   { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_JUROSMULTA],  Nil)})
    oBaixas:Cell("CORRECAO"):SetBlock(      { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_CORRECAO],    Nil)})
    oBaixas:Cell("DESCONTO"):SetBlock(      { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_DESCONTO],    Nil)})
    oBaixas:Cell("ABATIMENTO"):SetBlock(    { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_ABATIMENTO],  Nil)})
    oBaixas:Cell("IMPOSTOS"):SetBlock(      { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_IMPOSTO],     Nil)})

    If __lTemFKD .And. ExistFunc("FxLoadFK6")
        If !lRelMulNat
            oBaixas:Cell("VALACESS"):SetBlock(  { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_VALORVA],     Nil)})
        Else
            oBaixas:Cell("VALACESS"):SetBlock({ || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", FxLoadFK6(AllTrim("FK" + CValToChar(MV_PAR11)), SE5->E5_IDORIG, "VA")[1][2], Nil)})
        EndIf
    EndIf

    oBaixas:Cell("E5_VALORPG"):SetBlock(    { || aRelat[nI][ID_TOTALPAGO]})
    oBaixas:Cell("E5_BANCO"):SetBlock(      { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_BANCO],       Nil)})
    oBaixas:Cell("E5_DTDIGIT"):SetBlock(    { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_DTDIGITACAO], Nil)})
    oBaixas:Cell("E5_MOTBX"):SetBlock(      { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_MOTIVO],      Nil)})
    oBaixas:Cell("E5_ORIG"):SetBlock(       { || F190Sel(aRelat[nI][ID_TIPODOC] <> "VA", aRelat[nI][ID_FILORIG],     Nil)})
    ...
    TRFunction():New(oBaixas:Cell("E5_VALOR"),,     "SUM", oBreak1, STR0071, TM(SE5->E5_VALOR, oBaixas:Cell("E5_VALOR"):nSize, nDecs), {|| F190Sel(aRelat[nI][ID_LORIGINAL], aRelat[nI][ID_VALORORIG], 0)}, .F., .F.,.F.) //"Sub Total"
    ...
    If lVarFil .And. !(Empty(FwxFilial("SE5")))
        ...
        TRFunction():New(oBaixas:Cell("E5_VALOR"),, "SUM", oBreak2, STR0070, TM(SE5->E5_VALOR, oBaixas:Cell("E5_VALOR"):nSize, nDecs), {|| F190Sel(aRelat[nI][ID_LORIGINAL], aRelat[nI][ID_VALORORIG], 0)}, .F., .F.) //"FILIAL"
    ...
    TRFunction():New(oBaixas:Cell("E5_VALOR"),,     "SUM", oBreak3  , "", TM(SE5->E5_VALOR, oBaixas:Cell("E5_VALOR"):nSize, nDecs),     {|| F190Sel(aRelat[nI][ID_LORIGINAL], aRelat[nI][ID_VALORORIG], 0)} , .F., .F., .F.)
    ...
    TRFunction():New(oBaixas:Cell("CORRECAO"),,     "SUM", oBreak3  , "", TM(SE5->E5_VALOR, oBaixas:Cell("CORRECAO"):nSize, nDecs),     {|| F190Sel(aRelat[nI][ID_LORIGINAL], aRelat[nI][ID_CORRECAO], 0)}  , .F., .F., .F.)
    TRFunction():New(oBaixas:Cell("DESCONTO"),,     "SUM", oBreak3  , "", TM(SE5->E5_VALOR, oBaixas:Cell("DESCONTO"):nSize, nDecs),     {|| F190Sel(aRelat[nI][ID_LORIGINAL] .Or. aRelat[nI][ID_DESCONTO] > 0, aRelat[nI][ID_DESCONTO], 0)}, .F., .F., .F.)
    ...
    TRFunction():New(oBaixas:Cell("IMPOSTOS"),,     "SUM", oBreak3  , "", TM(SE5->E5_VALOR, oBaixas:Cell("IMPOSTOS"):nSize, nDecs),     {|| F190Sel(aRelat[nI][ID_LORIGINAL], aRelat[nI][ID_IMPOSTO], 0)}, .F., .F.,.F.)
    If __lTemFKD
    ...
    TRFunction():New(oBaixas:Cell("E5_VALOR"),,     "SUM", oBreak4  , "", TM(SE5->E5_VALOR, oBaixas:Cell("E5_VALOR"):nSize, nDecs),     {|| F190Sel(aRelat[nI][ID_TEMVALOR], aRelat[nI][ID_VALORORIG], 0)} , .F., .F., .F.)
    TRFunction():New(oBaixas:Cell("JUROS/MULTA"),,  "SUM", oBreak4  , "", TM(SE5->E5_VALOR, oBaixas:Cell("JUROS/MULTA"):nSize, nDecs),  {|| F190Sel(aRelat[nI][ID_TEMVALOR], aRelat[nI][ID_JUROSMULTA], 0)} , .F., .F., .F.)
    TRFunction():New(oBaixas:Cell("CORRECAO"),,     "SUM", oBreak4  , "", TM(SE5->E5_VALOR, oBaixas:Cell("CORRECAO"):nSize, nDecs),     {|| F190Sel(aRelat[nI][ID_TEMVALOR], aRelat[nI][ID_CORRECAO], 0)} , .F., .F., .F.)
    TRFunction():New(oBaixas:Cell("DESCONTO"),,     "SUM", oBreak4  , "", TM(SE5->E5_VALOR, oBaixas:Cell("DESCONTO"):nSize, nDecs),     {|| F190Sel(aRelat[nI][ID_TEMVALOR], aRelat[nI][ID_DESCONTO], 0)} , .F., .F., .F.)
    TRFunction():New(oBaixas:Cell("ABATIMENTO"),,   "SUM", oBreak4  , "", TM(SE5->E5_VALOR, oBaixas:Cell("ABATIMENTO"):nSize, nDecs),   {|| F190Sel(aRelat[nI][ID_TEMVALOR], aRelat[nI][ID_ABATIMENTO], 0)} , .F., .F., .F.)
    TRFunction():New(oBaixas:Cell("IMPOSTOS"),,     "SUM", oBreak4  , "", TM(SE5->E5_VALOR, oBaixas:Cell("IMPOSTOS"):nSize, nDecs),     {|| F190Sel(aRelat[nI][ID_TEMVALOR], aRelat[nI][ID_IMPOSTO], 0)} , .F., .F., .F.)
    If __lTemFKD 
        If !lRelMulNat
            TRFunction():New(oBaixas:Cell("VALACESS"),, "SUM", oBreak4  , "", TM(SE5->E5_VALOR, oBaixas:Cell("VALACESS"):nSize, nDecs),{|| F190Sel(aRelat[nI][ID_TEMVALOR], aRelat[nI][ID_VALORVA], 0)}, .F., .F., .F.)
        Else
            TRFunction():New(oBaixas:Cell("VALACESS"),, "SUM", oBreak4  , "", TM(SE5->E5_VALOR, oBaixas:Cell("VALACESS"):nSize, nDecs),{|| F190Sel(aRelat[nI][ID_TEMVALOR], FxLoadFK6(AllTrim("FK" + CValToChar(MV_PAR11)), SE5->E5_IDORIG, "VA")[1][2], 0)}, .F., .F., .F.)
        EndIf
    EndIf
    TRFunction():New(oBaixas:Cell("E5_VALORPG"),,   "SUM", oBreak4  , "", TM(SE5->E5_VALOR, oBaixas:Cell("E5_VALORPG"):nSize, nDecs),   {|| F190Sel(aRelat[nI][ID_TEMVALOR], aRelat[nI][ID_TOTALPAGO], 0)}  , .F., .F., .F.)
...
Static Function PrintTot(aTotais As Array, oReport As Object, lFil As Logical, nBreak As Numeric, nJ As Numeric)
    ...
    If nBreak <> 3
        ...
        While ((F190Sel((ValType(cAnt) <> ValType(aTotais[nJ][1]) .And. Len(aTotais[nJ]) == 3), "" , cAnt)) ==;
            (F190Sel(((MV_MULNATP .Or. MV_MULNATR) .And. !(aTotais[nJ][1] $ "/") .And. Len(aTotais[nJ]) > 3), aTotais[nJ][4], aTotais[nJ][1])) .And. (nJ < nAscan))
    ...
    Else
        ...
        While nAscan > 0
            ...
            nAscan := F190Sel((nAscan + 1) <= Len(aTotais) .And. aTotais[nAscan + 1][1] == cGeral, nAscan + 1, 0)
        End
    EndIf
...
User Function Fr190TstCond(cFilSE5 As Character) As Logical
    ...
    If lRet .And. MV_PAR14 == 1 .And. !F190MovBco(NEWSE5->E5_MOTBX) .And. !lManual
        lRet := .F.
    EndIf
    ...
    If lRet .And. MV_PAR25 == 2
        ...
        If nMoedaBco == 0 .And. !Empty(NEWSE5->E5_BANCO) .And. !Empty(NEWSE5->E5_AGENCIA) .And. !Empty(NEWSE5->E5_CONTA)
            ...
            EndIf
        EndIf
            nMoedaBco := F190Sel(nMoedaBco == 0, 1, nMoedaBco)
...
Static Function FA190ImpR4(aRet As Array, nOrdem As Numeric, aTotais As Array, oReport As Object, nGerOrig As Numeric, nI As Numeric, cTotText As Character) As Array
    ...
    cFilNome    := Space(15)
    cCliFor190  := ""
    aTam        := F190Sel(MV_PAR11 == 1, TamSX3("E1_CLIENTE"), TamSX3("E2_FORNECE"))
    aColu       := {}
    nDecs       := GetMv("MV_CENT" + (F190Sel(MV_PAR12 > 1 , STR(MV_PAR12, 1), "")))
    nMoedaBco   := 1
    aStru       := SE5->(DbStruct())
    ...
    cEmpresa    := F190Sel(lUnidNeg, FwCodEmp(), "")
    cAge        := ""
    cContaBco   := ""
    cMascNat    := ""
    lConsImp    := .T.
    ...
    nLenSelFil  := 0
    cLayout     := FwSM0Layout()
    lGestao     := F190Sel(lFwCodFil, ("E" $ cLayout .And. "U" $ cLayout), .F.)// Indica se usa Gestao Corporativa
    lExclusivo  := .F.
    aModoComp   := {}
    lMultiNat   := .F.
    nRecChkd    := 0
    ...
    nCasDec     := F190Sel(MV_PAR11 == 1, TamSx3("FK1_TXMOED")[2], TamSx3("FK2_TXMOED")[2])
    cVerifAbat  := ""
    lCalcIRBx   := .F.
    cOperador   := F190Sel(cSGBD $ "MSSQL7", "+", "||")
    ...
    Else
        If MV_PAR17 == 2 // Cons filiais abaixo
            cFilDe := F190Sel(lFwCodFil, FWGETCODFILIAL, SM0->M0_CODFIL)
            cFilAte:= F190Sel(lFwCodFil, FWGETCODFILIAL, SM0->M0_CODFIL)
        Else
    ...
    AEval(aStru, {|Estrutura| F190Sel( Estrutura[DBS_TYPE] <> "M", cCampos += "," + AllTrim(Estrutura[DBS_NAME]), Nil)})
    ...
    If !Empty(MV_PAR42) .And. MV_PAR42 == 2
        cInsert += "WHERE E5_RECPAG = '" + F190Sel(MV_PAR11 == 1, "R", "P") + "' AND "
    Else
        If MV_PAR11 == 1
    ...
    If cPaisLoc == "BRA"
        ...
        Else        
            ...
            cUpdate5 += "AND " + cTable + ".SE5CARTEIR = '" + F190Sel(mv_par11 == 1, "R", "P") + "' "
            cUpdate5 += "AND " + cTable + ".SE5MANUAL = 0 "
            cUpdate5 += "AND " + cTable + ".SE5MAXSEQ = " + cTable + ".E5_SEQ "
            cUpdate5 += "AND " + cTable + ".D_E_L_E_T_ = ' ' "        
        EndIf        
    ...
    aColu := F190Sel(aTam[1] > 6, {023, 027, TamParcela("E1_PARCELA", 40, 39, 38), 042, 000, 022}, {000, 004, TamParcela("E1_PARCELA", 17, 16, 15), 019, 023, 030})
    ...
    If MV_PAR40 == 1
        nSelFil := 0
    Else
        SM0->(DbSeek(cEmpAnt + F190Sel(Empty(cFilDe), "", cFilDe), .T.))
    EndIf
    
    While SM0->(!EoF()) .And. SM0->M0_CODIGO == cEmpAnt .And.  F190Sel(MV_PAR40 == 1, (nSelFil < nLenSelFil) .And. cFilDe <= cFilAte, SM0->M0_CODFIL <= cFilAte)
        If MV_PAR40 == 1
            nSelFil++
        ...
        Else
            ...
            cDlt2Fil  := StrTran(cDlt2Fil, "?2", F190Sel(mv_par11 == 1, cXFilSE1, cXFilSE2))
            
            If TCSqlExec(cDlt2Fil) < 0
                DbOnError(TCSqlError(), "cDelete2")
            EndIf
            
            cUpd5Fil := StrTran(cUpdate5, "?1", F190Sel(mv_par11 == 1, cXFilSE1, cXFilSE2))
            ...
            While NEWSE5->(!EoF()) .And. NEWSE5->E5_FILIAL == cXFilSE5
                ...
                While NEWSE5->(!EoF()) .And. NEWSE5->E5_FILIAL == cXFilSE5 .And. &cCond2 == cAnterior
                    ...
                    If cCarteira == "R"
                        ...
                        dDtMovFin := F190Sel(lManual, CToD("//"), SE1->E1_VENCREA)
                    ...
                    Else
                        ...
                        F190Sel(lBxTit, nRecSE2 := SE2->(Recno()), nRecSE2 := 0)

                        If !lBxTit
                            lBxTit := MsSeek(NEWSE5->E5_FILORIG + cPrefixo + cNumero + cParcela + cTipo + cFornece + cLoja)
                        EndIf
                        
                        dDtMovFin := F190Sel(lManual, CToD("//"), SE2->E2_VENCREA)
                        cCond3  := "2"
                        nDesc   := nJuros := nValor := nMulta := nJurMul := nCM := nVlMovFin := 0
                        cCheque := F190Sel(Empty(NEWSE5->E5_NUMCHEQ), SE2->E2_NUMBCO, NEWSE5->E5_NUMCHEQ)
                    EndIf
                    ...
                    While NEWSE5->(!EoF()) .And. NEWSE5->E5_FILIAL == cXFilSE5 .And.;
                        ...
                        cFilTrb := F190Sel(cCarteira == "R", "SE1", "SE2")
                        ...
                        If !Empty(NEWSE5->E5_NUMERO)
                            If (NEWSE5->E5_RECPAG == "R" .And. !(NEWSE5->E5_TIPO $ MVPAGANT + "/" + MV_CPNEG)) .Or. ;
                                ...
                                If lAchou
                                    cCliFor := F190Sel(MV_PAR30 == 1, GetLGPDValue("SA1", "A1_NREDUZ"), GetLGPDValue("SA1", "A1_NOME"))
                                Else
                            ...
                            Else
                                ...
                                If lAchou
                                    cCliFor := F190Sel(MV_PAR30 == 1, GetLGPDValue("SA2", "A2_NREDUZ"), GetLGPDValue("SA2", "A2_NOME"))
                                Else
                                    cCliFor := Upper(AllTrim(GetLGPDValue("NEWSE5", "E5_BENEF")))
                                EndIf
                            EndIf
                        ...
                        nDesc   += F190Sel(MV_PAR12 == 1 .And. nMoedaBco == 1, NEWSE5->E5_VLDESCO, Round(xMoeda(NEWSE5->E5_VLDESCO, nMoedaBco, MV_PAR12, NEWSE5->E5_DATA, nDecs + 1, nTaxa), nDecs + 1))
                        nJuros  += F190Sel(MV_PAR12 == 1 .And. nMoedaBco == 1, NEWSE5->E5_VLJUROS, Round(xMoeda(NEWSE5->E5_VLJUROS, nMoedaBco, MV_PAR12, NEWSE5->E5_DATA, nDecs +  1, nTaxa), nDecs + 1))
                        nMulta  += F190Sel(MV_PAR12 == 1 .And. nMoedaBco == 1, NEWSE5->E5_VLMULTA, Round(xMoeda(NEWSE5->E5_VLMULTA, nMoedaBco, MV_PAR12, NEWSE5->E5_DATA, nDecs + 1, nTaxa), nDecs + 1))
                        nJurMul += nJuros + nMulta
                        nCM     += F190Sel(MV_PAR12 == 1 .And. nMoedaBco == 1, NEWSE5->E5_VLCORRE, Round(xMoeda(NEWSE5->E5_VLCORRE, nMoedaBco, MV_PAR12, NEWSE5->E5_DATA, nDecs + 1, nTaxa), nDecs + 1))
                        ...
                        If NEWSE5->E5_TIPODOC $ "VL/V2/BA/RA/PA/CP"
                            ...
                            If MV_PAR11 == 2
                                If cPaisLoc == "ARG" .And. !Empty(NEWSE5->E5_ORDREC)
                                    nValor += F190Sel(nMoedMov == MV_PAR12, NEWSE5->E5_VALOR, Round(xMoeda(NEWSE5->E5_VALOR, Val(NEWSE5->E5_MOEDA), MV_PAR12, NEWSE5->E5_DATA, nDecs + 1, NEWSE5->E5_TXMOEDA), nDecs + 1))
                                Else
                                    ...
                                    Else
                                        ...
                                        Else
                                            nValor += Round(xMoeda(NEWSE5->E5_VALOR, nMoedMov, MV_PAR12, NEWSE5->E5_DATA, nDecs + 1, F190Sel(nMoedMov == 1, 0, nTaxa), F190Sel(nMoedMov == 1, nTaxa, 0)), nDecs + 1)
                                        EndIf
                                    EndIf
                                EndIf
                            Else
                                If cPaisLoc <> "BRA" .And. !Empty(NEWSE5->E5_ORDREC)
                                    nValor += F190Sel(MV_PAR12 == 1 .And. nMoedaBco == 1, NEWSE5->E5_VALOR, Round(xMoeda(NEWSE5->E5_VLMOED2, Val(NEWSE5->E5_MOEDA), MV_PAR12, NEWSE5->E5_DATA, nDecs + 1, F190Sel(cPaisLoc == "BRA", NEWSE5->E5_TXMOEDA, 0)), nDecs + 1))
                                Else
                                    If NEWSE5->E5_VLMOED2 > 0 .And. MovMoedEs(NEWSE5->E5_MOEDA, NEWSE5->E5_TIPODOC, NEWSE5->E5_MOTBX, DTOS(NEWSE5->E5_DATA), NEWSE5->E5_RECPAG)
                                        nValor += F190Sel(MV_PAR12 == 2, NEWSE5->E5_VALOR, NEWSE5->E5_VLMOED2)
                                    Else
                            ...
                            If lMVLjTroco
                                lTroco := F190Sel(SubStr(NEWSE5->E5_HISTOR, 1, 3) == "LOJ", .T., .F.)
                                If lTroco
                                    nRecnoSE5 := SE5->(Recno())
                                    DbSelectArea("SE5")
                                    DbSetOrder(7)
                        ...
                        Else
                            nVlMovFin   += F190Sel(MV_PAR12 == 1.And.nMoedaBco == 1, NEWSE5->E5_VALOR, Round(xMoeda(NEWSE5->E5_VALOR, nMoedaBco, MV_PAR12, NEWSE5->E5_DATA, nDecs + 1, nTaxa), nDecs + 1))
                            cHistorico  := F190Sel(Empty(NEWSE5->E5_HISTOR),"MOV FIN MANUAL", NEWSE5->E5_HISTOR)
                            cNatureza   := NEWSE5->E5_NATUREZ
                        EndIf
                    ...
                    If ((nDesc + nValor + nJurMul + nVlMovFin + nAbatLiq) != 0) .Or. (NEWSE5->SE5VA != 0)
                        ...
                        If !lManual
                            ...
                            If cCarteira == "R"
                                ...
                                nVlr := Round(xMoeda(SE1->E1_VALOR, SE1->E1_MOEDA, MV_PAR12, SE1->E1_BAIXA, nDecs + 1, F190Sel(cPaisLoc == "BRA", nTaxa, 0)), nDecs + 1)
                            Else
                                ...
                                nVlImp := 0
                                If lConsImp   
                                    nVlImp := SE2->E2_INSS + F190Sel(GetNewPar("MV_MRETISS", "1") == "1", SE2->E2_ISS, 0) + F190Sel(lCalcIRF, SE2->E2_IRRF, 0)
                                    If ! lPccBaixa  
                                        nVlImp += SE2->E2_VRETPIS + SE2->E2_VRETCOF + SE2->E2_VRETCSL
                                    EndIf
                                EndIf
                                
                                nVlImp  := Round(xMoeda(nVlImp, 1, MV_PAR12, SE2->E2_BAIXA, nDecs + 1, F190Sel(MV_PAR12 == 1, nTaxa, 0), F190Sel(MV_PAR12 > 1, nTaxa, 0)), nDecs + 1)
                                If MV_PAR12 == SE2->E2_MOEDA
                                    nVlr := SE2->E2_VALOR + nVlImp
                                Else
                                    nVlr := Round(xMoeda(SE2->E2_VALOR, SE2->E2_MOEDA, MV_PAR12, SE2->E2_BAIXA, nDecs + 1, F190Sel(MV_PAR12 == 1, nTaxa, 0), F190Sel(MV_PAR12 > 1, nTaxa, 0)) + nVlImp, nDecs + 1)
                                EndIf
                            EndIf
                            aRet[nTamRet, ID_E5RECNO] := nRecSE5
                            MsGoTo(nRecSe5)
                            cFilTrb := F190Sel(cCarteira == "R", "SE1", "SE2")
                            If DbSeek(F190Sel(cFilTrb == "SE1", cXFilSE1, cXFilSE2) + cPrefixo + cNumero + cParcela + cCliFor190 + cTipo)
                                nAbat       := 0
                                lOriginal   := .F.
                            Else
                                If cMaxSeq == cSeq
                                    RecLock("TRB", .T.)
                                    Replace linha With F190Sel(cFilTrb == "SE1", cXFilSE1, cXFilSE2) + cPrefixo + cNumero + cParcela + cCliFor190 + cTipo
                                    MsUnlock()
                                EndIf
                            EndIf
                        Else
                            ...
                            aRet[nTamRet, ID_E5RECNO] := nRecSE5
                            MsGoTo(nRecSe5)
                            nVlr := Round(xMoeda(E5_VALOR, nMoedaBco, MV_PAR12, E5_DATA, nDecs + 1,, F190Sel(cPaisLoc == "BRA", nTaxa, 0)), nDecs + 1)
                        ...
                        If cCarteira == "R"
                            If (!lManual)
                                If MV_PAR13 == 1 
                                    cHistorico := F190Sel(Empty(cHistorico), SE1->E1_HIST, cHistorico)
                                Else
                                    cHistorico := F190Sel(Empty(SE1->E1_HIST), cHistorico, SE1->E1_HIST)
                                EndIf
                            EndIf
                            ...
                            aRet[nTamRet][ID_NATUREZA] := F190Sel(Len(AllTrim(cNatureza)) > 8, cNatureza, cMascNat)
                            If Empty(dDtMovFin) .Or. dDtMovFin == Nil
                                dDtMovFin := CToD("  /  /  ")
                            EndIf
                            aRet[nTamRet][ID_VENCIMENTO]    := F190Sel(lManual, dDtMovFin, SE1->E1_VENCREA) //Vencto
                        ...
                        Else
                            If MV_PAR13 == 1  
                                cHistorico := F190Sel(Empty(cHistorico), SE2->E2_HIST, cHistorico)
                            Else
                                cHistorico := F190Sel(Empty(SE2->E2_HIST), cHistorico, SE2->E2_HIST)
                            EndIf
                            If aTam[1] <= 6 .And. !lManual
                                If lBxTit
                                    aRet[nTamRet][ID_CLIFOR] := SE2->E2_FORNECE
                            ...
                            If Empty(dDtMovFin) .Or. dDtMovFin == Nil
                                dDtMovFin := CToD("  /  /  ")
                            EndIf
                            aRet[nTamRet][ID_VENCIMENTO] := F190Sel(lManual, dDtMovFin, SE2->E2_VENCREA)
                            If !Empty(cCheque)
                        ...
                        aRet[nTamRet][ID_VALORPG]   := F190Sel(nVlMovFin <> 0, nVlMovFin, F190Sel(F190MovBco(cMotBaixa), nValor, 0))
                        aRet[nTamRet][ID_TEMVALOR]  := aRet[nTamRet][ID_VALORPG] == 0 .Or. NEWSE5->SE5VA != 0
                        nTotOrig    += F190Sel(lOriginal, nVlr, 0)
                        nTotBaixado += F190Sel(cTipoDoc $ "CP/BA" .And. cMotBaixa $ "CMP/FAT", 0, nValor) //não soma, pois já somou no principal
                        nTotDesc    += nDesc
                        nTotJurMul  += nJurMul
                        nTotCM      += nCM
                        nTotAbLiq   += nAbatLiq
                        ...
                        nTotComp    += F190Sel(cTipoDoc == "CP", nValor, 0)
                        nTotFat     += F190Sel(cMotBaixa $ "FAT", nValor, 0)
        ...
        If MV_PAR17 == 1 .And. SM0->(RecCount()) >= 1
            If nFilBaixado > 0
                AAdd(aTotais, {F190Sel(lFwCodFil, FWGETCODFILIAL, SM0->M0_CODFIL), STR0028, nFilBaixado})  //"Baixados"
            EndIf
            If nFilMovFin > 0
                AAdd(aTotais, {F190Sel(lFwCodFil, FWGETCODFILIAL, SM0->M0_CODFIL), STR0031, nFilMovFin})  //"Mov Fin."
            EndIf
            If nFilComp > 0
                AAdd(aTotais, {F190Sel(lFwCodFil, FWGETCODFILIAL, SM0->M0_CODFIL), STR0037, nFilComp})  //"Compens."
            EndIf
            If nFilFat > 0
                AAdd(aTotais, {F190Sel(lFwCodFil, FWGETCODFILIAL, SM0->M0_CODFIL), STR0076, nFilFat})  //"Compens."
            EndIf
...
/*/
Static Function F190MovBco(cMotBx As Character) As Logical
    ...
    If !(Empty(cMotBx))
        If (nPos := AScan(_aMotBaixa, {|motivo_baixa| SubStr(motivo_baixa, 1, 3) == Upper(cMotBx)})) > 0
            lRet := F190Sel(SubStr(_aMotBaixa[nPos], 19, 1) == "S", .T., .F.)
        EndIf
    EndIf
...
/*/
//-----------------------------------------------------------
Static Function F190Sel(lCond, xValA, xValB)
	Local uRet
