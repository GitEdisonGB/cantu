...
Local nSizeRem := 0
	...
	TRCell():New(oItCarga , "DAI_NFISCA" ,"DAI" , /*Titulo*/ , /*Picture*/                  , /*Tamanho*/             , /*lPixel*/ , /*{|| code-block de impressao }*/)
...
Local lDaiRedesp  := DAI->( ColumnPos( "DAI_REDESP" ) ) > 0
	...
	TRPosition():New(oCarga,"SC5",1,{|| F020Fil('SC5', DAI->DAI_FILPV)+DAI->DAI_PEDIDO})
	...
	TRPosition():New(oItCarga,"SF2",1,{||F020Fil('SF2', DAI->DAI_FILPV)+DAI->(DAI_NFISCA+DAI_SERIE)+SC5->(C5_CLIENTE+C5_LOJACLI)}) //NF
	TRPosition():New(oItCarga,"SA1",1,{||F020Fil('SA1', DAI->DAI_FILPV)+DAI->(DAI_CLIENT+DAI_LOJA)}) //Cliente
	TRPosition():New(oItCarga,"SA2",1,{||F020Fil('SA2', DAI->DAI_FILPV)+DAI->(DAI_CLIENT+DAI_LOJA)}) //Fornecedor - Necessário para tipos de documento B e D
	If lDaiRedesp
		TRPosition():New(oItCarga,"SA4",1,{||F020Fil('SA4', DAI->DAI_FILPV)+DAI->(DAI_REDESP)}) //Redespachante
	EndIf
	If lAnalitico
		...
		TRPosition():New(oItPedido,"SB1",1,{||F020Fil('SB1', DAI->DAI_FILPV)+(cAliasSC9)->C9_PRODUTO}) //Produto
	EndIf
	If MV_PAR12 == 1
...
Local lNotaFis   := MV_PAR13 == 2      //Indica se o relatório deve apresentar apenas cargas faturadas
	...
	TRPosition():New(oCarga,"DA4",1,{|| xFilial("DA4")+DAK->DAK_MOTORI })
	//Posiciona nas tabelas secundárias da seção 2 (Itens da Carga)
	TRPosition():New(oItCarNF,"SD2",3,{||F020Fil('SD2', SF2->F2_FILIAL)+SF2->(F2_DOC+F2_SERIE+F2_CLIENTE+F2_LOJA)}) //NF
	TRPosition():New(oItCarNF,"SA1",1,{||F020Fil('SA1', SF2->F2_FILIAL)+SF2->(F2_CLIENTE+F2_LOJA)}) //Cliente
	TRPosition():New(oItCarNF,"SA2",1,{||F020Fil('SA2', SF2->F2_FILIAL)+SF2->(F2_CLIENTE+F2_LOJA)}) //Fornecedor - Necessário para tipos de documento B e D
	If lAnalitico
		//Posiciona nas tabelas secundárias da seção 3 (Pedidos dos Itens da Carga)
		TRPosition():New(oItNotFis,"SD2",3,{||(cAliasSD2)->(D2_FILIAL+D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA+D2_COD+D2_ITEM)}) //Pedido
		TRPosition():New(oItNotFis,"SB1",1,{||F020Fil('SB1', SF2->F2_FILIAL)+(cAliasSD2)->D2_COD}) //Produto
	EndIf
	If MV_PAR12 == 1
		oCarga:SetPageBreak(.T.)
	EndIf
...
Local lNotaFis   := MV_PAR13 == 2      //Indica se o relatório deve apresentar apenas cargas faturadas
	...
	TRPosition():New(oPrdCarga,"SB1",1,{||F020Fil('SB1', (cAliasSC9)->C9_FILIAL)+(cAliasSC9)->C9_PRODUTO}) //Produto
	TRPosition():New(oPrdCarga,"SF2",1,{||F020Fil('SF2', (cAliasSC9)->C9_FILIAL)+(cAliasSC9)->(C9_NFISCAL+C9_SERIENF+C9_CLIENTE+C9_LOJA)}) //NF
	If MV_PAR12 == 1
		oCarga:SetPageBreak(.T.)
	EndIf
	oCarga:Print()
...
Local cFilSC9 := ""
	If lFilPad
		cFilSC9 := xFilial("SC9")
	Else
		cFilSC9 := "%"+OsFilQry("SC9","DAI.DAI_FILPV")+"%"
	EndIf
...
Local cFilSB5  := ""
Local cFilSB1  := ""
	cFilSB5 := F020Fil("SB5", (cAliasSC9)->C9_FILIAL)
	cFilSB1 := F020Fil("SB1", (cAliasSC9)->C9_FILIAL)
...
Local cFilSB5   := ""
Local cFilSB1   := ""
	cFilSB5 := F020Fil("SB5", (cAliasSD2)->D2_FILIAL)
	cFilSB1 := F020Fil("SB1", (cAliasSD2)->D2_FILIAL)
...
//----------------------------------------------------------
/*/{Protheus.doc}F020Fil
Retorna a filial usada para posicionar/consultar a tabela: a
filial padrao do usuario (xFilial) quando o metodo padrao de
entrega/coleta esta ativo (lFilPad), senao a filial calculada
por OsFilial() a partir da chave do documento de origem.
Extraido de expressao IIf() (repetida 12x em codeblock + 4x em
declaracao Local) para respeitar o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param cTabela Alias da tabela a posicionar (ex. "SC5", "SB1")
@param cChave Chave do documento de origem usada por OsFilial()
@return cRet Character
/*/
//-----------------------------------------------------------
Static Function F020Fil(cTabela, cChave)
	Local cRet as Character

	If lFilPad
		cRet := xFilial(cTabela)
	Else
		cRet := OsFilial(cTabela, cChave)
	EndIf
