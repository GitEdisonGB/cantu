...
cPerg := F680Sel(cPaisLoc == "BRA", "MTR680", "MR680A")
...
TRCell():New(oPedNEnt,"EMISSAO"		,"TRB",RetTitle("C5_EMISSAO"	),PesqPict("SC5","C5_EMISSAO"	),TamSx3("C5_EMISSAO"	)[1],/*lPixel*/,{|| F680Sel(oReport:nDevice == 4, DtoC(TRB->EMISSAO), TRB->EMISSAO)	        },/*cAlign*/,/*lLineBreak*/)
...
TRCell():New(oPedNEnt,"PRODUTO2"	,"TRB",RetTitle("C6_PRODUTO"	),PesqPict("SC6","C6_PRODUTO"	),TamSx3("C6_PRODUTO"	)[1],/*lPixel*/,{|| F680Sel(TRB->GRADE == 'S', Substr(TRB->PRODUTO,1,nTamRef), TRB->PRODUTO)},/*cAlign*/,/*lLineBreak*/)
...
TRCell():New(oPedNEnt,"DATENTR"	,"TRB"	,RetTitle("C6_ENTREG")	,PesqPict("SC6","C6_ENTREG"	),nTamData				      ,/*lPixel*/ ,{|| F680Sel(oReport:nDevice == 4, DtoC(TRB->DATENTR), TRB->DATENTR)	},/*cAlign*/,/*lLineBreak*/)	// Data de Entrega
...
TRCell():New(oProduto,"EMISSAO"		,"TRB",RetTitle("C5_EMISSAO"	),PesqPict("SC5","C5_EMISSAO"	),TamSx3("C5_EMISSAO"	)[1],/*lPixel*/,{|| F680Sel(oReport:nDevice == 4, DtoC(TRB->EMISSAO ), TRB->EMISSAO)	},/*cAlign*/,/*lLineBreak*/)
TRCell():New(oProduto,"DATENTR"		,"TRB",RetTitle("C6_ENTREG"		),PesqPict("SC6","C6_ENTREG"	),nTamData                  ,/*lPixel*/,{|| F680Sel(oReport:nDevice == 4, DtoC(TRB->DATENTR), TRB->DATENTR)   },/*cAlign*/,/*lLineBreak*/)
...
TRCell():New(oCliente,"EMISSAO"		,"TRB",RetTitle("C5_EMISSAO"	),PesqPict("SC5","C5_EMISSAO"	),TamSx3("C5_EMISSAO"	)[1],/*lPixel*/,{|| F680Sel(oReport:nDevice == 4, DtoC(TRB->EMISSAO), TRB->EMISSAO)	 },/*cAlign*/,/*lLineBreak*/)
...
TRCell():New(oCliente,"DATENTR"		,"TRB",RetTitle("C6_ENTREG"		),PesqPict("SC6","C6_ENTREG"	),nTamData                  ,/*lPixel*/,{|| F680Sel(oReport:nDevice == 4, DtoC(TRB->DATENTR), TRB->DATENTR)    },/*cAlign*/,/*lLineBreak*/)
...
TRCell():New(oData,"DATENTR"	,"TRB",RetTitle("C6_ENTREG"		),PesqPict("SC6","C6_ENTREG"	),nTamData,/*lPixel*/,{|| F680Sel(oReport:nDevice == 4, DtoC(TRB->DATENTR), TRB->DATENTR)							 },/*cAlign*/,/*lLineBreak*/)
TRCell():New(oData,"NUM"		,"TRB",RetTitle("C6_NUM"		),PesqPict("SC6","C6_NUM"		),TamSx3("C6_NUM"		)[1],/*lPixel*/,{|| TRB->NUM 														 },/*cAlign*/,/*lLineBreak*/)
TRCell():New(oData,"EMISSAO"	,"TRB",RetTitle("C5_EMISSAO"	),PesqPict("SC5","C5_EMISSAO"	),TamSx3("C5_EMISSAO"	)[1],/*lPixel*/,{|| F680Sel(oReport:nDevice == 4, DtoC(TRB->EMISSAO), TRB->EMISSAO)		 },/*cAlign*/,/*lLineBreak*/)
...
TRCell():New(oVendedor,"EMISSAO"	,"TRB",RetTitle("C5_EMISSAO"	),PesqPict("SC5","C5_EMISSAO"	),TamSx3("C5_EMISSAO"	)[1],/*lPixel*/,{|| F680Sel(oReport:nDevice == 4, DtoC(TRB->EMISSAO), TRB->EMISSAO)	 },/*cAlign*/,/*lLineBreak*/)
...
TRCell():New(oVendedor,"DATENTR"	,"TRB",RetTitle("C6_ENTREG"		),PesqPict("SC6","C6_ENTREG"	),nTamData,/*lPixel*/,{|| F680Sel(oReport:nDevice == 4, DtoC(TRB->DATENTR), TRB->DATENTR)						 },/*cAlign*/,/*lLineBreak*/)
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oReport:SetTitle(oReport:Title() + " - " + F680Sel(nSection == 1, STR0031, F680Sel(nSection==2, STR0032, F680Sel(nSection==3, STR0033, F680Sel(nSection==4, STR0034, STR0035)))) + " - " + GetMv("MV_MOEDA"+STR(nMoeda,1)))	// "Relacao de Pedidos nao entregues"###"Por Pedido"###"Por Produto"###"Por Cliente"###"Por Dt.Entrega"###"Por Vendedor"
...
While !oReport:Cancel() .And. !Eof()
	...
	If !&cQuebra
		...
		oSecao:SetTotalText(F680Sel(nSection == 1, STR0031 + " " + cNum, F680Sel(nSection==2, STR0032 + " " + cProduto, F680Sel(nSection==3, STR0033 + " " + cCli, F680Sel(nSection==4, STR0034 + " " + DTOC(dEntreg), STR0035  + " " + cVde)))))		// "Total do Pedido"###"Total do Produto"###"Total do Cliente"###"Total da Dt.Entrega"###"Total do Vendedor"
...
nTipVal := F680Sel(cPaisLoc == "BRA", MV_PAR19, MV_PAR20)
					...
					If (Empty(cFilSA3)) .Or. (!Empty(cFilSA3) .And. (&cFilSA3))
						...
						Else
							...
							Else
								...
								nValor := F680Sel(nValor<0, 0, nValor)
							EndIf
						EndIf
...
Local lImpTot   := .F.
Local lContinua := .T.
Local nMoeda    := F680Sel(cPaisLoc == "BRA", MV_PAR18, 1)
Local nTVGeral  := 0
Local nTEGeral  := 0
Local nTSGeral  := 0
Local nTIGeral  := 0
...
nTipo   := F680Sel(aReturn[4]==1, 15, 18)
nOrdem  := aReturn[8]
...
While !Eof() .And. lContinua
	...
	IF (nFirst = 0 .And. nOrdem != 4).Or. nOrdem == 4
		...
		Do Case
			Case nOrdem = 1
				...
				EndIf
				@li, 18 Psay Left(CLIENTE+"-"+LOJA+"/"+ F680Sel(Empty(cCLIObfus), TRB->NOMECLI, cCLIObfus), 50)
							
			Case nOrdem = 2
				@li,  0 Psay F680Sel(GRADE=="S" .And. MV_PAR12 == 1, Substr(PRODUTO,1,nTamRef), PRODUTO)
				@li, 16 Psay Left(TRB->DESCRICAO, 30)
			Case nOrdem = 3
				@li,  0 Psay Left(CLIENTE+"-"+LOJA+"/"+F680Sel(Empty(cCLIObfus), NOMECLI, cCLIObfus), 50)
			Case nOrdem = 4
				If cNumPed+cNumCli+DtoS(dData) != NUM+CLIENTE+DtoS(DATENTR)
					@li,  1 Psay DATENTR
					@li, 14 Psay NUM
					@li, 21 Psay EMISSAO
					@li, 34 Psay Left(CLIENTE+"-"+LOJA+"/"+F680Sel(Empty(cCLIObfus), NOMECLI, cCLIObfus), 50)
					cNumPed := NUM
					cNumCli := CLIENTE
				Else
					li--
				...
				dData := DATENTR
			Case nOrdem = 5
				@li,  0 Psay Left(TRB->VENDEDOR+" "+F680Sel(Empty(cCLIObfus), NOMEVEN, cCLIObfus), 47)
		EndCase
	...
	Do Case
		Case nOrdem = 1
			@li, 69 Psay ITEM
			@li, 72 Psay F680Sel(GRADE=="S" .And. MV_PAR12 == 1, Substr(PRODUTO,1,nTamRef), PRODUTO)
		...
		Case nOrdem = 2
			...
			@li, 57 Psay EMISSAO
			@li, 70 Psay DATENTR
			@li, 83 Psay Left(TRB->CLIENTE+"-"+LOJA+"/"+F680Sel(Empty(cCLIObfus), TRB->NOMECLI, cCLIObfus),50)
		Case nOrdem = 3
			@li, 51 Psay NUM
			@li, 58 Psay ITEM
			@li, 61 Psay EMISSAO
			@li, 74 Psay F680Sel(GRADE == "S" .And. MV_PAR12 == 1, Substr(PRODUTO,1,nTamRef), PRODUTO)
			@li, 91 Psay Left(TRB->DESCRICAO,30)
			@li,122 Psay DATENTR
		Case nOrdem = 4
			@li, 85 Psay ITEM
			@li, 88 Psay F680Sel(GRADE == "S" .And. MV_PAR12 == 1, Substr(PRODUTO,1,nTamRef), PRODUTO)
	...
	@li,136 Psay F680Sel(GRADE=="S" .And. MV_PAR12 == 1, nTotVen, QUANTIDADE)	PICTURE PesqPictQt("C6_QTDVEN",12)
	@li,149 Psay F680Sel(GRADE=="S" .And. MV_PAR12 == 1, nTotEnt, ENTREGUE)	PICTURE PesqPictQt("C6_QTDENT",12)
...
For nInd := 1 to nQtdElem
	If  !lMemo .AND. aStruSC6[nInd,02] == "M"
		Loop
	EndIf
	cRetFldQry	+= F680Sel(!Empty(cRetFldQry), ',', '') + cAlias + '.' + AllTrim(aStruSC6[nInd,01])
Next nInd
Return cRetFldQry
...
/*/
//-----------------------------------------------------------
Static Function F680Sel(lCond, xValA, xValB)
	Local uRet
