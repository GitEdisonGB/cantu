...
TRCell():New(oSection,'B1_DESC'	  ,'SB1',STR0039,/*Picture*/,F260Sel(oReport:GetOrientation() == 1, 50, Nil),/*lPixel*/,/*{|| code-block de impressao }*/)
...
If lRet
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	aMV_PAR[18] := F260Sel(((aMV_PAR[18] < 1) .Or. (aMV_PAR[18] > 5)), 1, aMV_PAR[18])
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	TRPosition():New(oSection,"SB1",1,{|| F260Sel(FWModeAccess("SB1") == "E" .And. FWModeAccess("SB2") == "E", (cAliasTRB)->FILIAL+(cAliasTRB)->CODIGO, xFilial("SB1")+(cAliasTRB)->CODIGO) })
...
If !lVisualiz
	...
	For nCount := 1 To Len(aSM0)
		If aSM0[nCount,1] == cGrEmp .And. !aSM0[nCount][11]
			...
			If ASCAN(aNotAccess, cFil) == 0
				AADD(aNotAccess, cFil)
			EndIf
		EndIf
	Next nCount
	...
	dDataRef := F260Sel(Empty(aMV_PAR[22]), dDataBase, aMV_PAR[22])
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	If !Empty(cFilUser)
		cWhere +=  + F260Sel(Empty( cWhere ), " " + cFilUser, " AND " + cFilUser)
	EndIf
	cWhere += ""
	...
	If (aMV_PAR[17] == 3)
		...
		cQuery += "	   			WHERE "+ F260Sel(Empty( cWhereD1 ), " SD1.D1_FILIAL IS NOT NULL ", cWhereD1) +" "
		...
		cQuery += "	   			WHERE "+ F260Sel(Empty( cWhereD2 ), " SD2.D2_FILIAL IS NOT NULL ", cWhereD2) +" "
		...
		cQuery += "			  	WHERE "+ F260Sel(Empty( cWhereD3 ), " SD3.D3_FILIAL IS NOT NULL ", cWhereD3) +" "
	...
	cQuery += "   	   "+ F260Sel(Empty( cCampos ), '', cCampos) +" "

	cQuery += " FROM "+ RetSQLName( 'SB2' ) +" SB2 "
	cQuery += " 	INNER JOIN "+ RetSQLName( 'SB1' ) +" SB1 ON ( "+ F260Sel(Empty( cWhereB1 ), " SB1.B1_COD = SB2.B2_COD ", cWhereB1 +" AND SB1.B1_COD = SB2.B2_COD ") +" ) "
	cQuery += " 	INNER JOIN "+ RetSQLName( 'NNR' ) +" NNR ON ( "+ F260Sel(Empty( cWhereNNR ), " NNR.NNR_CODIGO = SB2.B2_LOCAL ", cWhereNNR +" AND NNR.NNR_CODIGO = SB2.B2_LOCAL ") +" ) "
	...
	cQuery += " WHERE "+ F260Sel(Empty( cWhereB2 ), "", cWhereB2 + " AND ") +" "
	If !Empty( aMV_PAR[11] )
	...
	AEval( aStrucSB1, { | x | F260SetField(x, cAliasSB2) } )
	AEval( aStrucSB2, { | x | F260SetField(x, cAliasSB2) } )

	If (aMV_PAR[17] == 3)
		AEval( aStrucSB9, { | x | F260SetField(x, cAliasSB2) } )
	EndIf
	...
	While !oReport:Cancel() .And. !Eof()
		...
		Do Case
			Case (aMV_PAR[17] == 1)
				nQuant := F260Sel(aMV_PAR[24]==1, ConvUM( (cAliasSB2)->B2_COD, (cAliasSB2)->B2_QATU, (cAliasSB2)->B2_QTSEGUM, 2 ), (cAliasSB2)->B2_QATU)

			Case (aMV_PAR[17] == 2)
				nQuant := F260Sel(aMV_PAR[24]==1, ConvUM( (cAliasSB2)->B2_COD, (cAliasSB2)->B2_QFIM, (cAliasSB2)->B2_QFIM2, 2 ), (cAliasSB2)->B2_QFIM)

			Case (aMV_PAR[17] == 3)
				If lNoRunCalc
					nQuant := F260Sel(aMV_PAR[24]==1, ( cAliasSB2 )->B9_QISEGUM, ( cAliasSB2 )->B9_QINI)
				Else
					nQuant := (aSaldo := CalcEst( (cAliasSB2)->B2_COD,(cAliasSB2)->B2_LOCAL,dDataRef+1,(cAliasSB2)->B2_FILIAL ))[ F260Sel(aMV_PAR[24]==1, 7, 1) ]
				EndIf
			...
			Case (aMV_PAR[17] == 5)
				nQuant := (aSaldo := CalcEstFF( (cAliasSB2)->B2_COD,(cAliasSB2)->B2_LOCAL,dDataRef+1,(cAliasSB2)->B2_FILIAL ))[ F260Sel(aMV_PAR[24]==1, 7, 1) ]
		EndCase
		...
		If lAglutLoc .Or. lAglutFil
			...
			Do Case
				Case (nOrdem == 1)
					If (aMV_PAR[19] == 1)
						cSeek += F260Sel(aMV_PAR[24]==1, (cAliasSB1)->B1_SEGUM, (cAliasSB1)->B1_UM)
					EndIf
					cSeek += F260Sel(!lVeic, (cAliasSB2)->B2_COD, (cAliasSB2)->B1_CODITE)+F260Sel(lAglutFil, "", (cAliasSB2)->B2_FILIAL)+F260Sel(lAglutLoc, "", (cAliasSB2)->B2_LOCAL)
				Case (nOrdem == 2)
				...
				Case (nOrdem == 3)
					If (aMV_PAR[19] == 1)
						cSeek += F260Sel(aMV_PAR[24]==1, (cAliasSB1)->B1_SEGUM, (cAliasSB1)->B1_UM)
					EndIf
					cSeek += PADR((cAliasSB1)->B1_DESC,LEN((cAliasTRB)->DESCRI))+F260Sel(!lVeic, (cAliasSB2)->B2_COD, (cAliasSB2)->B1_CODITE)+F260Sel(lAglutFil, "", (cAliasSB2)->B2_FILIAL)+F260Sel(lAglutLoc, "", (cAliasSB2)->B2_LOCAL)
				Case (nOrdem == 4)
				...
				Case (nOrdem == 5)
					If (aMV_PAR[19] == 1)
						cSeek += F260Sel(aMV_PAR[24]==1, (cAliasSB1)->B1_SEGUM, (cAliasSB1)->B1_UM)
					EndIf
					cSeek += F260Sel(!lVeic, (cAliasSB2)->B2_COD, (cAliasSB2)->B1_CODITE)+F260Sel(lAglutFil, "", (cAliasSB2)->B2_FILIAL)
				OtherWise
					If (aMV_PAR[19] == 1)
						cSeek += F260Sel(aMV_PAR[24]==1, (cAliasSB1)->B1_SEGUM, (cAliasSB1)->B1_UM)
					EndIf
					cSeek += F260Sel(!lVeic, (cAliasSB2)->B2_COD, (cAliasSB2)->B1_CODITE)+F260Sel(lAglutLoc, "", (cAliasSB2)->B2_LOCAL)
			EndCase
		EndIf
...
DEFAULT aSizeVL	:= TamSX3( "B2_VATU1")
	...
	aCampos :={	;
				...
				{"TIPO"    , "C", 02                                       , 00},;
				{"GRUPO"   , "C", 04                                       , 00},;
				{"DESCRI"  , "C", F260Sel(lTamDesc == 1, 50, TamSX3("B1_DESC")[1]), 00},;
				{"UNDMED"  , "C", 02                                       , 00},;
...
//----------------------------------------------------------
/*/{Protheus.doc}F260Sel
Seletor generico usado no lugar de IIf()/If() ternario, para
respeitar o padrao do projeto (uso obrigatorio de If/Else/EndIf).
So deve ser usado quando os dois valores nao tem efeito colateral
condicional (nesse caso o efeito seria sempre executado, ja que
os parametros sao avaliados antes da chamada).
...
/*/
//-----------------------------------------------------------
Static Function F260Sel(lCond, xValA, xValB)
	Local uRet
...
//----------------------------------------------------------
/*/{Protheus.doc}F260SetField
Define o campo da tabela temporaria (TcSetField) quando o tipo
do campo nao e Memo. Extraido de expressao IIf() usada so pelo
efeito colateral para respeitar o padrao do projeto.

@author Ajuste Grupo Luiz Hohl
@since  01/08/26
@param aCampo Estrutura do campo (DbStruct())
@param cAliasTab Alias da tabela temporaria
@return Nil
/*/
//-----------------------------------------------------------
Static Function F260SetField(aCampo, cAliasTab)
	If aCampo[2] <> "C"
		TcSetField(cAliasTab, aCampo[1], aCampo[2], aCampo[3], aCampo[4])
	EndIf
