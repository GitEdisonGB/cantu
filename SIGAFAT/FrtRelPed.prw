...
User Function FrtRelPed(cCarg)
Local oReport
Local oDlg1
Local lCont := .F.
Private cCarga := cCarg 
...
if Empty(cCarga)
	cCarga := Space(9)
	@ 150,1 TO 280,300 DIALOG oDlg1 TITLE "Informe a carga a ser impressa"
	@ 015,004 Say "Carga: "
	@ 014,050 Get cCarga Size 60, 10 Picture "@!" When .T. F3 "ZZSROM" 
...
oReport := TReport():New("MATR730",STR0050,"", {|oReport| ReportPrint(oReport,oPreNota,@nItem,aItemPed,aCabPed)},STR0051 + " " + STR0052)	// "Emissao da Confirmacao do Pedido"###"Emissao da confirmacao dos pedidos de venda, de acordo com"###"intervalo informado na opcaoo Parametros."
...
#IFDEF TOP
	If TcSrvType() <> "AS/400"
		...
		%Exp:cQryAd%
		FROM %Table:SC5% SC5, %Table:SC6% SC6, %Table:ZZS% ZZS
		WHERE
		ZZS.ZZS_NUMROM = %Exp:cCarga% AND 
		ZZS.ZZS_PEDIDO = SC5.C5_NUM AND ZZS.ZZS_EMPRES = %Exp:cEmpAnt% AND 
		ZZS.ZZS_FILORI = SC5.C5_FILIAL AND 
