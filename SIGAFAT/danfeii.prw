...
User Function PrtNfeSef(cIdEnt, cVal1		, cVal2		, oDanfe,;
	...
	If nTipo <> 1
		...
		If lExistNFe
			...
			// Quando impressão por PDF, verifica se veio do e-commerce, caso positivo, gera o XML para envio ao site Martins.
			If oDanfe:nDevice == IMP_PDF .And. cEmpAnt $ "01/10"
				FWMsgRun(, {|| U_IntEcom({"7", , , oDanfe:cPathPDF + StrTran(oDanfe:cFileName, ".rel", ".pdf")})}, "Processando", "Enviando informações para Martins...")
			EndIf
						...
						If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
							...
							Else
								...
								If MsSeek(xFilial("SF2")+aNotas[nX][05]+aNotas[nX][04]+aNotas[nX][06]+aNotas[nX][07]) .And. Alltrim(aXML[nX][8])$"1,3,4,6,7" .Or. ( Alltrim(aXML[nX][8]) $ "2,5"  .And. !Empty(cAutoriza) )
									RecLock("SF2")
									If !SF2->F2_FIMP$"D"
										//SF2->F2_FIMP := "S" Padrao
										//Edison G. Barbieri Incio 10/01/22
										...
										//Edison G. Barbieri fim 10/01/22
							...
							if aNotas[nX][02]=="S" .And. (lGeraPDF)
								...
								While SD2->D2_FILIAL + SD2->D2_DOC + SD2->D2_SERIE == SF2->F2_FILIAL + SF2->F2_DOC + SF2->F2_SERIE .And. ;
										SD2->D2_LOCAL $ "VINHOS/AVARIA/E-COMM/MARTIN" .And. SD2->D2_CF != "5927" .and. SD2->D2_EMISSAO >= ctod("01/01/2021")
									/*Edison 21/11/20 tratado para não enviar DT, CFOP 5927*/
									/*Edison 01/02/21 tratado para não integrar notas antigas a existencia da CDR */
									...
									if nPos == 0
										//Edison G. Barbieri
										//Inicio 01/09/21 buscar o campo C5_REDESP (SC5) para integrar na CDR campo OBS da tabela pedido
										dbSelectArea("SC5")
										dbSetOrder(1)
										MsSeek(xFilial("SC5")+SD2->D2_PEDIDO)

										aAdd(aPedXml, {SD2->D2_PEDIDO, SD2->D2_FILIAL, SA1->A1_CGC, AllTrim(SA1->A1_NOME), SubStr(SF2->F2_VEND1, 2), SF2->F2_TRANSP, SF2->F2_DOC, SF2->F2_EST, SA1->A1_MUN, SA1->A1_CEP, SA1->A1_COMPENT, SC5->C5_REDESP})
										// Fim  01/09/21
									endIf
...
dbSetOrder(1)
	...
	*/
	...
	// Edison G. Barbieri 02/04/25
	//tratado para carregar o CNPJ do cliente na impressão da Danfe
	//Inicio
    Do Case
	...
	oDanfe:Say(028, 102, NoChar(oDestino:_XNome:TEXT,lConverte) + " "  + cAux, oFont07:oFont)//Roberto - 02/04/2013 - Imprime o nome do cliente no canhoto.
	// Fim Edison 02/04/25
	/**CANTU-FIM**/
	...
	aTamCol := {45, 169, 34, 16, 20, 14, 62, 42, 48, 39, 34, 24, 31, 25}
...
Static Function SimpDanfe(oDanfe,oNfe,cCodAutSef,cModalidade,oNfeDPEC,cCodAutDPEC,cDtHrRecCab,dDtReceb,aNota)
	...
	//25/11/2014 Gustavo - Tratamento para imprimir ou não data de saída na Danfe
	If SuperGetMv("MV_X_DTNFE",,"S") == "S"
		oDanfe:Say(264,503,aHrEnt[01],oFont08:oFont)
		oDanfe:Say(233,504,Iif( Empty(aDest[4]),"",ConvDate(aDest[4]) ),oFont08:oFont)
	End
	/**CANTU-FIM**/
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
