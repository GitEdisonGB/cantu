...
//Declaração de Strings
	...
	local cICMSZFM      := ""
	Local cMensCpl		:= ""
	Local cCodCST		:= Alltrim(Upper(GetNewPar("MV_CODCST", "DF;SC;PR=90;RJ;RS=10,90")))
	Local cCsosn2		:= ""
	Local cBarra 	    := ""
	Local cBarTrib 	    := ""
	local cUF			:= ""
...
//Parametro Logico - Define se sera impresso a 2a. Unidade de Medida para operacoes dentro do País - Opcoes: [.T.]-Não Imprime ou [.F.]-Imprime (Default)
	...
	Private aTransR   	:= {} //Edison G. Barbieri 30/08/22 Alimentar e-mail da transportadora de redespacho wine
					...
					*/
								...
								*/
								...
								If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
									...
									ElseIf !(IIF( SF2->(FieldPos("F2_MENNOTA")) > 0, AllTrim(SF2->F2_MENNOTA),AllTrim(SC5->C5_MENNOTA)) $ cMensCli)
										// Cantu Edison G. Barbieri 13/06/2024 devido ao tamanho do campo F2_MENNOTA ser menor que C5_MENNOTA a mensagem ficava incompleta e duplicada.
										//cMensCli += IIF( SF2->(FieldPos("F2_MENNOTA")) > 0, AllTrim(SF2->F2_MENNOTA),AllTrim(SC5->C5_MENNOTA))
										// comentado o padrao acima e deixado para alimentar a várivel cMensCli sempre com o campo do pedido  C5_MENNOTA
							...
							/**CANTU-INICIO**/
									...
									If SD2->(dbSeek(xFilial("SD2")+ SF2->F2_DOC + SF2->F2_SERIE + SF2->F2_CLIENT))
										While (SD2->D2_FILIAL == SF2->F2_FILIAL .And. SD2->D2_DOC == SF2->F2_DOC .And. SD2->D2_SERIE == SF2->F2_SERIE)
											...
											// Edison G. Barbieri inicio 16/05/23
											If Empty(cMenorg) .and. SubStr(SD2->D2_GRUPO,1,2) == "02"
												cMenorg := "Declaração de Transação Comercial - Produto produzido de acordo com os regulamentos técnicos aplicáveis conforme a Lei Brasileira 10.831 para produtos orgânicos, certificado pelo Tecpar Cert "
											EndIf
											// Edison G. Barbieri fim 16/05/23
											SD2->(dbSkip())
										EndDo
										//aPedido := {Iif(SC5->(FieldPos("C5_NTEMPEN")) > 0,Alltrim(SC5->C5_NTEMPEN),""),AllTrim(SC6->C6_PEDCLI),""}
										aPedido := {Iif(SC5->(FieldPos("C5_NTEMPEN")) > 0,Alltrim(SC5->C5_NTEMPEN),""),AllTrim(SC6->C6_PEDCLI),"",SC5->C5_VEND1, cPedidos, cMenorg}
									EndIf
							/**CANTU-FIM**/
					...
					If Upper(SM0->M0_ESTENT) $ cCodCST //TAG cBenef buscar o conteúdo da tabela 5.2 no sistema quando for do PR.
						...
						If MsSeek(cChvCdv)
							...
							//Edison G.Barbieri
							cPrdCbnef := AllTrim((cAliasSD2)->D2_COD)
							cPrdCfop := AllTrim((cAliasSD2)->D2_CF)
					...
					EndIf
								...
								cUmnew := ""
								if alltrim(cCodProd) $ "01050997|01050976|01051013|01050931|01050918"
									cUmnew := "BLS"
								else
								...
								aadd(aProd,	{Len(aProd)+1,;
									cCodProd,;
									IIF(cImpUni=='2',SB5->B5_2CODBAR,IIf(Val(SB1->B1_CODBAR)==0,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0))),; //Edison dia 03/08/20, tratado para cliente GPA enviar segunda unidade IIf(Val(SB1->B1_CODBAR)==0,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0)),;
									cDescProd,;
								...
								SB1->B1_EX_NCM,;
									cD2Cfop,;
									cUmnew,;  // Jean 22/04 - Alterado para considerar segunda un. de medida
								...
								IIF(!((cAliasSD2)->D2_TIPO$"IP" .Or. ((cAliasSD2)->D2_TIPO $ "D" .And. cTpOrig == "P")) ,IIF(!(lMvNFLeiZF),(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR-nDesVrIcms,(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR - ((cAliasSD2)->D2_DESCZFP+(cAliasSD2)->D2_DESCZFC+nDesVrIcms)),IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0)),;
									IIF(Empty(SB5->B5_UMDIPI),cUmnew,SB5->B5_UMDIPI),; // Jean 22/04 - Alterado para considerar segunda un. de medida
						...
						*/
                        ...
                        If !Empty(SC5->C5_X_UFEMB)
							...
							aAdd(aDados,{"NAT_EXP","Natureza" ,"" }) //Edison G. Barbieri 27/11/25
	...
	Else
		...
		If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				If SF1->F1_TIPO $ "DB" .or. (SF1->F1_TIPO == "5" .and. SF1->F1_TPCOMPL <> "1") //D-Devolucao / B-Beneficiamento / 5-Credito com 3 -Transferencia de Credito na sucessao
					dbSelectArea("SA1")
					dbSetOrder(1)
					...
					*/
					...
					If Upper(SM0->M0_ESTENT) $ cCodCST	//TAG cBenef buscar o conteúdo da tabela 5.2 no sistema quando for do PR.
						...
						If MsSeek(cChvCdv)
							...
							//Edison G.Barbieri
							cPrdCbnef := AllTrim((cAliasSD1)->D1_COD)
							cPrdCfop := AllTrim((cAliasSD1)->D1_CF)
							cCodlan := getCodLan( alltrim(SM0->M0_ESTENT), cCST, cCodCST )
						EndIF
					Else
						cCodlan := ""
					...
					*/
						...
						If !Empty(aNota)
							...
							If lPe01Nfe
								...
								aParam := {aProd,cMensCli,cMensFis,aDest,aNota,aInfoItem,aDupl,aTransp,aEntrega,aRetirada,aVeiculo,aReboque,aNfVincRur,aEspVol,aNfVinc,aDetPag,aObsCont,aProcRef,aMed,aLote,aTransR}
								...
								If ( Len(aParam) >= 5 )
									...
									endIf
									if len(aParam) >= 21
										aTransR := aParam[21]  // Edison G. Barbieri 30/08/22 E-mail transp redespacho wine
									endIf
								EndIf
							Endif
	...
	*/
	...
	If Len(aTransp) > 0
		If !Empty(aDest[16]) .and. !Empty(AllTrim(aTransp[07]))
			...
			If Len(aTransR) > 0
				cMailTrans := ";"+AllTrim(aTransp[07]) + ";"+AllTrim(aTransR[01][01]) //Edison G.barbieri 22/08/22 Enviar para transportadora Redespacho
			else
...
If Len(aPedido) > 0
	...
	//-- Edison G. Barbieri 15/05/21 Inicio

	If SC5->(dbSeek(xFilial("SC5") + aPedido[05]))

		If (!Empty(SC5->C5_RASTR))
			cPedTrans := SUBSTR(SC5->C5_RASTR,1,6)
			cNotaTrans := SUBSTR(SC5->C5_RASTR,8,9)
			cString += "Pedido de transferencia: " + cPedTrans + ". "
			cString += "Nota de transferencia: " + cNotaTrans + ". "
		EndIf
	Endif
	//-- Edison G. Barbieri 15/05/21 Fim

	cString += 'Pedido(s): ' + aPedido[05] + ' (' + aPedido[04] + ')  - Cliente: ' + SF2->F2_CLIENTE + '-' + SF2->F2_LOJA + cTipoCli + ". " + aPedido[06]
	//-- Inicio Edison G. Barbieri 01/11/21
	If cEmpAnt = "10"
		cString += "DÚVIDAS LIGUE: " + SuperGetMv("MV_TELCOB",,"11-941922387") + ". "
	else
		cString += 'CANAL DE ÉTICA CANTU: 0800 517 1230 Prezado Cliente, NAO ACEITAMOS DEVOLUCAO SEM NOTA FISCAL e junto a ela deve vir boleto ou dados para deposito: Banco, Agencia e conta E-mail para envio da nota fiscal de devolução e xml: nfe.cantu@centralxml.com.br ' + ""
	EndIf
	//-- Fim Edison G. Barbieri 01/11/21
...
// Datafrete - Início Cantu.
If SuperGetMV("DTF_INTDTF", .F., .F.) .And. !Empty(SC5->C5_XSTSDTF)
	cString += '<obsCont xCampo="datafrete_pednum">'
	cString += '<xTexto>' +SC5->C5_NUM+ '</xTexto>'
	cString += '</obsCont>'
EndIf
// Datafrete - Fim Cantu.
...
static function getCodLan( cUF, cCST, cCodCST )
	...
	if !empty(cUF) .and. !empty(cCST)
		...
		case  cUF == "PR"
			...
			if cCST == "90" .and. cEmpAnt $ "01/02/03/04/05/06/07/09/13/14/15/16/17/40/41/44/45/46"

				cCodlan := "SEM CBENEF"

			elseif cCST == "40" .and. cEmpAnt $ "01/02/03/04/05/06/07/09/13/14/15/16/17/40/41/44/45/46"
				cCodlan := "PR810128"

			elseif cCST == "41" .and. cEmpAnt $ "01/02/03/04/05/06/07/09/13/14/15/16/17/40/41/44/45/46"
				cCodlan := "PR809998"

			elseif cCST == "50" .and. cEmpAnt $ "01/02/03/04/05/06/07/09/13/14/15/16/17/40/41/44/45/46"
				If cPrdCbnef $ "17041404"
					cCodlan := "PR840024"
				else
					cCodlan := "PR849998"
				EndIf
			elseif cCST == "51" .and. cEmpAnt $ "01/03/04/05/06/07/09/13/14/15/16/17/40/41/44/45/46"
				cCodlan := "PR830001"

			elseif cCST == "51" .and. cEmpAnt $ "02"
				cCodlan := "PR830002"

			elseif cCST == "20" .and. cEmpAnt $ "40" .and. cFilAnt == "02" // Edison G. Barbieri 12/12/23
				If cPrdCbnef $ "01080001"
					cCodlan := "PR820009"
				Endif
			elseif cCST == "20" .and. cEmpAnt $ "40" .and. cFilAnt == "04" // Edison G. Barbieri 27/08/22
				cCodlan := "PR820009"

			elseif cCST == "20"  .and. cEmpAnt $ "40" .and. cFilAnt == "14" // Edison G. Barbieri 14/07/23
				If cPrdCbnef $ "04050197/04050198"
					cCodlan := "PR820055"
				elseif cPrdCbnef $ "04020040"
					cCodlan := "PR810021"
				elseif cPrdCbnef $ "04090069/04090070/04090191/04090192"
					cCodlan := "PR820009"
				EndIf

			elseif cCST == "70"  .and. cEmpAnt $ "40" .and. cFilAnt == "14" // Edison G. Barbieri 14/07/23
				if cPrdCbnef $ "04050197"
					cCodlan := "PR820055"
				Endif
			endif
		...
		case cUF == "RJ"
			...
			ElseIf cCST == "20" .and. cEmpAnt == "10" .and. cFilAnt == "05"
				cCodlan := "RJ820449"
		...
		case cUF == "SC"
			...
			Elseif cCST == "41"
				cCodlan := "SC800018"
			Elseif cCST == "20"
				If cPrdCbnef $ "02010346/02010347/02070012"
					cCodlan := "SC820030"
				endif
			endif

		case cUF == "SP"
			if cCST == "41" .and. cPrdCfop == "5906"
				cCodlan := "SP070030"
			endif	
...
Return cAmbiente == "2" .or. Date() >= SuperGetMV("MV_2500240",,ctod("02/09/2026"))	

Static Function Nt250024D(cAmbiente)
Return cAmbiente == "2" .or. Date() >= SuperGetMV("MV_250024D",,ctod("01/10/2026"))	
...
if (!lSaida)
	If !(SF1->F1_TIPO $ "BD") .And. (SA2->A2_TipoRur == "F")
		...
		if SA2->A2_X_LIMCS == "1" // Edison G. Barbieri 27/05/25 ajustado regra calculo para quando tem liminar
			cMsgFunR := "FunRural: " + TransForm(SF1->F1_ValMerc * 0.015, "@E 999,999.99") + chr(13) + chr(10) + "Ref. NF do Produtor n.º "
		else
			cMsgFunR := "FunRural: " + TransForm(SF1->F1_ValMerc * 0.0020, "@E 999,999.99") + chr(13) + chr(10) + "Ref. NF do Produtor n.º "
		endif
