...
//³JEAN - 04/09/14 Criação de arquivo temporário sobre a execução da rotina, devido a execução N vezes pelo schedule decorrente das threads³
...
IF alltrim(oXML:_Pedido:_Dados_Cliente:_Tipo:TEXT) == "J" //  não importa se por pessoa jurídica
...
If fTemPed(cChvUnq)
	...
	If pTipo == 1 // JEAN 04/09/14 Conteúdo anterior: If pTipo = 1
	...
	If lImporta
		...
		//³JEAN - 04/09/14 - CHAMADA DA FUNÇÃO PARA MONTAGEM DA CHAVE UNICA³
		...
		If fTemPed(cChvUnq)
			...
			Else //VERIFICAR CAMPO CUSTOMIZADO
				...
				If (Valtype(oXML:_Pedido:_formas_de_pagamento:_pagamento) == 'A') // se for array é mais de um pagamento, gravar a primera TID
			...
			Else
				...
				//APÓS A ENTRADA DO PEDIDO, SE FOR PESSOA JURIDICA, BLOQUEAR O CADASTRO PARA QUE NAO SEJA FATURADO AUTOMATICAMENTE    
...
±±ºPrograma  ³fChave    ºAutor  ³Jean Carlos Saggin  º Data ³  04/09/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Função utilizada para criar a chave única de gravação      º±±
±±º          ³ no alias SC5                                               º±±
