...
//  |66| FERIAS VENCIDAS
aPer := fPerAquisitivo()
If Len(aPer) = 0 .or. (Len(aPer) == 1 .and. aPer[1,3] < 30)
...
If ( Val(cVal69) > 0 )
	nPosFixa := aScan(aHomV, {|x| x[5] $ "111/250"})
	oPrint:say (nL+nPT,&("nCl0" + cValToChar(nXCol) + "a" )+30 , STR0069, oFont08)
	oPrint:say(nL+nPT+28,&("nCl0" + cValToChar(nXCol) + "a" )+30 ,If(nPosFixa > 0, Space(1) +Str(aHomV[nPosFixa,2])+STR0151, Space(6)+STR0151), oFont08)
Else
	oPrint:say (nL+nPT,&("nCl0" + cValToChar(nXCol) + "a" )+30 , STR0069 , oFont08)
EndIf
...
oPrint:say (nL+nPD,175 ,SUBSTR(If( Len(aInfo) >= 27 .And. !Empty( aInfo[27] ), Capital(aInfo[27]), Capital(aInfo[8]) )+Space(20),1,20), oFont10 ) //"|01- CNPJ: 
...
If lHomolog
	...
	oPrint:Box(nL,135,nL+((nAddL+nPT)*5.3) + 540,nColMax) //BOX
...
fGetPerAtual(@aPerAtual, xFilial("RCH",SRA->RA_FILIAL), SRA->RA_PROCES, fGetCalcRot('4'))
If !Empty(aPerAtual)
...
If !Empty(dDTPenSal)              
	...
	If nValPenSal == 0 .AND. (MesAno(dDTPenSal) >= MesAno(dDataDe) .AND. MesAno(dDTPenSal) <= MesAno(dDataAte))
