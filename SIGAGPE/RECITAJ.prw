...
±±³ Tania      ³04/08/2010³16698/³Inclusao FieldPos para o campo RA_DEPSF.  ³±±
±±³            ³          ³2010  |                                          ³±±±±±±±±±±±±±±±±±
±±ÃÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Programador ³ Data     ³ FNC            ³  Motivo da Alteracao                          ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Luis Ricardo³21/03/2012³00000006688/2012³Compatibilizacao do rdmake conforme GPER030.   ³±±
±±³Cinalli	   ³	      ³Chamado: TERMAI ³					                           ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Norberto M  ³29/03/2012³00000007966/2012³Compatibilizacao do rdmake conforme GPER030.   ³±±
±±³de Melo	   ³          ³Chamado: TESSJA ³						                       ³±±
±±³Luis Artuso ³14/06/2012³00000015579/2012³Ajuste na impressao de totais do recibo. Caso  ³±±
±±³		       ³ 	      ³		     TFEQV9³Haja algum valor superior a 1.000.000,00 , o va³±±
±±³		       ³ 	      ³		           ³lor nao era impresso por causa da picture.     ³±±
±±³Ademar Jr.  ³24/01/2013³Requisito:RHU210³-Inclusao de nova Ordenacao utilizando o campo ³±±
±±³            ³          ³    	           ³ RA_DEPTO, o qual foi liberado para toda folha.³±±
±±ÀÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±     
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
User Function recItaj(lTerminal,cFilTerminal,cMatTerminal,cMesAnoRef,nRecTipo,cSemanaTerminal)

//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Define Variaveis Locais (Basicas)                            ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Local cString:="SRA"        // alias do arquivo principal (Base)
Local aOrd   := {STR0001,STR0002,STR0003,STR0004,STR0005,STR0134,STR0135} //"Matricula"###"C.Custo"###"Nome"###"Chapa"###"C.Custo + Nome"###"Depto."###"Depto. + Nome"
...
cDeptoDe   := IF( !( lTerminal ),mv_par21,SRA->RA_DEPTO 		)	//Depto. De
cDeptoAte  := IF( !( lTerminal ),mv_par22,SRA->RA_DEPTO 		)	//Depto. Ate
...
Private cPict2  	:= "@E 999,999,999.99"
Private cPict3 		:= "@E 999,999.99"
If MsDecimais(1) == 0
	cPict1	:=	"@E 99,999,999,999"
	cPict2 	:=	"@E 99,999,999,999"
	cPict3 	:=	"@E 99,999,999"
Endif
	...
	If	cMesCorrente == cMesArqRef  .or. right(cMesCorrente,4)+left(cMesCorrente,2) == mesano(ddataref) .Or. ;
		...
		If Esc == 1
			If ( Right(cMesAnoRef,4)+Left(cMesAnoRef,2) > Right(cMesCorrente,4)+Left(cMesCorrente,2) ) .Or.;
				( If(MESANO(DATE()) == cAnoMesCorr,day(date()) < nTCFDADT,If ( MesAno(Date()) < cAnoMesCorr, .T., .F.) ) )
				Return( IF( lTerminal <> NIL .And. lTerminal , cHtml , NIL ) )
			EndIf
...
Local nContr  := 0
Local nContrT :=0
Private nLinhas := 15              // Numero de Linhas do Miolo do Recibo
...
If cPaisLoc == "BRA"
	@ LI,92+7 PSAY STR0123 + Desc_CGC	//"CNPJ  : "
Else
...
If !Empty(SRA->RA_DEPTO)
	@ LI,46   PSAY STR0134 + ": " + SRA->RA_DEPTO + " - " + fDesc('SQB',SRA->RA_DEPTO,'QB_DESCRIC')	//-9caracteres+30caracteres
EndIf
...
Local nCol := If(aLanca[nConta,1]="P",43,If(aLanca[nConta,1]="D",56,27))
...
If cPaisLoc <> "ARG"
	...
	@ LI,aPos[2] PSAY STR0033+SRA->RA_BCDEPSAL+"-"+substr(DescBco(SRA->RA_BCDEPSAL,SRA->RA_FILIAL),1,25)	//"| CREDITO:"
...
±±³Fun‡…o    ³AjustaSx1      ³ Autor ³ Luiana Silveira  ³ Data ³18/05/2010³±±
...
aHelp := {	"Informe se deseja imprimir as Bases ",;
			"Utilizadas para o Cálculo do Pagamento ",;
			"para impressão Pre-Impresso ou e-mail.",;
			"Ex.:  Base de FGTS, IRRF, INSS, etc."}
aHelpE:= ""
aHelpI:= ""
cHelp := ".GPR03020."
PutSX1Help("P"+cHelp,aHelp,aHelpI,aHelpE)
AADD( aPrgs ,{"GPR030    ","20","Imprime Bases"         ,"",    "",    "mv_chk","N" ,1   ,0   ,1   ,"C",""    ,"mv_par20","Sim","Yes","Si" ,""   ,""   ,"Nao","No","No",""   ,""   ,"","","" ,""  ,""     ,""   ,""     ,""     ,""   ,""   ,""   ,""    ,""     ,""   ,"" ,""    ,"S"  ,aHelp   ,aHelpI   ,aHelpE  , "" })



aHelp := {	"Informe o codigo do Departamento inicial."}
aHelpE:= ""
aHelpI:= ""
cHelp := ".GPR03021."
PutSX1Help("P"+cHelp,aHelp,aHelpI,aHelpE)
AADD( aPrgs ,{"GPR030    ","21","Depto. De ?"    ,"¿De Sector ?",    "From Department ?",  "mv_chl","C" ,9   ,0   ,1   ,"G",""    ,"mv_par21",""   ,""   ,""   ,""   ,""   ,""   ,""   ,""   ,""   ,""   ,"","","" ,""  ,""     ,""   ,""     ,""     ,""   ,""   ,""   ,""    ,""     ,""   ,"SQB" ,""    ,"S"  ,aHelp   ,aHelpI   ,aHelpE  , ".RHDPTDE." })

aHelp := {	"Informe o codigo do Departamento final."}
aHelpE:= ""
aHelpI:= ""
cHelp := ".GPR03022."
PutSX1Help("P"+cHelp,aHelp,aHelpI,aHelpE)
AADD( aPrgs ,{"GPR030    ","22","Depto. Ate ?"   ,"¿A Sector ?",    "To Department ?",     "mv_chm","C" ,9   ,0   ,1   ,"G",""    ,"mv_par22",""   ,""   ,""   ,""   ,""   ,""   ,""   ,""   ,""   ,""   ,"","","" ,""  ,""     ,""   ,""     ,""     ,""   ,""   ,""   ,""    ,""     ,""   ,"SQB" ,""    ,"S"  ,aHelp   ,aHelpI   ,aHelpE  , ".RHDPTAT." })

ValidPerg( aPrgs, "GPR030", .T.)
RestArea( aArea )
Return Nil
