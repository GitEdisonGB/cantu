...
// Verifica se a chamada é via Menu ou JOB
If	Select('SX2') == 0						
	RPCSetType( 3 )						//Não consome licensa de uso
	RpcSetEnv('50','01',,,,GetEnvServer(),{ "SM2" })
...
/* Adriano Regressao()							//Executa a Projeção das Moedas
...
//Real
If CTP->(DbSeek(xfilial('CTP')+Dtos(dData)+'01'))
	RecLock('CTP',.F.)		
Else
	RecLock('CTP',.T.)
		CTP->CTP_DATA	:= dData
...
//Dolar
If CTP->(DbSeek(xfilial('CTP')+Dtos(dData)+'02'))
	RecLock('CTP',.F.)
Else
	RecLock('CTP',.T.)
		CTP->CTP_DATA	:= dData
...
//Ufir	
If CTP->(DbSeek(xfilial('CTP')+Dtos(dData)+'03'))
	RecLock('CTP',.F.)
Else
	RecLock('CTP',.T.)
		CTP->CTP_DATA	:= dData
...
//Euro
If CTP->(DbSeek(xfilial('CTP')+Dtos(dData)+'04'))
	RecLock('CTP',.F.)
Else
	RecLock('CTP',.T.)
		CTP->CTP_DATA	:= dData
...
//Yen	
If CTP->(DbSeek(xfilial('CTP')+Dtos(dData)+'05'))
	RecLock('CTP',.F.)
Else
	RecLock('CTP',.T.)
		CTP->CTP_DATA	:= dData
