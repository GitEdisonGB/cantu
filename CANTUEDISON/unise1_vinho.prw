...
±±ºPrograma  UniSE1W    ºAutor  ³Edison G Barbieri   º Data ³  20/09/21   º±±
...
//VERIFICA SE ESTA RODANDO VIA MENU OU SCHEDULE
	If Select("SX6") == 0
		...
		conout("UniSE1W-INICIO SEM INTERFACE.")
		lEnd:=""
		cData:=dtos(date())
		...
		conout("UniSE1W-CHAMADA DA FUNCAO PARA TITULOS A RECEBER.")
		GravaCSV(lEnd, cGetCam+"CTAREC_VINHO_"+cData+cHora+cMinutos, dGetDtIni, dGetDtFim, "1")

		conout("UniSE1W-CHAMADA DA FUNCAO PARA TITULOS A PAGAR.")
		GravaCSV(lEnd, cGetCam+"CTAPAG_VINHO_"+cData+cHora+cMinutos, dGetDtIni, dGetDtFim, "2")

		conout("UniSE1W-FIM SEM INTERFACE.")
		RESET ENVIRONMENT

		Return
	EndIf

	conout("UniSE1W-INICIO COM INTERFACE.")
...
// Define propriedades dos campos baseado no SX3
	...
	If nHandle > 0
		...
		if SETMP->(EOF())
			conout("UniSE1W-SEM DADOS A PROCESSAR.")
			SETMP->(DbCloseArea())
			Return
		EndIf
