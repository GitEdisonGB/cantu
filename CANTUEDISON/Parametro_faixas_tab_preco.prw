...
/*/{Protheus.doc} Altparfx
Permissão para alterar EB_XFAIXAS
@type function
@version
...
/*/
User Function Altparfx()
	local aArea
	Private cCadastro := "Parametro validacao de faixas tab preco "
	Private aRotina := {}
	Private cAlias := "SX6"
	Private nOpc := 4
  ...
  // seta o filtro , foi usado o método ao invés do string pq tava dando erro
	...
	cFiltro := "SX6->X6_VAR == 'EB_XFAIXAS'"

	dbSelectArea("SX6")
	Set Filter To &cFiltro

 	// tem que ter a variavel pq é usada no GetDados
  	aAdd( aRotina, {"Alterar"    ,'U_Mod3Mnt',0,nOpc})
...
Static Function Mod2aHeader()
  ...
  aAdd(aHeader,{"Faixas","X6_CONTEUD","@!",1,0,"AllwaysTrue()", "","C","","R"})
	...
	For nI := 1 To Len( aREG )
		If nI <= Len( aREG )
			...
			If !aCOLS[nI, Len(aHeader)+1]  // avalia se foi alterado (o último campo guarda esta informação)
                ...
                PUTMV(FieldGet(FieldPos("X6_VAR")), aCols[nI, 4])
			Endif
...
//+--------------------------------------------------------------------+
//| Rotina | Mod3MntX | Autor | Robson Luiz (rleg)  | Data | 01.01.2007 |
//+--------------------------------------------------------------------+
//| Descr. | Rotina para Visualizar, Alterar e Excluir dados.          |
//+--------------------------------------------------------------------+
//| Uso    | Para treinamento e capacitação.                           |
//+--------------------------------------------------------------------+
User Function Mod3Mnt(cAlias)
