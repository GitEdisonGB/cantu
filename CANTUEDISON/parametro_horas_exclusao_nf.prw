...
//-------------------------------------------------------------------
/*/{Protheus.doc} Permissão para alterar MV_SPEDEXC
description
@type function
@version
...
/*/
//-------------------------------------------------------------------
User Function libexcnf()
	local aArea
	Private cCadastro := "Parametro para liberacao de horas, exclusao NF "
	Private aRotina := {}
	Private cAlias := "SX6"
	Private nOpc := 4
  ...
  // seta o filtro , foi usado o método ao invés do string pq tava dando erro
	...
	cFiltro := "SX6->X6_VAR == 'MV_SPEDEXC'"

	dbSelectArea("SX6")
	Set Filter To &cFiltro

 	// tem que ter a variavel pq é usada no GetDados
  	aAdd( aRotina, {"Alterar"    ,'U_Mod3hora',0,nOpc})
...
Static Function Mod2aHeader()
  ...
  aAdd(aHeader,{"Horas","X6_CONTEUD","@!",1,0,"AllwaysTrue()", "","C","","R"})
